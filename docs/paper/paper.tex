...
\item \textbf{Finite extensional $S$} (a \cppinline{std::set<T>} or \cppinline{std::unordered_set<T>}, lifted via \cppinline{ambient_set}): the kernel pair is finite, and its coequalizer is computed by hash deduplication --- exactly what \cppinline{std::unordered_set::insert} does to incoming values.
The trivial elementwise loop in \cppinline{sets:extensional} is, suitably read, the operationalisation of $\mathrm{coeq}(S \times_B S \rightrightarrows S)$ on a finite carrier.
...
The library exposes each shape's natural form (\texttt{Path<T>} for $\mathbb{N}$-indexed, \cppinline{Set\{element<A> | p\}} for predicates, the trivial helper in \texttt{:extensional} for finite carriers), and the compiler routes calls through the categorical primitive in \texttt{:category} that names the connection.\footnote{The categorical \texttt{image} primitive in \texttt{:category} pins the kernel-pair-coequalizer reading at the type level so CI verifies the connection.  The per-shape constructive realisations are not duplicates of one another --- they are honestly different algorithms for honestly different domain shapes --- but they share the same categorical name and the same external semantics.  The realisation boundary, again, is where the choice is made.}
