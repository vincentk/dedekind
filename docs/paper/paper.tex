...
The bottom row holds example carrier sets $\mathbb{B}, \mathbb{N}, \mathbb{Z}$, the objects of $\mathbf{Set}$, each carrying its set operations --- union $\cup$ and intersection $\cap$ as loops, with complement $\complement$ and the power object $\mathcal{P}$.
...
Table~\ref{tab:primitive-algebra} gives the headline; the four tables below are the exhaustive per-type breakdown, one per standard concept, split because the built-in primitives do not share a single operator surface.
...
\begin{table}[htbp]
\centering
\caption{\textbf{\cppinline{std::floating_point}.}  IEEE-754 rounding defeats associativity, and \cppinline{NaN} defeats even the order; almost nothing survives.}
