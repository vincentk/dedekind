...
\paragraph{Intensional sets, structurally.} The
\emph{intensional set} of this paper is the structuralist set in the
sense of the \emph{nLab}~\cite{nlab:structural_set_theory}: an object
whose membership is a characteristic morphism into a subobject
classifier, situated in a Cartesian-closed ambient. An object
satisfying $\textsc{IsSet}$ entails the CCC witness on its ambient
species, so any ETCS-set carrier comes with $1$, $\times$, and
$(-)^{(-)}$ for free; the resulting set-builder DSL reads as a syntax
for STLC by the Lambek--Scott
correspondence~\cite{lambek1988introduction}. The
C++$\leftrightarrow$STLC translation is catalogued in
Table~\ref{tab:cpp-stlc-mapping}.
...
The technique this paper exhibits is, in formal terms, the use of
\texttt{clang++} as an evaluator for a fragment of the simply-typed
lambda calculus (STLC) augmented with System-F-style
...
\begin{table}[htbp]
\centering
\caption{\textbf{C++23 surface $\leftrightarrow$ lambda calculus / category theory.}
The translation is restricted to the propositional fragment that the
C++ type checker can discharge mechanically. Template type-parameter
...
not claimed here. The ``Realisation'' column points to the section
where each row is exhibited.}
\label{tab:cpp-stlc-mapping}
\small
\begin{tabular}{@{}p{0.30\textwidth} p{0.32\textwidth} p{0.30\textwidth}@{}}
\toprule
\textbf{C++23 (\texttt{dedekind})} & \textbf{Lambda calculus / category theory} & \textbf{Realisation} \\
\midrule
\lstinline|template <typename T>| & Type variable $\alpha$; System-F universal quantification at the type level & §\ref{sec:axiomatic} \\
...
\lstinline{constexpr} evaluation that follows. The Curry--Howard
reading~\cite{wadler2015propositions} grounds this technically;
codified engineering ethics
ground it professionally --- the NSPE \emph{Code of Ethics for
Engineers}~\cite{nspe_ethics} canons III + IV (truthfulness in
public statements; faithful agency) treat a trait specialisation
as a public statement subject to the canons. The carrier-lattice's
recurring \emph{Honest Rejection} policy is the clearest example:
refusing \lstinline{unsigned int} as a witness for $\mathbb{N}$
is not pedantry but the kind of informal-claim re-examination
Canon III asks of a working software engineer. The trade is an
honesty obligation up front for a mechanical guarantee
afterward.\footnote{The argument is bounded. Most software claims
(TCP retransmit-timer correctness, scheduler fairness, end-to-end
latency) cannot be reduced to type-level structural assertions.
The paper covers the fragment where the reduction is available;
Section~\ref{sec:gap} names the bounds.}
...
\begin{tikzpicture}[
  ...
  % --- Lattice axes (oblique 2D projection of a 3D lattice) ---
  %   X axis (right):           carrier strength (𝔹 → ℕ → ℤ → ℚ → ℝ)
  %   Y axis (up):               object rank (scalar → V_2 → M_2)
  %   Z axis (diag, two depths): degree-2 quotient extensions
  %                              Dual(R) = R[ε]/(ε²)   — back-up-right (steeper)
  %                              Cplx(R) = R[i]/(i²+1) — back-right-up (flatter)
  \def\dx{4.0}
  \def\dy{2.6}
  % Dual depth axis: steeper slope.
  \def\zxoffD{2.0}
  \def\zyoffD{1.4}
  % Cplx depth axis: flatter slope, reaches further right (fans out from Dual).
  \def\zxoffC{5.0}
  \def\zyoffC{0.8}

  % --- Front face (z = 0): carrier-strength × rank grid (5 columns). ---
  % Scalars (y = 0).
  \node (B)  at (0*\dx, 0*\dy) {$\mathbb{B}$};
  \node (N)  at (1*\dx, 0*\dy) {$\mathbb{N}$};
  \node (Z)  at (2*\dx, 0*\dy) {$\mathbb{Z}$};
  \node (Q)  at (3*\dx, 0*\dy) {$\mathbb{Q}$};
  \node (R)  at (4*\dx, 0*\dy) {$\mathbb{R}$};
  % V_2 (y = 1).
  \node (V2B) at (0*\dx, 1*\dy) {$V_2(\mathbb{B})$};
  \node (V2N) at (1*\dx, 1*\dy) {$V_2(\mathbb{N})$};
  \node (V2Z) at (2*\dx, 1*\dy) {$V_2(\mathbb{Z})$};
  \node (V2Q) at (3*\dx, 1*\dy) {$V_2(\mathbb{Q})$};
  \node (V2R) at (4*\dx, 1*\dy) {$V_2(\mathbb{R})$};
  % M_2 (y = 2).
  \node (M2B) at (0*\dx, 2*\dy) {$M_2(\mathbb{B})$};
  \node (M2N) at (1*\dx, 2*\dy) {$M_2(\mathbb{N})$};
  \node (M2Z) at (2*\dx, 2*\dy) {$M_2(\mathbb{Z})$};
  \node (M2Q) at (3*\dx, 2*\dy) {$M_2(\mathbb{Q})$};
  \node (M2R) at (4*\dx, 2*\dy) {$M_2(\mathbb{R})$};

  % --- Back face #1 (Dual depth axis): D(ℚ), D(ℝ) over rank stack. ---
  \node[opacity=0.75] (DQ)   at (3*\dx + \zxoffD, 0*\dy + \zyoffD) {$D(\mathbb{Q})$};
  \node[opacity=0.75] (DR)   at (4*\dx + \zxoffD, 0*\dy + \zyoffD) {$D(\mathbb{R})$};
  \node[opacity=0.75] (V2DQ) at (3*\dx + \zxoffD, 1*\dy + \zyoffD) {$V_2(D(\mathbb{Q}))$};
  \node[opacity=0.75] (V2DR) at (4*\dx + \zxoffD, 1*\dy + \zyoffD) {$V_2(D(\mathbb{R}))$};
  \node[opacity=0.75] (M2DQ) at (3*\dx + \zxoffD, 2*\dy + \zyoffD) {$M_2(D(\mathbb{Q}))$};
  \node[opacity=0.75] (M2DR) at (4*\dx + \zxoffD, 2*\dy + \zyoffD) {$M_2(D(\mathbb{R}))$};

  % --- Back face #2 (Cplx depth axis): C(ℚ), C(ℝ) over rank stack. ---
  \node[opacity=0.75] (CQ)   at (3*\dx + \zxoffC, 0*\dy + \zyoffC) {$C(\mathbb{Q})$};
  \node[opacity=0.75] (CR)   at (4*\dx + \zxoffC, 0*\dy + \zyoffC) {$\mathbb{C} = C(\mathbb{R})$};
  \node[opacity=0.75] (V2CQ) at (3*\dx + \zxoffC, 1*\dy + \zyoffC) {$V_2(C(\mathbb{Q}))$};
  \node[opacity=0.75] (V2CR) at (4*\dx + \zxoffC, 1*\dy + \zyoffC) {$V_2(\mathbb{C})$};
  \node[opacity=0.75] (M2CQ) at (3*\dx + \zxoffC, 2*\dy + \zyoffC) {$M_2(C(\mathbb{Q}))$};
  \node[opacity=0.75] (M2CR) at (4*\dx + \zxoffC, 2*\dy + \zyoffC) {$M_2(\mathbb{C})$};

  % --- Back-far-corner face (Dual ∘ Cplx ≅ Cplx ∘ Dual): commuting square. ---
  % Position: zxoffD + zxoffC, zyoffD + zyoffC (sum of both depth offsets).
  \node[opacity=0.6] (DCQ)   at (3*\dx + \zxoffD + \zxoffC, 0*\dy + \zyoffD + \zyoffC) {$DC(\mathbb{Q})$};
  \node[opacity=0.6] (DCR)   at (4*\dx + \zxoffD + \zxoffC, 0*\dy + \zyoffD + \zyoffC) {$DC(\mathbb{R})$};
  \node[opacity=0.6] (V2DCQ) at (3*\dx + \zxoffD + \zxoffC, 1*\dy + \zyoffD + \zyoffC) {$V_2(DC(\mathbb{Q}))$};
  \node[opacity=0.6] (V2DCR) at (4*\dx + \zxoffD + \zxoffC, 1*\dy + \zyoffD + \zyoffC) {$V_2(DC(\mathbb{R}))$};
  \node[opacity=0.6] (M2DCQ) at (3*\dx + \zxoffD + \zxoffC, 2*\dy + \zyoffD + \zyoffC) {$M_2(DC(\mathbb{Q}))$};
  \node[opacity=0.6] (M2DCR) at (4*\dx + \zxoffD + \zxoffC, 2*\dy + \zyoffD + \zyoffC) {$M_2(DC(\mathbb{R}))$};

  % --- Arrow styles ---
  \tikzset{
    fwd/.style={->, >=stealth, semithick},
    fwdback/.style={->, >=stealth, semithick, opacity=0.55},
    diagD/.style={->, >=stealth, dashed, thick},
    diagC/.style={->, >=stealth, dash dot, thick},
  }

  % --- Front face arrows ---
  % Horizontal: carrier strength (Cplx step removed; ℂ now lives on Cplx back face).
  \draw[fwd] (B) -- (N);
  \draw[fwd] (N) -- (Z);
  \draw[fwd] (Z) -- (Q) node[midway, below=1pt, font=\scriptsize] {Frac};
  \draw[fwd] (Q) -- (R) node[midway, below=1pt, font=\scriptsize] {Cuts};
  \foreach \a/\b in {V2B/V2N, V2N/V2Z, V2Z/V2Q, V2Q/V2R} \draw[fwd] (\a) -- (\b);
  \foreach \a/\b in {M2B/M2N, M2N/M2Z, M2Z/M2Q, M2Q/M2R} \draw[fwd] (\a) -- (\b);
  % Vertical: rank (Free_2, End_R).
  \foreach \a/\b in {B/V2B, N/V2N, Z/V2Z, Q/V2Q, R/V2R} \draw[fwd] (\a) -- (\b);
  \draw[fwd] (Z) edge[draw=none] node[midway, right=1pt, font=\scriptsize] {Free$_2$} (V2Z);
  \foreach \a/\b in {V2B/M2B, V2N/M2N, V2Z/M2Z, V2Q/M2Q, V2R/M2R} \draw[fwd] (\a) -- (\b);
  \draw[fwd] (V2Z) edge[draw=none] node[midway, right=1pt, font=\scriptsize] {End$_R$} (M2Z);

  % --- Dual back-face arrows (horizontal + vertical) ---
  \foreach \a/\b in {DQ/DR, V2DQ/V2DR, M2DQ/M2DR}
    \draw[fwdback] (\a) -- (\b);
  \foreach \a/\b in {DQ/V2DQ, DR/V2DR, V2DQ/M2DQ, V2DR/M2DR}
    \draw[fwdback] (\a) -- (\b);

  % --- Cplx back-face arrows (horizontal + vertical) ---
  \foreach \a/\b in {CQ/CR, V2CQ/V2CR, M2CQ/M2CR}
    \draw[fwdback] (\a) -- (\b);
  \foreach \a/\b in {CQ/V2CQ, CR/V2CR, V2CQ/M2CQ, V2CR/M2CR}
    \draw[fwdback] (\a) -- (\b);

  % --- Far-corner face arrows (horizontal + vertical) ---
  \foreach \a/\b in {DCQ/DCR, V2DCQ/V2DCR, M2DCQ/M2DCR}
    \draw[fwdback] (\a) -- (\b);
  \foreach \a/\b in {DCQ/V2DCQ, DCR/V2DCR, V2DCQ/M2DCQ, V2DCR/M2DCR}
    \draw[fwdback] (\a) -- (\b);

  % --- Depth axis #1: Dual arrows (steeper, dashed) ---
  \draw[diagD] (Q) -- (DQ) node[midway, sloped, above, font=\scriptsize] {Dual};
  \draw[diagD] (R) -- (DR);
  \draw[diagD] (V2Q) -- (V2DQ);
  \draw[diagD] (V2R) -- (V2DR);
  \draw[diagD] (M2Q) -- (M2DQ);
  \draw[diagD] (M2R) -- (M2DR);
  % Same depth axis applied to the Cplx back face → far-corner.
  \draw[diagD] (CQ) -- (DCQ);
  \draw[diagD] (CR) -- (DCR);
  \draw[diagD] (V2CQ) -- (V2DCQ);
  \draw[diagD] (V2CR) -- (V2DCR);
  \draw[diagD] (M2CQ) -- (M2DCQ);
  \draw[diagD] (M2CR) -- (M2DCR);

  % --- Depth axis #2: Cplx arrows (flatter, dash-dot) ---
  \draw[diagC] (Q) -- (CQ) node[midway, sloped, below, font=\scriptsize] {Cplx};
  \draw[diagC] (R) -- (CR);
  \draw[diagC] (V2Q) -- (V2CQ);
  \draw[diagC] (V2R) -- (V2CR);
  \draw[diagC] (M2Q) -- (M2CQ);
  \draw[diagC] (M2R) -- (M2CR);
  % Same depth axis applied to the Dual back face → far-corner.
  \draw[diagC] (DQ) -- (DCQ);
  \draw[diagC] (DR) -- (DCR);
  \draw[diagC] (V2DQ) -- (V2DCQ);
  \draw[diagC] (V2DR) -- (V2DCR);
  \draw[diagC] (M2DQ) -- (M2DCQ);
  \draw[diagC] (M2DR) -- (M2DCR);

  % --- Machine substrate (Honest Rejection boundary, y = -1). ---
  \node (mbool)   at (0*\dx, -1*\dy) {\texttt{bool}};
  \node (muint)   at (1*\dx, -1*\dy) {\texttt{unsigned int}};
  \node (mint)    at (2*\dx, -1*\dy) {\texttt{int}};
  \node (mdouble) at (3*\dx, -1*\dy) {\texttt{double}};

  % Honest-Rejection boundary line.
  \draw[dotted, gray, thick]
    ($(mbool.west) + (-0.5, 0.55*\dy)$) --
    ($(mdouble.east) + (0.5, 0.55*\dy)$)
    node[midway, above=1pt, font=\scriptsize\itshape, gray]
      {Honest Rejection boundary};

  % Embedding arrows: machine → variant proxy.
  \tikzset{embed/.style={->, >=stealth, semithick, dotted}}
  \draw[embed] (mbool)   -- (B)
    node[midway, right=1pt, font=\scriptsize] {$\equiv$};
  \draw[embed] (muint)   -- (N)
    node[midway, right=1pt, font=\scriptsize] {\texttt{embed\_uint\_$\mathbb{N}$\_}};
  \draw[embed] (mint)    -- (Z)
    node[midway, right=1pt, font=\scriptsize] {\texttt{embed\_sint\_$\mathbb{Z}$\_}};
  \draw[embed] (mdouble) -- (Q)
    node[midway, right=1pt, font=\scriptsize] {\texttt{embed\_double\_$\mathbb{Q}$}};
\end{tikzpicture}
\caption{The Algebraic Tower (target-state Figure~1, first sketch).
Two-axis lattice: \emph{horizontal} = carrier-strength
($\mathbb{B} \to \mathbb{N} \to \mathbb{Z} \to \mathbb{Q} \to \mathbb{R}
\to \mathbb{C}$, each step a labelled functor application
\textsc{Frac} / \textsc{Cuts} / \textsc{Cplx}); \emph{vertical} =
object rank (Scalars $\to$ Vectors $V_2(R)$ via $\mathrm{Free}_2$
$\to$ Matrices $M_2(R)$ via $\mathrm{End}_R$). The finite-field side
branch ($\mathbb{F}_p$, $\mathbb{F}_{64}$) hangs off $\mathbb{Z}$
through the same $\mathrm{Quot}$ functor at prime / polynomial
ideals (cf.\ Figure~\ref{fig:initial-ring-modular} for the
modular-arithmetic exhibit). The Extension-Functor row (Dual /
Cplx as quotient rings) names the constructions used at each
step. The Machine Substrate row (\texttt{bool} / \texttt{uint} /
\texttt{int} / \texttt{double}) sits below the Honest-Rejection
boundary; the dashed arrows mark the realisation crossings that
embed each machine type into its variant proxy. \emph{Hooks for
the off-figure dimensions:} higher rank ($V_n$, $M_n$, $n > 2$;
\#368), predicate-set / order-theoretic constructions (a sibling
\emph{Geometric Tower}, candidate Figure~1b), tensor / exterior
powers, and the trait-registry pinning of every arrow as a
mechanical witness (\#499). The 2D projection is necessarily
lossy.}
...
instantiation; no substitution-failure cascade. The predicate sits
inside the class body as a literal \texttt{static\_assert} rather
than as a \lstinline{requires}-clause that has to be probed, so the
diagnostic fires at the named axiom rather than as a probe failure
deep in overload resolution. This is \emph{fail fast at the named
axiom}: the mathematical claim that broke surfaces before the
