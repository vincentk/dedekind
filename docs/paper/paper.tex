...
\begin{tikzpicture}[
  ...
  %   X axis (right):  carrier strength
  %   Y axis (up):     object rank
  %   Z axis (back):   quotient functor X (a parameter; specific values
  %                     enumerated in the caption)
  %
  % Eight corners only.  Intermediate lattice points (e.g.\ V_2(\mathbb{Z}),
  % D(\mathbb{Q}), \mathbb{F}_64) live conceptually along the edges and are
  % itemised in the per-axis tables below.
  ...
  % X-axis edges (4): "carrier strength" — generic; specific functor steps
  % (Frac, Cuts, Cplx) listed in the caption / Table~\ref{tab:carriers-exact}.
  \draw[edge]     (B)    -- node[midway, fill=white, inner sep=2pt, font=\large]
                              {$\cdots$} (C);
  \draw[edge]     (M2B)  -- node[midway, fill=white, inner sep=2pt, font=\large]
                              {$\cdots$} (M2C);
  \draw[edgeback] (XB)   -- node[midway, fill=white, inner sep=2pt, font=\large,
                                 opacity=0.7] {$\cdots$} (XC);
  \draw[edgeback] (M2XB) -- node[midway, fill=white, inner sep=2pt, font=\large,
                                 opacity=0.7] {$\cdots$} (M2XC);

  % Y-axis edges (4): "rank" — Free_2 / End_R; intermediate V_2(R) implicit.
  \draw[edge]     (B)    -- node[midway, fill=white, inner sep=2pt, font=\large]
                              {$\cdots$} (M2B);
  \draw[edge]     (C)    -- node[midway, fill=white, inner sep=2pt, font=\large]
                              {$\cdots$} (M2C);
  \draw[edgeback] (XB)   -- node[midway, fill=white, inner sep=2pt, font=\large,
                                 opacity=0.7] {$\cdots$} (M2XB);
  \draw[edgeback] (XC)   -- node[midway, fill=white, inner sep=2pt, font=\large,
                                 opacity=0.7] {$\cdots$} (M2XC);

  % Z-axis edges (4): "quotient X" — parameterised; intermediate
  % compositions implicit.
  \draw[diag] (B)   -- node[midway, sloped, above, font=\scriptsize] {$X$} (XB);
  \draw[diag] (C)   -- (XC);
  \draw[diag] (M2B) -- (M2XB);
  \draw[diag] (M2C) -- (M2XC);

  % --- Axis-direction labels (placed beside the relevant edge) ---
