...
\begin{tikzpicture}[every node/.style={font=\small},
                    axlbl/.style={font=\footnotesize\itshape}]
  \coordinate (z)  at (0,0);
  \coordinate (q)  at (2.7,0);
  \coordinate (r)  at (2.7,2.2);
  \coordinate (qp) at (4.7,0);
  \draw[->,thick] (z)--(q)  node[axlbl,pos=0.5,below]{localize};
  \draw[->,thick] (q)--(r)  node[axlbl,pos=0.5,right]{complete};
  \draw[->,thick,dashed,gray] (q)--(qp) node[axlbl,pos=0.5,below,black]{$p$-adic};
  \node[below left]  at (z)  {$\mathbb{Z}$};
  \node[below]       at (q)  {$\mathbb{Q}$};
  \node[above]       at (r)  {$\mathbb{R}$};
  \node[below right] at (qp) {$\mathbb{Q}_p$};
\end{tikzpicture}}
\\[0.4ex]{\footnotesize (b) localization / completion}
\end{minipage}
\caption{Two construction spaces.  \textbf{(a)} The HSP cube: the Birkhoff axes H (quotient), S (subalgebra), P (product) span the carriers, a lattice reading of Table~\ref{tab:hsp-rosetta} in the spirit of the lambda cube; the origin is a base magma ($\mathbb{B}$), composites sit at the corners up to $M_n(\mathbb{D})$ (labels are representative carriers, not a functor on a fixed base).  \textbf{(b)} The two \emph{non-}Birkhoff constructions at the cube's boundary (Table~\ref{tab:constructions}): \emph{localization} ($\mathbb{Z}\to\mathbb{Q}=\operatorname{Frac}\mathbb{Z}$, a colimit adjoining inverses) and \emph{completion} ($\mathbb{Q}\to\mathbb{R}$, a limit filling limit points, with the $p$-adic $\mathbb{Q}_p$ the other completion by Ostrowski).  Neither is finitary-equational: localization needs $\exists$ (inverses), completion needs infinite limits.}
\label{fig:hsp-cube}
\end{figure}
