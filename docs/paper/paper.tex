...
\begin{cpp}
// 𝔹: the universal set of bool,
//    every value of the type
constexpr UniversalSet<bool,
...
\begin{listing}[!ht]
\caption{An \emph{algebraic set} reified, assembled from smaller concepts.
\textbf{Left:} Reification of Eq.~\ref{eqn:ddk} \cppinline{concept IsAlgebraOnSet} corresponds to $\mathbf Ddk$, \cppinline{concept IsSet} to $\mathbf{Trsk}$ (Listing~\ref{lst:concept_isset})) and \cppinline{concept IsAlgebra} enforces operator closure as required by $\mathbf{Alg}(-)$.
\textbf{Right:} a sample structure built the same bottom-up way, the monoid: a magma (closure) refined by associativity and a unit.}
...
\begin{listing}[!ht]
\caption{Concrete carriers inhabiting the concepts of Listing~\ref{lst:algebra:universal}.  \textbf{Left:} the primitive \cppinline{bool}, at once a lattice under the logical operators $(\vee, \wedge)$ and the field $\mathbb{F}_2$ under the bitwise operators $(\oplus, \wedge)$; the Boolean \emph{set} $\mathbb{B} = $ \cppinline{Ω<bool>} it carries is introduced in Listing~\ref{lst:pruning}.  \textbf{Right:} the exact rationals $\mathbb{Q}$, a field under standard arithmetic.}
\label{lst:algebra:boolean_case}
\noindent
\begin{minipage}[t]{0.52\linewidth}
the primitive \cppinline{bool} as carrier
\begin{cpp}
// bool the carrier; the set 𝔹 = Ω<bool>
// is introduced in §3. Here its algebra:
...
\label{eqn:ddk}
    \mathbf{Ddk} \;=\; \mathbf{Trsk} \,\cap\, \mathbf{Alg}(-).
\end{equation}
Listing~\ref{lst:algebra:boolean_case} exhibits two inhabitants with real programmatic access: the primitive \cppinline{bool}, at once a lattice and the field $\mathbb{F}_2$, and the exact rationals $\mathbb{Q}$.
