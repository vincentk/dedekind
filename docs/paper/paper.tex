...
The \emph{member} column is the odd one out: membership $x \in S$ is not a lattice operation but the characteristic morphism $\chi : A \to \Omega$, the relation the whole lattice is built on ($\models$ satisfaction, $\in$ set membership, $\chi$ the classifier, \cppinline{s(x)} the set applied to a point).
...
\begin{listing}[htbp]
\caption{$\mathbf{Trsk}$ is strictly more expressive than \cppinline{std::set}, the extensional container it generalises.
\textbf{Left} ($\iota$, total): every container \emph{is} a $\mathbf{Trsk}$ set, its \cppinline{contains} the characteristic morphism, so \cppinline{ambient_set(from_std(·))} lifts any \cppinline{std::set} to an \cppinline{IsSet} object.
\textbf{Right} ($\mu$, partial): \cppinline{materialise} enumerates the universe and keeps the $\chi$-matches, defined only for a finite, enumerable universe with a decidable predicate.
The naturals do not materialise; the price of staying intensional is discharging membership by proof, not by enumeration.}
