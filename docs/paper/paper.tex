...
\textbf{Top}: \cppinline{maximum} / \cppinline{minimum} over an \cppinline{IsSet} domain \cppinline{S}, each the comprehension whose predicate is a bounded \cppinline{forall} over the order (\cppinline{minimum} is \cppinline{maximum} in the dual order).
\textbf{Bottom}: over the finite range $[2,6)$ each extremum is a singleton; \cppinline{sole} walks the lazy view to its unique element.}
...
\begin{cpp}
constexpr auto R = iota(2, 6);            // the finite range [2,6)
static_assert(sole(maximum(R)) == 5);     // greatest element, as {5}
static_assert(sole(minimum(R)) == 2);     // least element,    as {2}
\end{cpp}
\end{minipage}
\end{listing}
...
\begin{equation}
\label{eqn:argmax}
\arg\max_{f} \mathrm{dom} \;:\equiv\; \{x \in \mathrm{dom} \mid \forall x' \in \mathrm{dom}.\; f(x') \le f(x)\},
\qquad
...
\textbf{Top}: the forward filter, a \cppinline{forall} over $f(x')\le f(x)$; no inverse of $f$ enters.
\textbf{Bottom}: over $\{0,\dots,6\}$ the concave $x(6-x)$ has the unique peak $x=3$ (a singleton fibre, so $\arg\max$ is a function), while $x \bmod 2$ ties across every odd argument (fibre $\{1,3,5\}$, a proper relation); \cppinline{sole} / \cppinline{size} read the fibre's cardinality.}
\label{lst:argmax_def}
\noindent
\begin{minipage}[t]{0.99\linewidth}
\begin{cpp}
template <IsSet S, class F>
constexpr auto argmax(S dom, F f) {     // {x∈dom | ∀x'∈dom. f(x') ≤ f(x)}
  return set(dom, ForAll(dom, f(a) <= f(b)));
}
\end{cpp}
\end{minipage}
...
static_assert(sole(argmax(D, [](int x){ return x*(6-x); })) == 3);  // {3}
static_assert(size(argmax(D, [](int x){ return x % 2;   })) == 3);  // {1,3,5}
\end{cpp}
\end{minipage}
\end{listing}
