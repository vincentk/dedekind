...
\resizebox{\linewidth}{!}{%
\begin{tikzpicture}[every node/.style={font=\small}]
  % each machine type is an EXACT shadow of a modular Form
  \foreach \C/\Cube/\Type in {%
      1.5/{$\mathbb{B}$}/bool,
      5.5/{$\mathbb{Z}/2^{w}\mathbb{Z}$}/{size\_t},
      9.5/{$\mathbb{Q}\bmod L$}/double}{
    ...
    \node[font=\scriptsize\itshape,gray] at (\C+0.17,0.4) {exact};
  }
  % the infinite ideals, reached as period -> infinity
  \node[font=\footnotesize] at (5.67,3.7) {$\mathbb{Z}$};
  \draw[->,dashed] (5.67,2.75)--(5.67,3.45) node[midway,right,font=\scriptsize\itshape]{$w\to\infty$};
  \node[font=\footnotesize] at (9.67,3.7) {$\mathbb{R}$};
  \draw[->,dashed] (9.67,2.75)--(9.67,3.45) node[midway,right,font=\scriptsize\itshape]{$L\to\infty$};
  \node[font=\scriptsize\itshape] at (1.67,3.55) {already finite};
  % the tower of forms
  \draw[->,thick] (2.5,1.8)--(4.5,1.8) node[midway,above,font=\scriptsize\itshape]{$\subset$};
  \draw[->,thick] (6.6,1.8)--(8.5,1.8) node[midway,above,font=\scriptsize\itshape]{$\subset$};
  \node[font=\scriptsize\itshape] at (5.67,-1.25) {machine primitives $=$ exact shadows of modular Forms; the ideals are their period-$\infty$ limits};
\end{tikzpicture}}
\caption{The primitive carriers are \emph{exact} shadows of modular Forms.  Each machine type materialises a finite, periodic algebra exactly: \texttt{bool} is $\mathbb{B}$; \texttt{size\_t} is the modular ring $\mathbb{Z}/2^{w}\mathbb{Z}$; \texttt{double}, being a dyadic rational, realises a bounded modular-rational carrier ($\mathbb{Q}\bmod L$) as a genuine subalgebra.  The infinite ideals are not \emph{lost} but \emph{approached}: as the period grows ($w\to\infty$, $L\to\infty$) the modular Form becomes an ever more faithful shadow of the algebra being modelled --- $\mathbb{Z}/2^{w}\mathbb{Z}\to\mathbb{Z}$, and the refining modular-rational circle $\to\mathbb{R}$.  So the materialise boundary is not a wall but a limit: the machine gives \emph{exact} algebra at every finite stage, and the continuum is the colimit no finite stage reaches (Equation~\ref{eqn:model-materialise}).}
