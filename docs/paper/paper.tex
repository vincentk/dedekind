...
\textbf{Right:} \cppinline{std::size_t}, the modular ring $\mathbb{Z}/2^{64}\mathbb{Z}$ under $(+,\times)$; composite carriers such as $\mathbb{Q}$ appear in \S\ref{sec:hsp_varieties}.}
...
\begin{listing}[!ht]
\caption{The HSP combinators (left) and one application (right).  Registering \cppinline{Dual} as a quotient of \cppinline{F} forwards \cppinline{F}'s ring certificate to the dual numbers $\mathbb{D} = F[\varepsilon]/(\varepsilon^2)$, and the nilpotent axiom $\varepsilon^2 = 0$ is checked at compile time.  The H (quotient) family is shown; P (product) and S (subalgebra) mirror it.}
