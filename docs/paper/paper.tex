...
\section{Evaluation: Compile-Time Cost and Complexity}
...
Rather than extrapolate that trade from a single benchmark point, we characterise it in terms of the two parameters that actually govern it, the template instantiation depth and the number of constraints the reduction unrolls.
...
The second parameter is the number of constraints the quantifier unrolls.
For the chain-network dual of §\ref{sec:optimization_lp}, \cppinline{ForAll} expands once per edge, so the constraint count is linear in the edge count $E$.
Composing the two, the translation-time cost is $\mathcal{O}(E)$ instantiations at bounded depth: polynomial in the problem size precisely because the depth is capped.
...
When the constraints are declared statically, the reduction resolves the optimum to a typed constant, the telescoped coboundary sum of §\ref{sec:optimization_lp}, and the backend receives an already constant-folded kernel.
The runtime cost of the resulting binary is therefore $\mathcal{O}(1)$: a literal load, with no solver, no active-set search, and no dynamic indexing left in the emitted code, as the synthetic IR of Listing~\ref{lst:chain-dual-spec-ir} specifies.
...
The translation-time cost is $\mathcal{O}(E)$ and is incurred once; the runtime cost of the compiled kernel is $\mathcal{O}(1)$ and is paid on every invocation.
...
Measured cost curves against a reference implementation are the natural next step once the kernel of §\ref{sec:optimization_lp} is built.
The claim made here is the structural one: the cost is governed by instantiation depth and constraint count, not by any single observation.
