...
theory and methodology are not new: Lawvere's Elementary Theory of
the Category of Sets~\cite{lawvere1964etcs}, Wadler's
parametricity~\cite{wadler1989theorems} and propositions-as-types
reading~\cite{wadler2015propositions}, and structural concept-based
typing have been studied for decades, and proof-assistant-grounded
formalisation (Coq, Agda, Lean) has been the canonical academic
deliverable. What is new is the \emph{cost-benefit calculus} of
encoding mathematical structures directly in an industrial typed
substrate. Modern transformer-based coding assistants substantially
compress the time and expertise required to author opt-in trait
specialisations, axiom-shape concepts, and the surrounding library
scaffolding, turning what was previously an academic specialism
into an actionable workflow for working software engineers. This
paper argues, by exhibit rather than by theorem, that the
cost-benefit turn matters in practice today; the meta-claim is not
ours to originate---adjacent positions have been argued
elsewhere---and we make it here primarily so the body's executable
exhibit can be read in that light.

\medskip
\noindent We present \texttt{dedekind}~\cite{dedekind2026}, an
open-source C++23 library (Apache-2.0), as the executable exhibit of
that workflow. The library embeds formal set-theoretic and
categorical structures directly into the type system and targets
\emph{mainstream C++23}---concepts, non-type template parameters,
named modules---rather than a research compiler or a separate proof
environment. The central design rule is \emph{intensional first,
realize it when you mean it}: a set is represented as a predicate
$P : A \to \Omega$ over an ambient species $A$, not as a
heap-allocated container of values. This comprehension view has two
practical consequences that the paper explores:
\begin{itemize}
...
determined uniquely by ring-axiom preservation.
Figure~\ref{fig:initial-ring-modular} exhibits this concretely
at three prime targets $R = \texttt{Modular}\langle n\rangle$
($n \in \{2, 5, 7\}$).  The second is the
...
pinned at a concrete file:line in source so a reader can verify the
mechanism rather than take the claim on faith.
\emph{Integer-promotion shape mismatch}: \lstinline|bool * bool|
under \lstinline|+| / \lstinline|*| returns \lstinline|int| rather
than \lstinline|bool|; \lstinline|unsigned short * unsigned short|
and \lstinline|unsigned char * unsigned char| likewise promote to
...
not registered, and \lstinline|!IsTotallyOrdered<double>| / \dots /
\lstinline|<long double>| / \dots / \lstinline|!category::IsRing<double, ...>|
hold (\texttt{numbers/floating\_point.cppm} +
...
gate. Downstream combinators bind to \lstinline|IsFieldLikeScalar|
