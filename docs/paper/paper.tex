...
In $\mathbf{Trsk}$:
\begin{enumerate}
\item A \emph{set} in $\mathbf{Trsk}$ is a regularized carrier type \cppinline{A} in $\mathbf{Jlt}$ satisfying \cppinline{concept IsSetObject} combined with a \textbf{characteristic predicate} that defines set membership.
\item A \emph{relation} is a subset of the \textbf{Cartesian Product} of the \cppinline{::Domain} and \cppinline{::Codomain}.
\item A \emph{set} is thus a relation between the carrier type and \cppinline{bool}.
\end{enumerate}
The definition of \cppinline{concept IsSetObject} follows the definition of $\mathbf{Set}$ in the ETCS sense (Definition~\ref{def:etcs-set}) and so one can write:
\begin{equation}
\label{eqn:trsk}
