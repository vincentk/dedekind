...
constexpr Singleton<true> T    = set{𝔹 | isTrue()};
constexpr Singleton<false> F = ~T;
static_assert((F | T) == 𝔹);
constexpr Ø<𝔹> empty = F & T;
\end{cpp}
\end{minipage}\hfill
\begin{minipage}[t]{0.49\linewidth}
...
constexpr Halfspace<5, P> gt_5 = set(ℕ | gt(5))
constexpr Halfspace<5, L> le_5 = ~gt_5;
static_assert((le_5 | gt_5 ) == ℕ);
constexpr Ø<ℕ> empty = le_5 & gt_5;
\end{cpp}
\end{minipage}
\end{listing}
