...
%%%
\clearpage
\section{The System F $\leftrightarrow$ Juliet Posture $\leftrightarrow$ Bourbaki Bridge}
...
%%%
\clearpage
\subsection{The Traits Registry: Abstract Naturals vs.\ IEEE Numerics}
\label{sec:numeric-tension}

The library records these divergences one algebraic law at a time in the \texttt{:species} trait registry.
Take associativity of addition as the exhibit.
The ring $\mathbb{Z}$ has it; a 32-bit signed \texttt{int} does not, because overflow is undefined behaviour; and IEEE~754 \texttt{double}~\cite{ieee754_2019} does not either, because rounding happens at every step.
The registry states each case directly:

% Source: src/main/modules/dedekind/category/species.cppm (false-by-default primary l. 91; std::plus specialisations ll. 565-612)
\begin{listing}[htbp]
\caption{The species registry records algebraic properties and their absence. Signed \texttt{int} is explicitly rejected as associative under \texttt{+} (signed overflow is undefined behaviour); IEEE \texttt{double} is rejected by the false-by-default primary, with no per-carrier line.}
\label{lst:species}
\begin{cpp}
...
A generic algorithm requiring associativity therefore fails to compile on \texttt{int} or \texttt{double} unless the programmer supplies an explicit numeric policy: modular semantics, compensated summation, an interval wrapper, or a lift to \cppinline{Rational<long>}.
