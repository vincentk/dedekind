...
In this paper, we do not want to re-invent the wheel. 
Instead we want to re-discover the both mathematical structures as well as basic concepts and types found in the C++ language.
To do this, we use a structural type theory, which allows us to identify a mathematic `concept` in the standard library and make it accessible to programmers and the compiler alike.

While traditional object-oriented systems typically rely on \textit{nominal subtyping}, where a type $T$ is recognized as belonging to a category $C$ only through explicit inheritance declarations (e.g., \texttt{class T : public Monoid}). In this "Capulet/Montague" paradigm, a type is rejected if it lacks the correct "family name," regardless of its actual properties. 
...
Product ($A \times B$) & \texttt{std::pair<A, B>} & \texttt{IsProduct} & Universal Projection \\
