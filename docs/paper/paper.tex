...
Supplying that knowledge once, and letting the compiler enforce it thereafter, is what turns these familiar rules into build-time checks and optimizations; Appendix~\ref{sec:trait-registry} records, per carrier, which laws the type system can and cannot certify.
...
%%%
\clearpage
\section{Triangulating System F, Structuralism and $\mathbf{Jlt}$}
...
%%%
\clearpage
\section{Certification of Carrier Semantics: The Traits Registry}
\label{sec:trait-registry}

Where Appendix~\ref{app:jlt-systemf} grounds the structural surface, this appendix records what the type system can certify about the algebra of each carrier, with a special focus on the built-in primitive types.
Specifically, it explores the tension between the definitions of the IEEE standard which are largely informed by the bounded physical substrate of computation on the one hand and the abstract definitions of algebra on the other hand.
...
Notable examples are exhibited in this appendix.
...
\emph{Strict} is the categorical proof: the carrier satisfies the axiom bundle itself (\cppinline{IsRing}, \cppinline{IsField}), not merely its operator shape.
When all three agree under \cppinline{(std::plus, std::multiplies)}, the bundled seal \cppinline{IsArithmeticRing} fires and generic code may be written in plain syntax over the carrier.
The distinction earns its keep because the tiers can disagree: a carrier can pass the literal tier (the operators compile) yet fail the strict tier (the axioms do not hold), which is exactly the IEEE and signed-overflow situation this appendix catalogues.

\subsection{The Algebra of the Built-in Types}
\label{sec:builtin-algebra}

Table~\ref{tab:primitive-algebra} gives the headline; the four tables below break the principal built-in types down by operator surface, one table per standard-concept family (\cppinline{bool} on its own), since the built-in primitives share no single operator surface.
The library carriers built on these primitives, graded by the tiers of §\ref{par:concept-tiers}, are catalogued in §\ref{sec:carrier-rosetta}.
...
%%%
\clearpage
\subsection{An Example Algebraic Certification of IEEE Numerics}
...
\cppinline{double} fails the honest rejection default policy.}
\label{lst:species}
\begin{cpp}
...
This section records, per carrier (Figure~\ref{fig:hub-spoke-bnz}), which algebraic laws the type system certifies, and at which tier of §\ref{par:concept-tiers}.
...
needed, the relevant species trait is specialised explicitly: e.g.\ \cppinline{is_associative_v<MyType, std::plus<>> = true;}
for a user-defined type whose addition is provably associative.
...
Table~\ref{tab:carriers-exact} records the library's own exact carriers, one per mathematical role, with the algebraic concept each certifies.
Table~\ref{tab:carriers-primitive} records the built-in primitives and the concept each satisfies.
Table~\ref{tab:carriers-crossings} is the boundary between them: the named realise and embed arrows.
...
satisfied, not approximated.  Here \cppinline{I} abbreviates \cppinline{SignedExtensionalCardinal<>}, the default exact integer.}
\label{tab:carriers-exact}
\footnotesize
\begin{tabular}{@{}l p{0.33\textwidth} p{0.37\textwidth}@{}}
\toprule
...
$\mathbb{Q}$ & \cppinline{Rational<I>} & \cppinline{HasFieldOperators<Rational<I>>}\textsuperscript{$\dagger$} \\
$\mathbb{R}$ exact & \cppinline{ExactReal<I>} = \cppinline{Real<Rational<I>>} & \cppinline{IsDedekindComplete} (exact rational scalar) \\
$\mathbb{C}$ & \cppinline{Complex<Rational<I>>} & \cppinline{Algebra_}$\mathbb{C}$ \\
$\mathbb{D}$ (dual) & \cppinline{Dual<Rational<I>>} & \cppinline{IsRing<T,+,*>}, $\varepsilon^2 = 0$ \\
\bottomrule
\end{tabular}
...
\begin{table}[htbp]
\centering
\caption{\textbf{Built-in primitives to concepts.} The algebraic concept each \cppinline{std}-library primitive satisfies under its C++-standard arithmetic, graded by the tiers of §\ref{par:concept-tiers}.}
\label{tab:carriers-primitive}
\footnotesize
\begin{tabular}{@{}p{0.27\textwidth} p{0.65\textwidth}@{}}
\toprule
\textbf{Primitive type} & \textbf{Concept satisfied} \\
\midrule
\cppinline{bool} & \cppinline{IsField<bool, std::bit_xor, std::bit_and>} ($\mathbb{F}_2$); \cppinline{HasLatticeOperators<bool>} \\
\cppinline{unsigned long}, \cppinline{size_t} & \cppinline{IsCyclicGroup}, \cppinline{IsAbelianGroup<T,+>} (modular, wraps at $2^N$) \\
\cppinline{int}, \cppinline{long} & \cppinline{HasRingOperators}\textsuperscript{a} (strict \cppinline{IsRing} refused: signed-overflow UB) \\
\cppinline{int8_t}, \cppinline{int16_t} & \cppinline{IsAlgebra<T, std::plus<T>, std::multiplies<T>>}\textsuperscript{b} (functor-surface only; promotes) \\
\cppinline{float}, \cppinline{double} & \cppinline{Continuum_}$\mathbb{R}$ (non-assoc.\ \cppinline{+}) \\
\cppinline{std::complex<double>} & \cppinline{Algebra_}$\mathbb{C}$ (non-assoc.) \\
\bottomrule
\end{tabular}

\vspace{0.3em}
\begin{flushleft}
{\footnotesize \textsuperscript{a} The operator surface \cppinline{HasRingOperators} holds ($+, -, *$ close on the carrier), but the strict \cppinline{IsRing} bundle refuses: signed-overflow UB makes $+$ partial. Total alternatives: \cppinline{SignedCardinality} (saturating) or \cppinline{SignedExtensionalCardinal<>} (cyclic).
\textsuperscript{b} Integer promotion (C++ [conv.prom]) lifts \cppinline{int8_t + int8_t} to \cppinline{int}, so only the functor-parametric surface \cppinline{std::plus<T>} (which returns \cppinline{T} by signature) closes on the carrier, not literal arithmetic.}
\end{flushleft}
\end{table}
...
$\mathbb{Q}$ & through $\mathbb{R}$ proxy (\cppinline{Real<Rational<Z>>}) & \cppinline{embed_double_ℚ<I>(v)} \\
...
$\mathbb{C}$ & per-component through $\mathbb{R}$ proxy & \cppinline{Complex<R>(re, im)} \\
$\mathbb{D}$ (dual) & \cppinline{.val} (primal only; tangent discarded) & \cppinline{Dual<F>(v, d)} \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:rosetta-textbook-vs-cpp} catalogues, per carrier, where textbook ring structure and the literal C++ operators diverge, grading each by the tiers of §\ref{par:concept-tiers} and the \cppinline{IsArithmeticRing} seal.
The carriers are those used in the worked examples (Issue~\#393); the friction list is summarised structurally in Section~\ref{sec:compiler-wall}.

\begin{table}[htbp]
\centering
\caption{\textbf{Math textbook $\leftrightarrow$ C++ standard} (library carriers; the built-in primitives are itemised per operator surface in Appendix~\ref{sec:builtin-algebra}). Where the
...
generic code can be written in plain syntax. Where they diverge, the table names the precise
...
\textsuperscript{e} & vectors have no internal product \cppinline{Vec2V} $\times$ \cppinline{Vec2V} $\to$ \cppinline{Vec2V}, only scalar multiplication; the shape concept correctly refuses. \\
\end{tabular}
}
\end{table}

At a call site, pick the \emph{seal} for plain C++ syntax over a textbook-aligned carrier, \emph{strict + functor} when the ring structure rides non-standard functors (\cppinline{bool} as $\mathbb{F}_2$), and \emph{literal} for the operator surface without the axioms (elementwise-lifted matrix bodies).

The discipline is \textbf{start correct, finish fast}: write the reference implementation against an exact carrier (Table~\ref{tab:carriers-exact}) and cross to a machine primitive (Table~\ref{tab:carriers-crossings}) only in profiled inner loops, accepting the caveat the concept cell records; the type system enforces that the laws your code relies on are the laws the carrier provides, or it fails to compile.
