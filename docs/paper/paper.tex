...
\end{tikzcd}
\caption{
A sketch of the desired functionality to be supported by the \texttt{dedekind} set DSL, drawn to separate the two categories it straddles.
The bottom row holds the carrier sets $\mathbb{B}, \mathbb{N}, \mathbb{Z}$, the objects of $\mathbf{Set}$; above them sit algebraic sets, the objects of $\mathbf{Alg}(\Sigma)$, staggered across two tiers so one carrier can support several without the arrows crossing.
The carriers are not inert: as objects of the set DSL they carry the set operations, union $\cup$ and intersection $\cap$ drawn as loops (the same lattice as $\mathbb{B}$'s $\wedge, \vee$, now on subsets), with complement $\complement$ and the power object $\mathcal{P}$ completing the Boolean algebra of sets.
Each node above the carriers is that carrier decorated with its operations, the family $F$ of the algebra, drawn as loops: $\mathbb{B}$ as the Boolean lattice $(\wedge, \vee)$ and the Galois field $\mathbb{F}_2$ $(\oplus, \wedge)$, and $\mathbb{N}, \mathbb{Z}$ each with their usual arithmetic $(+, \times)$ on the lower tier and a tropical semiring on the upper, its sum $\oplus = \max$ and product $\otimes = +$, the classical-versus-tropical pairing §\ref{sec:optimization_lp} exploits as a semiring swap.
A loop is an operation, so a binary one such as $\wedge$ or $+$ is a map $A \times A \to A$ decorating $A$, not a category endomorphism.
The dashed forgetful functor $U : \mathbf{Alg}(\Sigma) \to \mathbf{Set}$ strips those operations; it is not injective on objects, as the two algebras over each carrier show.
The two hooked arrows are the inclusion $\mathbb{N} \hookrightarrow \mathbb{Z}$, a homomorphism in each semiring: it preserves $(+, \times)$ classically and $(\oplus, \otimes)$ tropically.
Dotted arrows are typed maps that are \emph{not} homomorphisms, so they live downstairs in $\mathbf{Set}$: the embedding $\mathfrak{i}$ with the parity classifier back, and $x \mapsto -x$ (preserves $+$, not $\times$) with $x \mapsto x^2$ (preserves $\times$, not $+$).
}
