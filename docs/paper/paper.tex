...
which moves you back. Rows or cells marked with $^{\dagger}$ point at
concepts introduced under the concept-cleanup follow-up work; rows or
cells marked with $^{\ddagger}$ point at carriers whose
ℵ$_0$-escalating variant is planned but not yet shipped.
...
$\mathbb{N}$ unbounded & \lstinline|Cardinality|$^{\ddagger}$ (variant over ℵ$_0$) & \lstinline|Monoid_|$\mathbb{N}$ \\
$\mathbb{Z}$ bounded & \lstinline|SignedExtensionalCardinal<N>| & \lstinline|IsAbelianGroup<T,+>|, \lstinline|IsRing| \\
$\mathbb{Z}$ unbounded & \lstinline|SignedCardinality|$^{\ddagger}$ & \lstinline|Group_|$\mathbb{Z}$ \\
...
\lstinline|int|, \lstinline|long| & Partial (UB on signed overflow) & \lstinline|IsPartialAbelianGroup|$^{\dagger}$ \\
\lstinline|int8_t|, \lstinline|int16_t| & Partial + SIMD-vectorisable & \lstinline|IsPartialCyclicGroup|$^{\dagger}$ \\
...
failure point and the tier that still fires. ``$\circ$'' marks
operational-only assertion under the active numeric policy; ``--''
...
\textsuperscript{b} & signed-overflow UB defeats the modular-wrap axiom; the categorical \lstinline|IsRing| chain breaks at the periodicity step. \\
...
arbitrary-precision signed rational whose exact range easily
accommodates every arithmetic operation the showcases exercise.
Cramer's rule over $\mathbb{Q}$ is exact, and on this carrier no
determinant --- however fat --- ever overflows silently; escalation
to $\aleph_0$ is tracked in Section~\ref{sec:carrier-rosetta}'s
Rosetta table.

A programmer who trades the exact carrier for a native one
(\lstinline{Rational<long>}, or \lstinline{double}, per
Table~\ref{tab:carriers-crossings}) inherits the corresponding
failure mode: signed-integer overflow is undefined behaviour on the
former, rounding is non-associative on the latter. That trade is
deliberate and the Rosetta table spells out the named arrow for
making it. The crucial property the showcases demonstrate ---
compile-time reduction to a typed constant --- does not depend on
carrier choice; it survives every row of the table. Every combinator
in Section~\ref{sec:pruning} is written against the
\lstinline{IsRingLike} concept, not against any particular carrier,
so the swap between exact and native is a one-line change at the
\lstinline{using} declaration.
