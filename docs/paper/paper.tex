...
Listing~\ref{lst:algebra:universal} illustrates the reification of \emph{algebraic sets} as sketched in Figure~\ref{fig:hub-spoke-bnz}.
By reinterpreting logical conjunction of predicates as set intersection, we can define a family $\mathbf{Ddk}$ of algebraic sets expressible in \Cpp{23} as the intersection of the previously introduced reified intensional sets $\mathbf{Trsk}$~\eqref{eqn:trsk} with $\mathbf{Alg}(-)$:
\begin{equation}
\label{eqn:ddk}
    \mathbf{Ddk} \;=\; \mathbf{Trsk} \,\cap\, \mathbf{Alg}(-),
\end{equation}

$\mathbf{Alg}(-)$ is universal algebra in the textbook sense~\cite{burris1981universalalgebra}: sets equipped with operations of fixed arity.\footnote{The dash is left unbound, the same $F_\Sigma$-algebra fragment a functional programmer would recognise~\cite{pierce1991basic}.  The smallness ceiling that keeps $\mathbf{Ddk}$ clear of proper-class self-reference is the ETCS power-object bound (Appendix~\ref{sec:etcs}, Axiom~9, $\beth_1$).}

The smallest inhabitant makes the shape concrete.
The Boolean carrier $\mathbb{B}$ is an algebraic set the moment it type-checks as $\mathbf{Alg}(\wedge, \vee, \neg)$ over \cppinline{bool} (Listing~\ref{lst:algebra:boolean_case}), with $\wedge$ and $\vee$ (both binary $\mathbb{B} \times \mathbb{B} \to \mathbb{B}$) drawn as loops on its Boolean-lattice node in Figure~\ref{fig:hub-spoke-bnz}.
