...
\paragraph{What falls out for free.}  Naming the structure in the
type buys mileage at no runtime cost.  An \emph{intensional set}
here is the structuralist set in the sense of the
\emph{nLab}~\cite{nlab:structural_set_theory} --- a predicate into
a subobject classifier, situated in a Cartesian-closed
ambient~\cite{lambek1988introduction} --- and once the
$\textsc{IsSet}$ concept is wired in, the same machinery that
discharges set membership also lifts arrows over functorial
families ($T \mapsto T^2 \mapsto M_2(T)$ etc.), pins ring monomorphisms
($T \hookrightarrow M_2(T)$ as a monadic unit), and extracts optima
from polytopes (the LP reduction of
Section~\ref{sec:lp-centrepiece} is a co-Kleisli arrow).\footnote{%
...
algebraic laws --- the categorical vocabulary is not decorative.
The C++$\leftrightarrow$STLC translation that names this machinery
is in Table~\ref{tab:cpp-stlc-mapping}.}
