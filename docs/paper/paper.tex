...
\begin{tabular}{@{}llll@{}}
\toprule
arrow $f$ & one-sided inverse & fibre $f^{-1}(y)$ & composition $h = g \circ f$ \\
\midrule
iso              & $f^{-1}$ (two-sided)                     & $\{f^{-1}(y)\}$                                    & $h^{-1} = f^{-1}\!\circ g^{-1}$ \\
\addlinespace
mono / injective & retract $r$, $\;r\circ f = \mathrm{id}$   & $\{r(y)\}$ if $f(r(y)){=}y$, else $\varnothing$   & $r_h = r_f\!\circ r_g$ \\
\addlinespace
epi / surjective & section $s$, $\;f\circ s = \mathrm{id}$   & a union, containing $s(y)$                         & $s_h = s_f\!\circ s_g$ \\
\addlinespace
