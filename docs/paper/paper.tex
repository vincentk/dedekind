...
The resulting capabilities are demonstrated by the compile-time formulation of the dual of a shortest-path problem over a directed chain network in Section~\ref{sec:optimization_lp}.
...
\paragraph{Edge-cost sensitivity by a ring swap.}
The same reduction carries its own first-order sensitivity for free.
Re-run the dual over $\mathbb{Q}[\varepsilon]/(\varepsilon^2)$ in place of $\mathbb{Q}$, so each edge cost $c_{ij}$ carries a tangent beside its value.
The optimum $d_N - d_0$ then returns paired with its derivative $\partial(d_N - d_0)/\partial c_{ij}$ as one typed constant, with no separate derivative pass.
This is forward-mode automatic differentiation in Elliott's sense~\cite{elliott2018simple}, the chain-rule-carrying product $(f, f')$, with the chain rule falling out of the dual-number ring laws.
The \cppinline{Dual<F>} carrier drops into any NTTP-parameterised carrier without special-casing; the chain-network exhibit is deferred to a follow-up.
...
Pinning the object-size at $\beth_1$ admits the transfinite carriers required for Dedekind-cut arithmetic ($\mathbb{R}$, §\ref{sec:numeric-tension}) and the continuum-sized $\mathbb{C}$, $\mathbb{D}$ used in §\ref{sec:operational_calculus}, while keeping a foundational ceiling that forbids the self-referential paradoxes of the proper-class meta-categories $\mathbf{Set}$ and $\mathbf{Cat}$.
...
drift (see Section~\ref{sec:pruning}). A general inductive proof over
...
\begin{itemize}
  ...
  \item \textbf{Linear programming} (demonstrated in
    Section~\ref{sec:optimization_lp}).
    The dual of a shortest-path problem over a chain network reduces, at
    compile time, to a typed constant; the same reduction over a
...
\cppinline{constexpr} evaluation (§\ref{sec:ddk_comprehension}, IR fixtures) & Normalisation ($\beta$-reduction) --- reduction of a term to its value or literal form \\
\cppinline{std::same_as} (§\ref{sec:ddk_etcs}) & Definitional type equality $\tau = \sigma$ \\
\midrule
\texttt{dedekind} & \\
\midrule
