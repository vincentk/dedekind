...
\section{Topological Operators as Coordinate-Free Matrix Substitutes}
\label{sec:topological_operators}

To compress the library's scope and eliminate the compilation overhead of 2D storage layouts, `dedekind` replaces physical coordinate matrices with structural operators defined directly over graph topologies.
Following this convention, vectors are modeled as functions over element domains, while linear transformations are realized as stateless functional operators between these spaces.
For a finite, oriented network graph $G = (V, E)$, the space of vertex signals $\mathbb{R}^V$ maps scalar properties to nodes, while the space of edge signals $\mathbb{R}^E$ maps flows to directed links.
This topological representation is strictly isomorphic to vanilla sparse linear algebra, bridging abstract complexes with coordinate vectors and traditional matrices via an exact dual dictionary.

\subsection{The Isomorphism Dictionary}
\label{sec:isomorphism_dictionary}

Let the graph vertices be indexed as $V = \{v_1, v_2, \dots, v_n\}$ and directed edges as $E = \{e_1, e_2, \dots, e_m\}$.
An edge $e_k$ directed from source node $v_i$ to target node $v_j$ establishes the coordinate topology of the system.
The exact structural isomorphisms linking these topological operators to standard matrices and vectors are defined as follows:

\begin{equation}
\begin{array}{rccc}
\text{\textbf{Vector Spaces:}} & \mathbb{R}^V \cong \mathbb{R}^n & \text{and} & \mathbb{R}^E \cong \mathbb{R}^m \\
\text{\textbf{Boundary Operator:}} & \partial \colon \mathbb{R}^E \to \mathbb{R}^V & \cong & A \in \mathbb{R}^{n \times m} \\
\text{\textbf{Coboundary Operator:}} & \delta \colon \mathbb{R}^V \to \mathbb{R}^E & \cong & A^T \in \mathbb{R}^{m \times n}
\end{array}
\end{equation}

Here, $A$ represents the classical node-edge incidence matrix.
The coordinate-wise coefficients of this matrix are mapped directly from the graph's structural orientation rules:

\begin{equation}
A_{ik} = \begin{cases} 
-1, & \text{if } v_i = \text{source}(e_k), \\
+1, & \text{if } v_i = \text{target}(e_k), \\
0, & \text{otherwise}.
\end{cases}
\end{equation}

Under this mapping, the boundary operator $\partial$ transforms an edge flow vector $f \in \mathbb{R}^E$ into a vertex divergence vector $d \in \mathbb{R}^V$.
This transformation unrolls pointwise over the graph elements, recreating standard matrix-vector multiplication without grid lookups:
\begin{equation}
d(v_i) = (\partial f)(v_i) = \sum_{e: \text{target}(e)=v_i} f(e) - \sum_{e: \text{source}(e)=v_i} f(e) \quad \Longleftrightarrow \quad d = A f.
\end{equation}

Conversely, the coboundary operator $\delta$ maps a vertex potential vector $\phi \in \mathbb{R}^V$ to an edge gradient vector $g \in \mathbb{R}^E$.
This operation evaluates the localized potential difference across each link, serving as the exact coordinate-free equivalent of the transposed incidence matrix:
\begin{equation}
g(e_k) = (\delta \phi)(e_k) = \phi(\text{target}(e_k)) - \phi(\text{source}(e_k)) \quad \Longleftrightarrow \quad g = A^T \phi.
\end{equation}

The fundamental adjoint relationship linking these transformations asserts that $\langle \partial f, \phi \rangle = \langle f, \delta \phi \rangle$.
This identity maps directly onto the conservation of energy principle, validating Tellegen's theorem by mirroring the standard matrix transposition transpose identity:
\begin{equation}
(Af)^T \phi = f^T (A^T \phi).
\end{equation}

\subsection{C++23 Architectural Reification and Target Models}
\label{sec:operator_reification}

\begin{listing}[htbp]
\caption{A code extract from the project's test suite demonstrating how topological operators, inner products, and outer products are invoked as point-free expressions within the set comprehension eDSL.}
\label{lst:operator_usage}
\begin{cpp}
// Setup topological spaces (V and E are ExtensionalSets)
constexpr auto G = ChainTopology{Vertices{1u, 2u, 3u}, Edges{e1, e2}};

// Define signal fields over Vertices (potentials) and Edges (flows)
constexpr auto phi = [](Vertex v) { return (v == 1u) ? bound<10> : bound<0>; };
constexpr auto flow = [](Edge e)   { return (e == e1)  ? bound<2>  : bound<0>; };

// 1. Coboundary Mapping: Evaluate edge gradients from vertex potentials
constexpr auto grad = Coboundary(G)(phi);
static_assert(grad(e1) == bound<-10>); // phi(2) - phi(1) = 0 - 10

// 2. Inner Product: Evaluate total system network power <grad, flow>
constexpr auto power = InnerProduct(G.E)(grad, flow);
static_assert(power == bound<-20>); // (-10 * 2) + (0 * 0)

// 3. Outer Product Set: { (v, e) in V x E | (phi \otimes flow)(v, e) > 15 }
constexpr auto S_tensor = Set{in(G.V, G.E) | 
    OuterProduct(phi, flow)(in(G.V), in(G.E)) > bound<15>
};

// Only vertex 1 (phi=10) combined with edge e1 (flow=2) yields a product > 15
static_assert(S_tensor(1u, e1) && !S_tensor(2u, e1));
\end{cpp}
\end{listing}

To ensure that the compilation toolchain can enforce these structural contracts before code generation, the properties are codified using C++23 constraints.
Listing~\ref{lst:topological_operators_implementation} contains the ready-to-compile target specifications.
It introduces the core concept gates and demonstrates how inner products, outer products, and coboundary mappings are reified as zero-overhead, point-free expressions inside the set comprehension engine.

By routing calculations through the \cppinline{evaluate_elements} fold expansion, inner products over extensional sets resolve to immediate compile-time scalars.
Similarly, \cppinline{OuterProduct} and \cppinline{Coboundary} return lazy expression templates that compute values pointwise when queried by downstream logic.
