...
\begin{tikzpicture}[
    ...
    \node[partition, right=of total] (actions) {:actions};
...
The \texttt{:actions} partition materializes the concept of a Species being acted upon by an Algebraic Structure. This partition defines the \texttt{IsSemiModule} and \texttt{IsModule} concepts, where a \textbf{Monoid} (from the \texttt{:total} algebraic layer) defines a transformation rule over a target Species. 
