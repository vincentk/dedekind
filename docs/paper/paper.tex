...
\begin{listing}[htbp]
\caption{A translation acts on \emph{sets} through its total inverse: it shifts a bounded halfspace and fixes the unbounded line.  \cppinline{translation<ℤ,3>} ($x\mapsto x+3$) is a point-free iso, so its \cppinline{retract} is the total inverse $x-3$, a named combinator whose inverse falls out of the type.  \cppinline{image} of the bounded $\{x\le5\}$ is again a halfspace, the pivot advanced to $\{x\le8\}$, structurally the same type and not an opaque predicate, while the unbounded $\mathbb{Z}$ comes back unchanged (abridged from the checked-in witnesses).}
\label{lst:functional_image_bijection}
\begin{cpp}
constexpr auto f = translation<ℤ, 3>;             // x ↦ x+3, a point-free iso
static_assert(retract(f)(10) == 7);                // total inverse x−3
constexpr auto s = image(f, ℤ | π <= fix(5_c));   // {x≤5} ↦ {x≤8}, still a halfspace
static_assert(s(8) && !s(9));                       // the bound moved to 8
static_assert(image(f, ℤ) == ℤ);                   // the unbounded line is fixed
\end{cpp}
\end{listing}

The translation is the iso case: entire, functional, surjective and injective at once, so its retract is a total inverse, and because it is surjective its image of a set is decided by that inverse alone, $b \in \mathrm{image}(f,S) \iff f^{-1}(b) \in S$.
A bounded halfspace stays a halfspace with its bound advanced, and the unbounded line is fixed.
