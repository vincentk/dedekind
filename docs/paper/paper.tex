...
\addlinespace
$\mathbf{CCC}$ & $\chi$ & $\times$ & $\sqcup$ & $\neg$ & $\mathbf{0}$ & $\mathbf{1}$ & $B^A$ \\
\addlinespace
$\mathbf{Trsk}$ & \cppinline{s(x)} & \cppinline{&} & \cppinline{|} & \cppinline{~} & \cppinline{Ø} & \cppinline{Ω} & \cppinline{<=} \\
\bottomrule
\end{tabular}
\caption{
Operator-mapping table: each column is one construction, spelled in four languages over the rows $\mathbb{B}$, $\mathcal{P}(A)$, $\mathbf{CCC}$, $\mathbf{Trsk}$ (Section~\ref{sec:ddk_etcs}, Eqn~\ref{eqn:trsk}).
...
\item A \emph{set} is thus a relation between its carrier and \cppinline{bool}, the graph of $\chi_S$.
\end{enumerate}
...
Each column of that table is one universal construction of a Cartesian-closed category in four languages: the meet \emph{is} the categorical product, the implication the exponential.
