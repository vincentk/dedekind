...
What makes it more than a name is that it carries its domain in its own type, so the compiler knows what the variable ranges over, and it carries a representative value (its \emph{pivot}) that the template engine substitutes to evaluate and prune the predicate at compile time.
Informally, the scout is sent into the domain to probe membership.
Formally it is the characteristic-morphism handle that the comprehension pipe (\cppinline{operator|}) binds to a predicate (\cppinline{BoundScout} in the code, gated by \cppinline{IsArrow} since a set is its own characteristic morphism).
