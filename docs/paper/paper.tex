...
\end{tikzpicture}
\caption{\textbf{The trait registry's two axes: syntactic surface, semantic law.}  A carrier is placed by the operators it supports (horizontal, which the compiler settles by overload resolution) and by how many algebraic laws it obeys (vertical, from closure up to inverses).  Below the dashed line the standard library answers mechanically; above it, each law is the engineer's honesty obligation.  \texttt{int} and \texttt{double} carry the operators but stall low, their associativity lost to overflow and to IEEE rounding, while \texttt{Rational<int>} reaches a full field.}
\label{fig:two-axes}
\end{figure}
