...
As we appear to have too many choices as opposed to too few, we can ask: are the features that we want to select from this host language to maximize semantic overlap with the fragment $\mathbf{Alg}(\Sigma)$~\ref{fig:hub-spoke-bnz} that we have just identified?
...
% TODO(future-edit): the two-axis framing introduced in the next paragraph
% is the load-bearing setup for the trait-registry implementation discussion
% in §\ref{sec:pruning} (Algebraic Lattice).  Keep the touch light here ---
% name the split, forward-pointer, and stop.  The actual registry mechanics
% (specialisation tables, propagation rules, audit rosettas) belong in §5,
% not in §3.  If §3 prose grows past one paragraph on this point, push the
% overflow into §\ref{sec:pruning}.

The listing exhibits a load-bearing split that runs through the rest of the paper, and which the C++ standard already recognises in its own vocabulary~\cite{cpp23standard} ([concepts.lib]): a type \emph{satisfies} a concept when its syntactic requirements compile, and \emph{models} it when it additionally meets the concept's semantic requirements.\footnote{The C++ standard's \emph{syntactic} / \emph{semantic} split is fundamentally \textbf{decidable vs.\ undecidable} --- what the spec commits to enforcing mechanically vs.\ what it deliberately refuses to enforce --- and does not coincide with the static-vs-dynamic semantics split familiar from PL theory~\cite{pierce2002tapl}.  C++'s \emph{syntactic} bucket subsumes that tradition's \emph{static semantics} (type checking, concept resolution, auto-derived theorems via \cppinline{static_assert}) because those are all things the compiler verifies mechanically; the C++ \emph{semantic} bucket is roughly what resists mechanical decision in the spec's chosen target structure.  The committee's vocabulary optimises for responsibility allocation (compiler vs.\ user) rather than for type-theoretic precision; the mismatch is real but learnable.}
The operator surface witnessed by \cppinline{HasRingOperators} states \emph{closure} --- which types satisfy the syntactic half --- and this axis can be checked \emph{by inspection of the standard library}: the compiler resolves the overloads, the concept either binds or it does not, and no further human judgement is required.
The algebraic \emph{laws} (associativity, commutativity, distributivity, identities, inverses, totality) are the corresponding semantic half and must be \emph{validated and certified by hand}: a type that satisfies but does not model triggers the standard's \emph{ill-formed, no diagnostic required} (IFNDR) clause, under which the implementation may produce arbitrary code without warning --- the C++ analogue of \emph{ex falso quodlibet}.
Signed-overflow UB defeats ring-associativity on \cppinline{int}, IEEE-754 rounding defeats field-associativity on \cppinline{double}, matrix multiplication is non-commutative even when the scalar field is, and the standard library --- principled in its silence --- refuses to commit to a position on any of these.
The Juliet Posture's job is to take that silence as a job description: lift each meta-symmetry to a compile-time predicate (\cppinline{IsAssociative<T, Op>}, \cppinline{IsCommutative<T, Op>}, \dots), populate it per carrier in a \emph{trait registry}, and let downstream concepts (\cppinline{IsRing}, \cppinline{IsField}) compose closure-on-the-surface with law-from-the-registry into a single \cppinline{static_assert} obligation --- the manual-certification path C++ Core Guideline T.20~\cite{cppcoreguidelines} prescribes (``Avoid `concepts' without meaningful semantics''), and the path the committee left open when the C++0x \texttt{axiom} keyword was considered for in-language semantic certification and ultimately withdrawn~\cite{isocpp_concepts_history}.
...
The framing pays off in a row-by-row mapping: each Juliet-Posture restriction recovers a corresponding System-F property, with the canonical references for both calculi found in Pierce~\cite{pierce2002tapl} (Chapters 23 and 29).
