...
\begin{cpp}
constexpr auto R =
  (ℕ | (π <= fix(5_c))) * ℕ   // {x ≤ 5} × ℕ = Ω<pair> | π1 ≤ fix(5)
  | π1 + fix(1_c) == π2;
static_assert( R(pair{4,5}));
static_assert(!R(pair{7,8}));  // 7 ≰ 5: bound not dropped
\end{cpp}
\end{listing}
...
\begin{listing}[htbp]
\caption{An iso makes image membership a lookup through the inverse.  The point-free translation \cppinline{translation<int,3>} ($x \mapsto x+3$) is a two-sided iso, so its \cppinline{retract} is the \emph{total} inverse $x-3$, a named combinator whose inverse falls out of the type with nothing to prove.  Then $b \in \mathrm{image}(f, S)$ is just $f^{-1}(b) \in S$, a lookup on the image boundary with no search of the domain and no partiality (here \cppinline{positive} is the halfspace $\{x>0\}$; $8 \mapsto 5 \in$ \cppinline{positive}, $2 \mapsto -1 \notin$; abridged from the checked-in test).}
\label{lst:functional_image_bijection}
\begin{cpp}
constexpr auto f = translation<int, 3>;   // x ↦ x+3, a point-free iso
static_assert(retract(f)(10) == 7);        // retract = total inverse x−3
constexpr auto img = image(f, positive);   // {b | b−3 ∈ positive}
\end{cpp}
\end{listing}

The translation is the iso case: entire, functional, surjective and injective at once, so its retract is a total inverse and image membership always decides.
