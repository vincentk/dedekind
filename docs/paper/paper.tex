...
\midrule
\cppinline{template <typename T>} (§\ref{sec:algebraic_lattice_carriers}) & Type variable $\alpha$; System-F universal quantification at the type level (parametric \emph{in the disciplined subset}; no specialisation, no SFINAE branch in the public surface) \\
\cppinline{concept} (§\ref{sec:algebraic_lattice_carriers}) & Type-class / kind constraint --- a predicate on types restricting the domain of a polymorphic function \\
\cppinline{requires}-clause (§\ref{sec:algebraic_lattice_carriers}) & Evidence / constraint satisfaction --- a proof obligation that the type carries the term-level operations \\
...
\cppinline{using} alias (§\ref{sec:algebraic_lattice_carriers}) & Type-level redex --- the compiler reduces by $\beta$-style substitution \\
