...
Here we want the engineering counterpart: the Juliet Posture $\mathbf{Jlt}$, a structural subcategory sitting at the intersection $\mathbf{CCC} \cap \mathbf{Cpp}$.
The idea is to find a fragment of C++ that, suitably disciplined, behaves as a Cartesian Closed Category --- exactly the structure our algebraic hubs need to inhabit.
...
\begin{enumerate}
    ...
    By itself \cppinline{constexpr} doesn't get us pure and terminating arrows --- captures can be mutable, and template instantiation is Turing-complete in the general case.
    ...
    \item \textbf{Products.}
    \cppinline{std::pair} with structural binding hands us the universal property of products: for any $A, B$ we have a product $A \times B$ with projections $\pi_1, \pi_2$, and the mediating-arrow construction is the obvious tuple-build.
...
The intersection $\mathbf{CCC} \cap \mathbf{Cpp}$ is inhabited, and the \textbf{Juliet Posture} is the deliberate choice to live in the inhabitant.
Table~\ref{tab:cpp23-ccc-mapping} recaps.
\end{proof}
