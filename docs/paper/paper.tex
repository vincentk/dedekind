...
One direction of that boundary is in fact already exercised, unbidden.
Exact-carrier arithmetic admits constant evaluation but is step-heavy, so a deep enough exact computation exhausts the translator's constant-evaluation budget and spills to run time of its own accord (Appendix~\ref{app:partial-eval}); because the carrier stays exact across that spill the crossing is lossless, the run-time value being the very constant the compiler would have folded.
The controlled boundary is therefore only half a conjecture: the exact-to-exact crossing already happens and already costs nothing; what remains to design is the deliberate, lossy exact-to-floating-point map, and it is the exactness of the ring that buys the distinction.
...
The boundary can also place itself.
An exact carrier admits constant evaluation but is step-heavy, so a deep enough computation exhausts the translator's constant-evaluation budget and spills to run time of its own accord.
Because the carrier is exact that spill is lossless: the run-time value is the very constant the compiler would have folded, computed a moment later rather than during translation, so what is lost is the folding and never the answer.
