...
\toprule
lattice operation & meet & join & complement & bottom & top & order \\
\midrule
Boolean lattice $\mathbb{B}$ & $\wedge$ & $\vee$ & $\neg$ & $\bot$ & $\top$ & $\Rightarrow$ \\
set lattice $\mathcal{P}(A)$ & $\cap$ & $\cup$ & $\complement$ & $\varnothing$ & $A$ & $\subseteq$ \\
$\mathbf{Trsk}$ (C\texttt{++}) & \cppinline{&} & \cppinline{|} & \cppinline{~} & \cppinline{Ø} & \cppinline{Ω} & \cppinline{<=} \\
\bottomrule
\end{tabular}
\caption{
The set lattice \emph{is} the Boolean lattice, lifted pointwise, and the collapse in Listing~\ref{lst:pruning} is that lift at work.
...
Each set operation compiles to the corresponding Boolean operation on predicates, no element materialized; the third row is the $\mathbf{Trsk}$ operator surface that realises it (the order $\subseteq$ is \cppinline{operator<=}, pointwise implication $\chi_S \Rightarrow \chi_T$).
