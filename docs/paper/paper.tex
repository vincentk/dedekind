...
\textbf{Right}: composition \cppinline{>>} is symbolic (the shifts add, no $\exists$ over the intermediate), so closure, \emph{contravariant inversion} $(f\circ g)^{-1}=g^{-1}\circ f^{-1}$, and \emph{associativity, commutativity} are compile-time facts; the conjugate $g\,f\,g^{-1}=f$ collapses because $+$ commutes.  For an invertible map the inverse \emph{is} the retract, so this is retract composition in the total case (abridged from the checked-in witnesses).}
