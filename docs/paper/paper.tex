...
\toprule
 & add & mult & $\mathbf{0}$ & $\mathbf{1}$ & converse & residual \\
\midrule
dioid $(S, \oplus, \otimes)$ & $\oplus$ & $\otimes$ & $\bar{0}$ & $\bar{1}$ & $(\cdot)^{\ast}$ & $/$ \\
\addlinespace
relation $R \subseteq A\times B$ & $\cup$ & $\mathbin{;}$ & $\varnothing$ & $\Delta$ & $R^{\circ}$ & $R/S$ \\
\addlinespace
operator / $\mathrm{Mat}(S)$ & $\oplus$ & $\otimes$ & $0$ & $I$ & $A^{\dagger}$ & $A^{+}$ \\
\addlinespace
$\mathbf{Trsk}$ & \cppinline{+} & \cppinline{*} & \cppinline{Ø} & \cppinline{diagonal} & \cppinline{converse} & \cppinline{/} \\
\bottomrule
\end{tabular}
\caption{Operator-mapping for \cppinline{IsRelation} as a dioid, the counterpart to the lattice map of Table~\ref{tab:lattice-lift}: each column is one dioid construction, spelled in four vocabularies (the abstract semiring, Tarski's relations, the matrix / operator semiring $\mathrm{Mat}(S)$, and \cppinline{Trsk}).
Multiplication $\otimes$ is composition ($\mathbin{;}$, the matrix product), unit the diagonal $\Delta = I$; addition $\oplus$ is union (elementwise join), unit the empty relation.
Two constructions sit \emph{off} the table because they are not dioid operations: the object-level \emph{type former} \cppinline{A >> B} (the ambient $A\times B$, how a relation space is built, one level above the algebra), and the \emph{withheld} inverses (no \cppinline{-}, no reciprocal), whose very absence is the signature of a semiring (the diagnostic of Table~\ref{tab:primitive-algebra}).
The distinct alphabet is deliberate, chosen for legibility over reuse, exactly as the set lattice took the \emph{bitwise} operators rather than logic's \cppinline{&&} / \cppinline{||}.
\emph{Caveat:} the \cppinline{Trsk} row is the design target; assigning composition to \cppinline{*} (freeing \cppinline{>>} for the type former) is a category-wide operator pass still underway, since \cppinline{IsRelation} implies \cppinline{IsArrow} and composition must therefore migrate uniformly across every arrow.}
\label{tab:relation-operators}
\end{table}
...
\toprule
concept & structure & \cppinline{+} & \cppinline{-} & \cppinline{*} & \cppinline{/} \\
\midrule
\cppinline{IsRelation}    & dioid  & $\cup$ & --- & $\mathbin{;}$ & --- \\
\cppinline{IsFunction}    & monoid & --- & --- & $\mathbin{;}$ & --- \\
\cppinline{IsIsomorphism} & group  & --- & --- & $\mathbin{;}$ & $R^{-1}\!=\!R^{\circ}$ \\
\midrule
$\mathrm{End}$ of a line  & field  & $+$ & $-$ & $\times$ & $\div$ \\
\bottomrule
\end{tabular}
\caption{The specialization ladder beneath \cppinline{IsRelation}, and the inverse each concept restores.
Restricting the morphism class sharpens the algebra: a \emph{function} is single-valued, so union drops and only composition survives (a monoid); an \emph{isomorphism} is invertible, so the \emph{multiplicative} inverse returns as the converse ($R^{\circ}=R^{-1}$, Table~\ref{tab:inverse-image-laws}), a group.
The \emph{additive} inverse returns only at the scalar level: the endomorphisms of a line form a \emph{field} (Schur), the full arithmetic $+\;-\;\times\;\div$, and the isomorphisms are exactly its group of units ($F^{\ast}$, or $\mathrm{GL}$ in several dimensions).
So the inverses the dioid withholds (Table~\ref{tab:relation-operators}) are restored one specialization at a time, and the field is where they all return.}
\label{tab:relation-ladder}
\end{table}
