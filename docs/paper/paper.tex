...
The concrete carriers\footnote{We use ``carrier'' loosely throughout to mean the C++ type $\mathrm{T}$ that realises both the underlying set of a $\Sigma$-algebra and the operator-shape concept fired against it.  In strict universal algebra (Burris--Sankappanavar~\cite{burris1981universalalgebra}), \emph{carrier} names only the underlying set $A$, not the algebra $(A, \Sigma_A)$; our usage matches the engineering idiom in which ``the carrier type'' indexes both --- a sentence like ``the carrier $\mathbb{Z}$ is closed under additive inverse'' is a claim about $(\mathbb{Z}, +, -)$ as a $\Sigma$-algebra, not about the bare set.  Section~\ref{sec:alg-sigma} makes the layering precise.} a programming language offers violate those laws conditionally, sometimes silently:
