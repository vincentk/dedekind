...
Section~\ref{sec:alg-sigma} carved out a well-behaved structure on the mathematics side: $\mathbf{Alg}(-)_{\beth_1}$, here we want the engineering counterpart.
...
The findings presented below as a proof sketch for clarity of exposition.
...
i) \textbf{nominal} (declared lineage (often inheritance); checked at compile time but blind to behaviour),
ii) \textbf{duck} (structural by call-site method probe; checked at runtime, too late for the optimiser),
...
\item~developer-supplied claims and axioms via \emph{trait specialisations} consult to discharge a single \cppinline{static_assert} obligation.
\end{enumerate}
...
The motivation for the third tier as follows:
