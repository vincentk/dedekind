...
The \texttt{dedekind} types and combinators are the inhabitants of $\mathbf{Jlt} \cap \mathbf{Spo}$\footnote{$\mathbf{Spo}$ is a project-internal coinage for the single-species fragment of $\mathbf{Cat}$ developed in §\ref{sec:spo} (Burris--Sankappanavar~\cite{burris1981universalalgebra} §II.10).  The textbook anchor is the bijection between monoids and one-object categories (Mac Lane~\cite{maclane1971categories} §I.1); single-species categories are the engineering-library generalisation that admits operations of arbitrary signature (e.g.\ both $+$ and $\cdot$ on a ring carrier), not only the single binary operation of a monoid.  We deliberately avoid the textbook short-name $\mathbf{Mon}$ to side-step the name collision with the standard \emph{category of monoids} in Mac Lane §I.5.}: C++23 types and arrows that simultaneously sit in the disciplined syntactic fragment $\mathbf{Jlt}$ and carry single-species categorical structure from $\mathbf{Spo}$.
