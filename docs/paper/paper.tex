...
First-order quantification is restricted to enumerable domains (\cppinline{IsExtensional}): over an infinite domain it would demand an unbounded search and is undecidable (Rice's theorem), whereas a bounded domain makes that search finite.
On such a domain the quantifier \emph{is} a set operation, and we take that structural (Bourbaki) reading as the definition rather than as a later coincidence:
\begin{equation}
\exists x \in S.\,P(x) \;:\equiv\; \{x\in S \mid P(x)\} \neq \varnothing,
\qquad
\forall x \in S.\,P(x) \;:\equiv\; \neg\,\exists x \in S.\,\neg P(x).
\end{equation}
In code the enumerable comprehension is \cppinline{set(S,P)} and the emptiness test an overload of \cppinline{Ø::operator==}, so \cppinline{exists(S,P)} is \cppinline{!(Ø{} == set(S,P))} and \cppinline{forall} is \cppinline{!exists(S,!P)}.
There is no fold: the reduction lives in \cppinline{Ø::operator==}, so the Rice boundary is a \emph{type check}, an operand with no overload being a compile error rather than a fabricated answer.

The combinators \cppinline{Exists} and \cppinline{ForAll} build on this definition: they bind the inner variable over the domain and leave a predicate in the outer variable, exactly the shape a comprehension turns into a new set (Listing~\ref{lst:quantifiers}).
