...
First-order quantification is restricted to enumerable domains (\cppinline{IsExtensional}), since unconstrained evaluation over an infinite intensional space is undecidable at compile time (Rice's theorem).
...
\end{equation}
In code the enumerable comprehension is \cppinline{set(S,P)}, the emptiness test is an overload of \cppinline{Ø::operator==}, and $\forall$ is the double negation of $\exists$: \cppinline{exists(S,P)} is \cppinline{!(Ø{} == set(S,P))} and \cppinline{forall} is \cppinline{!exists(S,!P)}.
There is no separate $\bigvee$/$\bigwedge$ fold.
The whole reduction lives in how \cppinline{Ø::operator==} decides emptiness, so the Rice boundary is a \emph{type check}: an operand with no suitable overload is a compile error, not a fabricated answer, and the wall never spills into prose.
...
The two decidable regimes are exactly the two \cppinline{Ø::operator==} overloads, selected structurally by the shape of the comprehension, and they fall at two architectural layers.
A transparent halfspace meet collapses to $\varnothing$ at the type level through the \cppinline{dedekind.order} specialization of \cppinline{structured_and}, so $\forall x>5.\,x\ge 3$ is a \cppinline{static_assert} on the empty counterexample set $\{x>5 \wedge x<3\}$.
That collapse is an order-layer capability, reached through the \cppinline{\&} meet combinator rather than \cppinline{set}, since it needs the halfspace types themselves.
Over an enumerable domain \cppinline{set(S,P)} is instead a lazy \cppinline{std::views::filter}, and the same \cppinline{==} decides emptiness by \cppinline{begin == end}, short-circuiting at the first witness (Listing~\ref{lst:quantifier_regimes}).

\begin{listing}[htbp]
\caption{One definition, two regimes selected by \cppinline{Ø::operator==}.  Compile time: the order-specialized \cppinline{\&} meet collapses two disjoint halfspaces to $\varnothing$ by type.  Run time: \cppinline{set} filters an enumerable domain and emptiness is \cppinline{begin == end}.  A non-enumerable, opaque operand matches no overload: the honest Rice wall.}
