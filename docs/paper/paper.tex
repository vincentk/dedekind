...
\textbf{Top}: \cppinline{max} is \cppinline{S} met with its \cppinline{upperbounds} (the $\forall$-projection $R/\ni$, $\{\pi_2\in S \mid \forall\pi_1\in S.\; \pi_1\le\pi_2\}$, Equation~\ref{eqn:extrema}); \cppinline{min} is the dual.
...
\begin{cpp}
// One generic definition — S met with its ∀-dominators (Bird & de Moor):
//   max S = structured_and(S, upperbounds(S)) = S ∩ (R/∋) = {π2∈S | ∀π1∈S. π1 ≤ π2}
auto max(S s) { return structured_and(s, upperbounds(s)); }  // min: S ∩ lowerbounds(S)

// upperbounds = the ∀-projection R/∋, decided by structure (the pluggable point):
auto upperbounds(Halfspace<T,p,Down,_>) { return Halfspace<T,p,Up,NonStrict>{}; } // {x≥p}
auto upperbounds(Halfspace<T,p,Up,_>)   { return Ø<T>{}; }        // unbounded → no bound
auto upperbounds(Ω<bool>)               { return Singleton<true>{}; }   // 𝔹: the top ⊤
\end{cpp}
\end{minipage}
...
\begin{minipage}[t]{0.99\linewidth}
\begin{cpp}
static_assert(max(𝔹)(true));   static_assert(min(𝔹)(false)); // 𝔹 ∩ {⊤}/{⊥} = {true}/{false}
static_assert(max(ℤ | (π <= fix(5_c)))(5));   // {x≤5} ∩ {x≥5} = {5}: the attained sup
static_assert(min(ℤ | (π >= fix(5_c)))(5));   // {x≥5} ∩ {x≤5} = {5}
\end{cpp}
\end{minipage}
\end{listing}
...
\textbf{Right}: composition \cppinline{>>} is symbolic --- the shifts add, no $\exists$ over the intermediate --- so closure, the inverse law, and \emph{associativity, invertibility, commutativity} are compile-time facts; the conjugate $g\,f\,g^{-1}=f$ collapses because $+$ commutes (abridged from the checked-in witnesses).}
...
static_assert((f >> g)              // closure: T₃∘T₂=T₅
  == (ℤ*ℤ | π1+fix(5_c)==π2));
static_assert((g >> inverse(g))     // g∘g⁻¹ = id
  == (ℤ*ℤ | π1+fix(0_c)==π2));
...
\begin{cpp}
struct Fold { using Domain = int; using Codomain = int;
  constexpr int operator()(int x) const { return 2*(x<0?-x:x); } };
constexpr auto retract(Fold) {          // mono leg: canonical +y/2
  return [](int y) -> std::optional<int> {
    return (y>=0 && y%2==0) ? std::optional{y/2} : std::nullopt; }; }
constexpr auto cofibre(Fold) {          // epi leg: the class {+x, -x}
  return [](int x) { return std::array{x, -x}; }; }

constexpr auto img = image(Fold{}, negative);   // img(6) == true, via -3
\end{cpp}
\end{listing}
