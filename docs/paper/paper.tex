...
\begin{tikzpicture}[every node/.style={font=\small},
  ...
  \node[above right] at (ps) {$M_n(\mathbb{B})=\mathbf{Rel}$};
\end{tikzpicture}}
\\[0.3ex]{\footnotesize (b) HSP on \texttt{bool}: P-rich, S/H trivial --- $\mathbb{B}^{n}$, $M_n(\mathbb{B})=\mathbf{Rel}$ (relations)}
\end{minipage}\hfill
...
\begin{tikzpicture}[every node/.style={font=\small},
  ...
  \node[above right] at (psh) {$M_n(\cdot)$};
\end{tikzpicture}}
\\[0.3ex]{\footnotesize (c) HSP on \texttt{size\_t}: full H/S/P; H reduces $\mathbb{Z}/2^{k}\mathbb{Z}\to\mathbb{B}$}
\end{minipage}
\caption{The primitive carriers, two readings stacked.  \textbf{(a)} The \emph{shadow plot} (vertical axis): each machine type is an \emph{exact} shadow of a finite modular Form.  \texttt{bool} is $\mathbb{B}$; \texttt{double}, a dyadic rational, is a bounded $\mathbb{Q}\bmod L$.  \texttt{size\_t} and \texttt{int} share the bit-width and two's-complement \emph{representation} but are different \emph{algebras}: \texttt{size\_t} wraps into the \emph{total} ring $\mathbb{Z}/2^{w}\mathbb{Z}$ (a Birkhoff H-quotient of $\mathbb{Z}$, $\to\mathbb{N}$), whereas \texttt{int} has \emph{no} total ring --- signed overflow is undefined --- so it is a \emph{partial} $\mathbb{Z}$, exact only in range ($\to\mathbb{Z}$, the ${}^{\dagger}$).  The ideals are \emph{approached}, not lost: as the period grows ($w,L\to\infty$) the Form becomes an ever more faithful shadow, and the continuum is the colimit no finite stage reaches (Equation~\ref{eqn:model-materialise}).  The horizontal tower is \emph{not} one construction (H from \texttt{size\_t} down to \texttt{bool}, the partial widening to \texttt{int}, extension to \texttt{double}); that lateral structure lives in the cubes, not in a single arrow.  \textbf{(b,c)} Two Forms opened as HSP-varieties (Figure~\ref{fig:hub-spoke-bnz}'s constructions, re-based).  On \texttt{bool} the variety is \emph{P-rich} though S and H are trivial ($\mathbb{B}$ is simple): P gives $\mathbb{B}^{n}$ (all finite Boolean algebras, by Stone) \emph{and} $M_n(\mathbb{B})=\mathbf{Rel}$ --- Boolean matrices, i.e.\ relations (\texttt{std::vector<std::vector<bool>>}).  On \texttt{size\_t} all three legs are live --- P gives vectors and matrices $M_n(\cdot)$, S the ideals $2^{k}\mathbb{Z}/2^{w}\mathbb{Z}$, H the coarser rings $\mathbb{Z}/2^{k}\mathbb{Z}$; at $k=1$ that is $\mathbb{B}$, so the \texttt{bool} cube \emph{is} the H-corner of the \texttt{size\_t} cube.  HSP furnishes the cubes; the vertical limits above are the non-Birkhoff axis it does not span.}
...
The primitive machine carriers sit at the base of the tower as \emph{exact shadows} of finite modular Forms (Figure~\ref{fig:primitive-reconciliation}): \texttt{bool} is $\mathbb{B}$, \texttt{size\_t} and \texttt{int} the modular ring $\mathbb{Z}/2^{w}\mathbb{Z}$, and \texttt{double} a bounded modular-rational carrier, the infinite ideals ($\mathbb{N}$, $\mathbb{Z}$, $\mathbb{R}$) reached only in the period-$\infty$ limit.
