...
\begin{enumerate}
\item A basic model of \textbf{Categories} in $\mathbf{Jlt}$ can circumscribe a collection of objects as a type constraint \cppinline{std::regular A},  \cppinline{std::regular B}, \ldots and a collection of morphisms as \cppinline{std::invocable<A, B>}.
Basic composition rules \footnote{The category is automatically small because physical constraints on the runtime environment will result in a finite runtime size.}. 
