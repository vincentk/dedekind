...
\mathbb{N}$ explicit at the type level.\footnote{The symmetric three-element
chain $\mathbb{K3} \hookrightarrow \texttt{std::signed\_integral} \to
\mathbb{Z}$ is the Kleene-truth-value version, with \texttt{embed\_K3\_}$\mathbb{Z}$
as the first leg (False $\mapsto -1$, Unknown $\mapsto 0$, True $\mapsto 1$);
the symmetric story is tracked under issue~\#418.} The full discussion lives
