...
\bottomrule
\end{tabular}
\caption{\textbf{What the built-in primitives actually certify.}  Columns are the operators a type offers (in the \Cpp{} sense); rows are a cumulative ladder of algebraic laws, each rung assuming the ones below.  Closure the compiler settles by itself ($\vdash$); every rung above is semantic, so it is either certified by hand in the trait registry (\checkmark) or absent, in which case the type is rejected at the gate ($\times$).  The bit-exact primitives \texttt{bool} and \texttt{unsigned} climb the whole ladder; \texttt{int} and \texttt{double} stall at closure, their associativity already lost to overflow and to IEEE rounding.  Under the \emph{logical} operators \texttt{bool} is only a Boolean lattice, without inverses; the field $\mathbb{F}_2$ is the bitwise reading.}
\label{fig:two-axes}
\end{figure}
...
The discrepancy between the aspirational semantic model sketched in Figure~\ref{fig:hub-spoke-bnz} and the reality of IEEE numerical primitives as exposed in \Cpp{23} is highlighted in Figure~\ref{fig:two-axes}.
