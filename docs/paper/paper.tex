...
\paragraph{Scope of the claim.}  We make no pretence that a complete
and faithful taxonomy of mathematics is achievable; the structure
sketched in Figure~\ref{fig:carrier-lattice} is a bounded-below
lattice with no canonical top --- iterated quotients, tensor
products, $p$-adic completions and similar each yield a strictly
larger object without a unique maximum, so any rendering is
necessarily a finite slice for clarity of exposition.  What we
\emph{do} report is a small set of findings that are both interesting
...
The Algebraic Lattice is rendered as a 2D oblique projection of a 3D
cube; eight corners, twelve edges, three Fraktur-letter axis
parameters ($\mathfrak{A}$ carrier-strength, $\mathfrak{R}$ rank,
$\mathfrak{Q}$ quotient).  Five reading aids worth pinning before the
figure does the work:

\emph{(i) Interpretive choice.}  Where the carrier axis stops is
partly conventional.  Placing $\mathbb{C}$ at the front-right corner
treats $\mathbb{C}$ as a textbook carrier; placing $\mathbb{R}$ there
would treat $\mathbb{C}$ as the back-face quotient
$C(\mathbb{R}) = \mathbb{R}[i]/(i^2 + 1)$; placing $\mathbb{Q}$ would
treat both $\mathbb{R}$ (via $\mathrm{Cuts}$) and $\mathbb{C}$ (via
$\mathrm{Cplx} \circ \mathrm{Cuts}$) as quotient-axis nodes.  All
three readings are equally valid; the figure encodes one such choice
($\mathbb{C}$-as-carrier).  The carrier-vs-quotient line is partly
interpretive (§\ref{sec:carrier-rosetta}).

\emph{(ii) Computability-tier complement.}  A parallel three-axis
lattice on top of any cell --- representation (Int / Ext), logic
(K3 / Cl), knowledge (Opq / Elt) --- describes the density of
compile-time information at that cell
(§\ref{sec:gap}, Definition~\ref{def:computability-tier}).  That
lattice is orthogonal to the three drawn here: the algebraic tower
says \emph{which algebra}, the computability tier says \emph{how
much the compiler knows about a set built on it}.  We do not draw
the second cube --- one 3D figure already carries the structural
weight.

\emph{(iii) Commutation.}  Not all axes commute: $\mathfrak{R}$ and
$\mathfrak{Q}$ always commute ($M_n(X(R)) \cong X(M_n(R))$ when both
sides are defined); different $\mathfrak{Q}$ values commute when
their generating variables are independent ($D \circ C \cong C \circ D$);
$\mathfrak{A}$ and $\mathfrak{Q}$ commute partially --- e.g.\
$\mathrm{Frac}(D(R))$ is undefined ($D(R)$ has nilpotents) but
$\mathrm{Frac}(C(R)) \cong C(\mathrm{Frac}(R))$; $\mathfrak{A}$ and
$\mathfrak{R}$ commute restrictively ($\mathfrak{R}$ produces modules,
not rings, so subsequent $\mathfrak{A}$ steps may not type-check).
Per-cell compile-status is pinned in the trait registry (\#499); the
figure shows the structural shape.

\emph{(iv) Off-figure dimensions.}  Higher rank ($n > 2$; \#368),
predicate-set / order-theoretic constructions (candidate sibling
\emph{Geometric Tower}; future Figure~1b), iterated $\mathfrak{Q}$
(jet bundles; \#504), and the Honest-Rejection machine-substrate
boundary (\texttt{bool}, \texttt{uint}, \texttt{int}, \texttt{double}
embed into $\mathbb{B} / \mathbb{N} / \mathbb{Z} / \mathbb{Q}$ via
named \texttt{embed\_*} arrows in \texttt{dedekind.numbers}).  The 2D
projection is necessarily lossy.

\emph{(v) Structural caveat.}  The lattice has a unique bottom
($\mathbb{B}$) but \emph{no} unique top --- iterated quotients,
tensor products, Witt vectors, $p$-adic completions and similar each
yield a strictly larger object without a canonical maximum.  Binary
meets (largest shared substructure) are easy to identify; binary
joins (smallest common extension) exist categorically as pushouts
but are not unique without ambient choice, so in practice the
structure is better thought of as a bounded-below lattice (or,
informally, a meet-semilattice with constructive joins).  The
visual top-right-back corner $M_2(X(\mathbb{C}))$ is therefore an
illustrative stopping point, not a categorical maximum.
...
\to \mathbb{C}$ via successive functors Frac / Cuts / Cplx
...
degree-2 polynomial ideal --- $X = D$ gives
$R[\varepsilon]/(\varepsilon^2)$ (dual numbers), $X = C$ gives
$R[i]/(i^2 + 1)$ (complex extension), $X = G$ gives $R[x]/(p(x))$
for $p$ irreducible (Galois extension); see
Table~\ref{tab:rosetta-quotient} for the full enumeration.
...
$C$ (Cplx) & $(i^2 + 1)$ & $R[i]/(i^2 + 1)$ & field-extension when $p$ irreducible$^a$ & \lstinline|Complex<R>| (\#505 generalises) \\
...
\vspace{0.5em}
{\footnotesize
\textsuperscript{a} Cplx is a field-extension when $i^2+1$ is irreducible
over $R$ (true over $\mathbb{R}, \mathbb{Q}, \mathbb{Z}$); over $\mathbb{F}_2$
the polynomial factors as $(x+1)^2$ and the result is a nilpotent ring
isomorphic to $D(\mathbb{F}_2)$ (cf.\ \#505).  This is a concrete example of
the "same construction, different result depending on base" pattern the
quotient axis encodes.  Per-cell compile-status (which $R \times X$
