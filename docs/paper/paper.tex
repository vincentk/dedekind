...
\end{aligned}
\end{equation}
In code the comprehension is \cppinline{set(S,P)} and each formulation is one \cppinline{operator==} against a bound: (A) \cppinline{exists(S,P)} is \cppinline{!(Ø{} == set(S,P))}, (B) \cppinline{forall(S,P)} is \cppinline{set(S,P) == S}, and each recovers its partner by De Morgan's duality, complementing the predicate, \cppinline{!P} (Listing~\ref{lst:quantifiers_def}, top row).
...
Read the same definitions intensionally, as an algebra on the characteristic predicates, and both formulations collapse to one cheap question: does the composed predicate \cppinline{set(S,P)}, the meet $S \wedge P$, reduce to always-false (then $\exists$ fails) or to always-true (then $\forall$ holds)?
...
\begin{listing}[htbp]
\caption{The finite-quotient quantifiers.
\textbf{Top}: the generic \cppinline{exists} / \cppinline{forall} over an \cppinline{IsSet} domain \cppinline{S} and an \cppinline{IsPredicate} query \cppinline{P}, as (A) a test of \cppinline{set(S,P)} against $\varnothing$ and (B) against the input set $S$.
\textbf{Bottom}: the two worked cases, $\mathbb{B} = $\cppinline{Ω<bool>} with the predicate \cppinline{Singleton<true>}, and $\mathbb{N} = $\cppinline{Ω<Cardinality>} with \cppinline{isDivisibleBy3} (factored through \cppinline{Modular<3>}).}
\label{lst:quantifiers_def}
\noindent
\begin{minipage}[t]{0.49\linewidth}
\begin{cpp}
template <IsSet S, IsPredicate P>
constexpr bool
exists(S s, P p) {
  return !(Ø{} == (s | p));
}
...
Singleton<true> isTrue;
static_assert( exists(𝔹, isTrue));
static_assert(!forall(𝔹, isTrue));
\end{cpp}
\end{minipage}\hfill
\begin{minipage}[t]{0.49\linewidth}
\begin{cpp}
constexpr auto ℕ = Ω<Cardinality>;
constexpr auto isDivBy3 =
  in<ℕ> % Modular<3>{} == bound<0>;
static_assert( exists(ℕ, isDivBy3));
static_assert(!forall(ℕ, isDivBy3));
\end{cpp}
\end{minipage}
\end{listing}
