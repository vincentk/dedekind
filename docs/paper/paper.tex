...
% Source: SPEC — implementation follows before merge (operator-, Ø<ambient>);
%         verify test src/test/.../pruning_lattice_laws_test.cpp (TBD).
\begin{listing}[htbp]
\caption{The two absorbing laws of the complement lattice, witnessed as compile-time collapses on an \textbf{extensional} carrier ($\mathbb{B}$, whose two values enumerate) and an \textbf{intensional} one ($\mathbb{N}$, infinite and predicate-only).
From the universe, \cppinline{operator-} removes a subset $B$ to reach $\neg B$; \textbf{union} with $B$ round-trips back to the universe ($a \cup \neg a = \top$), while \textbf{intersection} with $B$ falls to the empty set ($a \cap \neg a = \bot$).
The reduction is a type identity, the same on both carriers: no element is ever enumerated.}
\label{lst:pruning}
\noindent
...
constexpr auto B    = Set{in<𝔹> | in<𝔹>};
constexpr auto notB = 𝔹 - B;   // {false}

// | B: round-trip to the universe
static_assert((notB | B) == 𝔹);

// & B: collapse to the empty set
constexpr Ø<𝔹> empty = notB & B;
\end{cpp}
\end{minipage}\hfill
\begin{minipage}[t]{0.49\linewidth}
$\mathbb{N} =$ \cppinline{Ω<Cardinality>}: intensional
\begin{cpp}
// ℕ is a library global
static_assert(IsSet<decltype(ℕ)>);


constexpr auto B = Set{
  in<ℕ> | in<ℕ> > bound<5>};    // {x>5}
constexpr auto notB = ℕ - B;   // {x<=5}

// | B: round-trip to the universe
static_assert((notB | B) == ℕ);

// & B: collapse to the empty set
constexpr Ø<ℕ> empty = notB & B;
\end{cpp}
\end{minipage}
\end{listing}
