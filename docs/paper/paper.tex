...
makes the intensional-first style concrete.  Fibonacci on $\mathbb{N}$ is
defined inductively by $F_0 = 0$, $F_1 = 1$, $F_{n+1} = F_n + F_{n-1}$;
the recurrence uses only addition, so the sequence stays in $\mathbb{N}$
throughout.  Negafibonacci on $\mathbb{Z}$ extends the same recurrence
to negative indices via $F_{n-1} = F_{n+1} - F_n$, producing
...
recurrence is not directly the universal property of the natural
numbers object (which captures one-step recurrences $a_{n+1} =
g(a_n)$); it is a coalgebra for the functor $F(X) = A \times X$
where the state carries the pair $(F_n, F_{n-1})$ and the step
function advances both.  Negafibonacci is the same coalgebra run
with the subtractive step function in place of the additive one ---
both well-typed \textbf{only} because the state carrier admits the
