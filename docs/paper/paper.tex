...
\begin{tikzpicture}[every node/.style={font=\small}]
  % bool
  \node (b)  at (1,0) {\texttt{bool}};
  \node (bf) at (0.1,1.6) {$\mathbb{F}_2$};
  \node (bl) at (1.9,1.6) {lattice};
  \draw[->,thick] (b)--(bf) node[midway,left=-2pt,font=\scriptsize]{$\oplus,\wedge$};
  \draw[->,thick] (b)--(bl) node[midway,right=-2pt,font=\scriptsize]{$\vee,\wedge$};
  % unsigned
  \node (u)  at (5,0) {\texttt{unsigned}};
  \node (ur) at (3.9,1.6) {$\mathbb{Z}/2^{w}\mathbb{Z}$};
  \node (ul) at (6.1,1.6) {lattice};
  \draw[->,thick] (u)--(ur) node[midway,left=-2pt,font=\scriptsize]{$+,\times$};
  \draw[->,thick] (u)--(ul) node[midway,right=-2pt,font=\scriptsize]{$\max,\min$};
  % double
  \node (d)  at (9.6,0) {\texttt{double}};
  \node (dc) at (8.4,1.6) {$\mathbb{R}/L\mathbb{Z}$};
  \node (dx) at (10.8,1.6) {fixed-pt};
  \node (dR) at (9.6,3.0) {$\mathbb{R}$};
  \draw[->,thick] (d)--(dc) node[midway,left=-2pt,font=\scriptsize]{$+\bmod L$};
  \draw[->,thick] (d)--(dx) node[midway,right=-2pt,font=\scriptsize]{bounded};
  \draw[->,dashed,gray] (d)--(dR) node[midway,right,font=\scriptsize,black]{models};
  % inter-cluster arrows (the tower)
  \draw[->,thick] (b.east)--(u.west) node[midway,above,font=\scriptsize\itshape]{embed};
  \draw[->,thick] (u.east)--(d.west) node[midway,above,font=\scriptsize\itshape]{to the continuum};
  % materialise boundary
  \draw[dashed] (7.25,-0.7) -- (7.25,2.3);
  \node[font=\scriptsize\itshape,align=center] at (7.25,2.65) {materialise\\ boundary};
  \node[font=\scriptsize\itshape] at (5.9,-0.65) {finite: exact};
  \node[font=\scriptsize\itshape] at (8.7,-0.65) {infinite: modelled};
\end{tikzpicture}}
\caption{The primitive carriers as a tower, reconciled.  \texttt{bool} carries its algebras ($\mathbb{F}_2$, the Boolean lattice) \emph{exactly} --- the algebra \emph{is} the carrier --- and embeds into \texttt{unsigned}, still finite and exact: a full commutative ring $\mathbb{Z}/2^{w}\mathbb{Z}$ that wraps cleanly.  Crossing to the continuum, \texttt{double} \emph{models} $\mathbb{R}$ but cannot materialise it (overflow, rounding --- the dashed arrow); yet on \emph{compact} targets it is controlled --- a modulus gives the circle group $\mathbb{R}/L\mathbb{Z}$, the \emph{continuous} $\mathbb{Z}/n\mathbb{Z}$, and a bounded range gives fixed-point.  The same ``mod a subgroup'' that makes \texttt{unsigned} an exact ring makes \texttt{double} an honest carrier --- discrete on the left of the materialise boundary, continuous on the right.  None is rejected; the only rejection is the unbounded claim $\texttt{double} = \mathbb{R}$ (Equation~\ref{eqn:model-materialise}).}
