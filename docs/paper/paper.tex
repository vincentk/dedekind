...
concept IsAlgebraOnSet =
    ...
    && (... && IsClosedUnderEither<typename X::Domain, Ops>)  // Alg: ops close on the carrier
    && IsAlgebra<typename X::Domain, Ops...>;                 // the (A, F) algebra
\end{cpp}
\end{listing}
An \emph{algebraic set} in \texttt{dedekind} is a C++ type that is a set in the sense of §\ref{sec:intensional_sets} and carries operations on that set (its \emph{carrier}).
...
Figure~\ref{fig:hub-spoke-bnz} from the introduction is a sketch of this category, worth keeping in hand for the rest of the section.
Its objects are algebraic sets: a carrier decorated with operations, the loops, with the bare carriers on the bottom row and the forgetful functor $U : \mathbf{Alg}(-) \to \mathbf{Set}$ dropping the operations to reach them.
One carrier can support several algebras, so $U$ is not injective on objects; the hooked homomorphisms are the carrier maps that preserve the operations, and the dotted maps are carrier maps that break one, falling to $\mathbf{Set}$.
A loop is an operation, so a binary one such as $\wedge$ or $+$ decorates its carrier as a map $A \times A \to A$, not a category endomorphism; concretely the two hooked inclusions $\mathbb{N} \hookrightarrow \mathbb{Z}$ preserve $(+, \times)$ and $(\oplus, \otimes)$, while $x \mapsto -x$ breaks $\times$ and $x \mapsto x^2$ breaks $+$.
...
The Boolean carrier $\mathbb{B}$ is an algebraic set the moment it type-checks as $\mathbf{Alg}(\wedge, \vee, \neg)$ over \cppinline{bool} (Listing~\ref{lst:algebra:boolean_case}): the object is the set, and $\wedge, \vee, \neg$ are the operations it closes under, with $\wedge$ and $\vee$ (both binary $\mathbb{B} \times \mathbb{B} \to \mathbb{B}$) drawn as loops on its Boolean-lattice node in Figure~\ref{fig:hub-spoke-bnz}.
