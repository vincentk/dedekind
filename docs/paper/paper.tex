...
The partition DAG is the diagram of inter-hub arrows.
...
\subsection{The Structuralist Intersection: $\mathbf{Ddk} = \mathbf{Jlt} \cap \mathbf{Alg}(-)$}
\label{sec:cpp-stlc-mapping}

The \texttt{dedekind} types and combinators are the inhabitants of $\mathbf{Jlt} \cap \mathbf{Alg}(-)$.
These are C++23 types in the disciplined fragment $\mathbf{Jlt}$ (§\ref{sec:jlt}) that simultaneously carry universal-algebra structure $\mathbf{Alg}(\Sigma)$ (§\ref{sec:alg-sigma}) for some signature $\Sigma$ the carrier supplies operators for.
The dash notation $\mathbf{Alg}(-)$ is the family $\bigcup_\Sigma \mathbf{Alg}(\Sigma)$ over the signatures the library names.
...
\paragraph{The encoding's deliberate ceiling: small categories only.}
If the encoding gates functors and adjunctions through C++23 concepts, the natural follow-up is whether it also gates the meta-categories $\mathbf{Set}$ and $\mathbf{Cat}$ themselves.
The deliberate answer is no: $\mathbf{Ddk}$'s categorical surface faithfully represents \emph{small} categories (one \cppinline{Species} per \cppinline{IsCategory}-witnessing type) and stops there.
The reason is foundational, not engineering: $\mathbf{Set}$ and $\mathbf{Cat}$ in textbook category theory are \emph{large} categories whose objects form a proper class rather than a set, and any unrestricted type-level encoding of ``all categories as objects of one category'' would court the same Russell-style self-reference that ZFC's missing set-of-all-sets is shaped to forbid.\footnote{The standard mathematical workaround is a Grothendieck universe hierarchy~\cite{maclane1971categories}: pick a universe $U$, define $\mathbf{Set}_U$ as the category of $U$-small sets, then let $\mathbf{Set}_U$ live as an object of a higher-universe $\mathbf{Cat}_{U'}$.  The C++ analogue would be type erasure on the \cppinline{Species} (an \cppinline{AnyCategory} wrapper), which works in principle but degrades the structuralist gate this paper is built on: erased witnesses are opaque to both the concept checker and the LLVM optimizer, so the paper's compile-time-correctness and structural-pruning stories ($\S$\ref{sec:compiler-wall}, $\S$\ref{sec:pruning}) would only fire on the un-erased layer below.  The trade weighs against erasure today.}
The project therefore takes the same discipline mathematics takes: small categories with concrete species; meta-categorical statements (the textbook $\mathrm{Disc} \dashv U$, the size-hierarchy adjunctions, free / forgetful pairs across $\mathbf{Cat}$) carried as documented prose-level CT vocabulary, anchored where possible by mechanically-witnessed \emph{restrictions} to a single small slice.
...
The 2-categorical layer above thus becomes the engineer's honesty obligation, named by reference rather than by type-level proof --- a feature of the encoding's deliberate scope, mirrored in $\S$\ref{sec:compiler-wall}'s account of where the type checker stops.
