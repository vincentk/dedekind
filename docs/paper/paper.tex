...
In the context of \texttt{dedekind}, we further restrict our attention to a subset that most readers will be familiar with: algebras,in the unversal algebra sense.
That is: sets equipped with operations realising a signature $\Sigma$: 
$\mathbf{Alg}(\Sigma) \subset \mathbf{Alg}(-) \subset \mathbf{Set} \subset \mathbf{Cat}$.\footnote{The unrelated algebraic-geometry usage of ``algebraic set'' for the zero locus of a system of polynomials is \emph{not} what we mean.}
In algebra textbook terminology and for a fixed algebraic signature $\Sigma$ (a list of operation symbols with their arities): the universal-algebra category $\mathbf{Alg}(\Sigma)$~\cite{burris1981universalalgebra}.
However, we \emph{deliberately leave $\Sigma$ unbound for now}, the intent is to defer the formalization of operations such as $\neg, \vee, \wedge, \cup, \cap, +, \times, \ldots$.
Instead, the binding of concrete operator-shape signatures is deferred to later sections~\ref{sec:cpp-stlc-mapping}, \ref{sec:sets-rules}.
\footnote{The dash is a typographic placeholder meaning ``fill in any signature here'' (Mac Lane's standard argument-blank convention~\cite{maclane1971categories}, same family as $\mathrm{Hom}(-, -)$ for the hom-bifunctor and $(-)^{\mathrm{op}}$ for the opposite-category functor); it is \emph{not} a unary minus.  The notation reappears in §\ref{sec:cpp-stlc-mapping}.}
Familiar examples of such algebras are: The Booleans $\mathbb{B}$ with logical operations, the integers $\mathbb{Z}$ with addition and subtraction and so on.
