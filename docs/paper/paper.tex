...
\bottomrule
\end{tabular}
\caption{The relation algebra sliced by concept (\cppinline{IsX} $\subset$ \cppinline{IsY}), each closed by its \cppinline{Trsk} row; the columns are a field's structure, undefined where a slice lacks it.
A function \emph{is} a relation (\cppinline{IsFunction} $\subset$ \cppinline{IsRelation}); the monoid/group is only what that sub-class forms under $\mathbin{;}$.
\textbf{The two inverses enter by different routes.}
The additive $-$ is symmetric difference $\triangle$ ($R\triangle R=\varnothing$: the $\mathrm{GF}(2)$ ring, Stone) --- available \emph{early}, on every Boolean rung (set, relation, scalar), by switching $\cup\!\to\!\triangle$ --- but lost where the class goes rigid, and there the lattice order $\subseteq$ collapses to $=$ (a function is a \emph{maximal} single-valued relation: no proper sub-function).
The multiplicative $\div$ enters \emph{late}, only by restricting to the invertibles: at the group it is compose-with-converse ($R/S=R\mathbin{;}S^{\circ}$, so $R^{-1}=R^{\circ}$), and at the scalars it is division in $\mathrm{End}(\mathbf{1})=\mathbb{F}_2$~\cite{kellylaplaza1980coherence} / $\mathbb{C}$~\cite{abramskycoecke2004categorical}.
A set is itself a relation --- its classifier $\chi\colon A\to\Omega$ (equivalently a coreflexive, $\mathbin{;}=\cap$) --- so it carries a dagger $\chi^{\circ}$, real in ETCS but \emph{unreified} in Trsk (parenthesised), since reifying it would cycle the set/relation bootstrap.
The converse is contravariant on relations (Tarski B7~\cite{tarski1941calculus}, cf.\ function-domain contravariance~\cite{cardelli1996typesystems, pierce2002tapl}), covariant on the commutative scalars; $\mathbf{0}=\varnothing$, $\mathbf{1}=\Delta$ throughout.}
