...
$\mathbf{CRing}$ (commutative unital rings; the
\lstinline{IsInitialRing} concept is specialised to that
category), and free abelian group on the commutative
...
\emph{Form $\to$ Carrier $\to$ Symbol}. The Form is the universal
property reified as a C++ concept (\lstinline{IsNNO},
\lstinline{IsInitialRing}, \lstinline{IsGrothendieckGroup}); the
carrier is the canonical inhabitant
(\lstinline{Cardinality} for $\mathbb{N}$ with $\aleph_0$
saturation; \lstinline{SignedCardinality} for $\mathbb{Z}$ with
$\pm\aleph_0$ and \lstinline{NaZ}); the symbol is the alias
practitioners use. Existence-and-uniqueness of the universal
morphism remains the engineer's honesty obligation; what the type
system pins mechanically is the structural shape (the
$(z, s)$ pair of the NNO; the $\chi_R$ shape of the initial-ring
homomorphism, witnessed concretely at $R =
\texttt{Modular}\langle n\rangle$ in
Figure~\ref{fig:initial-ring-modular}). The architecture's
recurring Platonist reading: textbook structure precedes machine
carriers.}

The pedagogical exhibit is the Fibonacci / negafibonacci pair: the
two-term recurrence $F_{n+1} = F_n + F_{n-1}$ on $\mathbb{N}$
extends by $F_{n-1} = F_{n+1} - F_n$ to negative indices on
$\mathbb{Z}$, with the backward step requiring exactly the
operator (subtraction) that structurally distinguishes $\mathbb{Z}$
from $\mathbb{N}$. Computing either series on the canonical
variant carriers keeps the practitioner in the textbook structure
throughout, with $\aleph_0$ / $\pm\aleph_0$ saturation as the
honest sentinel for oversize values; the descent to a machine
integer is an explicit realize-step.\footnote{The proper framing
is coalgebraic: a two-term recurrence is a coalgebra for $F(X) = A
\times X$ on state $(F_n, F_{n-1})$, not the NNO universal
property directly. The same recurrence on a 32-bit
\lstinline{int} silently overflows once $F_n > \texttt{INT\_MAX}/2$
(UB); on \lstinline{unsigned int} it wraps modulo $2^{32}$. The
variant carriers either return the correct large value or escalate
to $\aleph_0$ explicitly --- the law that fails on the machine
carriers becomes either a true claim or a typed sentinel. The
realize-step's proof obligation (sentinel for the transfinite
case) is precisely the truthfulness canon at work.}
