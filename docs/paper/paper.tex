...
We note a deep connection between converse and inverse, which we summarise in Table~\ref{tab:inverse-image-laws}'s $\dagger$-class column.
In a dagger category the dagger is itself the inverse, so the isometry, coisometry, unitary and partial-isometry conditions ($f^\dagger f = \mathrm{id}$, $f f^\dagger = \mathrm{id}$, both, $f f^\dagger f = f$) name the mono, epi, iso and general classes and supply their witness at no cost: ``the converse is the inverse'' (a bijective relation) and ``the transpose is the inverse'' (an orthogonal matrix) become one theorem.
...
\begin{tabular}{@{}lllll@{}}
\toprule
arrow $f$ & one-sided inverse & fibre $f^{-1}(y)$ & composition $h = g \circ f$ & $\dagger$-class \\
\midrule
...
\addlinespace
general $m\circ e$ & $f^\dagger$, $\;f f^\dagger\! f = f$      & $\bigcup_i \{s_i(y)\}$                             & $(g\circ f)^\dagger = f^\dagger\!\circ g^\dagger$ & partial isometry \\
\bottomrule
\end{tabular}%
}
...
The final column names the arrow's $\dagger$-class when a dagger is present: the dagger then \emph{is} the inverse (unitary $f^{-1}=f^\dagger$, isometry retract $r=f^\dagger$, coisometry section $s=f^\dagger$), and for a general $m\circ e$ the \emph{generalized} inverse $f^\dagger$ with $f f^\dagger f = f$, a partial isometry (the Moore--Penrose pseudoinverse in the Hilbert case).}
