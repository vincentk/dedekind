...
\textbf{Right} ($\mathbb{N}$): less-than, and the bounded relation $\{(x,y) \mid x<y \land x>5\}$.}
...
static_assert(!lt(pair{5,2}));
constexpr auto R =
  ℕ * ℕ | π1 < π2 & π1 > fix(5_c);
static_assert( R(pair{6,9}));
static_assert(!R(pair{3,9}));
\end{cpp}
\end{minipage}
\end{listing}
...
\cppinline{Double} is the appetizer: injective, its retract the obvious halving.
The main course keeps the machinery and drops the injectivity.
Take $f(x) = 2|x|$.
...
\begin{listing}[htbp]
\caption{The non-injective main course: $2|x|$ decided through its (regular epi, mono) factorization.  It is not monic, so it ships a \cppinline{retract} (the mono leg: the canonical preimage ${+}y/2$) and a \cppinline{cofibre} (the epi leg: the class $\{{+}x,{-}x\}$), and \cppinline{image} ORs the source over the whole fibre.  Sound where a lone retract is not: $6 \in 2|x|(\{x<0\})$ holds via ${-}3$, though the canonical ${+}3$ is not in the source (abridged from the checked-in test).}
