...
of it. The centrepiece (Section~\ref{sec:lp-centrepiece}) is a
textbook LP that reduces at compile time to its optimal vertex as a
typed constant; the same reduction over the dual-number ring
(Section~\ref{sec:ad-elliott}) recovers forward-mode automatic
differentiation as a sibling compile-time reduction in the sense of
Elliott~\cite{elliott2018simple}; a third showcase
(Section~\ref{sec:poly-ftc}) lifts the discipline to polynomial
calculus.
...
\paragraph{What else falls out for free.} Naming the structure in
the type buys non-trivial mileage at no runtime cost. Three quick
examples illustrate the pattern: (i) the scalar / column / row /
square family $\bigl(T, T^2, {}^t T^2, M_2(T)\bigr)$ is functorial
in $T$ and each functor is an $\textsc{IsFunctor}$ witness, so an
arrow $f: T \to T$ lifts elementwise without per-carrier
boilerplate; (ii) the scalar embedding $T \hookrightarrow M_2(T)$ is
the monadic unit $\eta_F$ of the shape functor, $\eta_{M_2}(s) = s
\cdot I_2$, pinned as a witness next to the carrier; (iii) the LP
reduction of Section~\ref{sec:lp-centrepiece} is a co-Kleisli arrow
$\varepsilon_{\mathrm{LP}}: \mathrm{Polytope}(T) \to T^2$ extracting
the optimal vertex from the polytope context.\footnote{Each
companion claim composes through the Kleisli machinery already in
the library (the $\mathrm{Path}$ comonad, the
$\mathrm{Box}/\mathrm{Maybe}$ monads), with bind ($\gg{=}$) and
extend ($\lll{=}$) operators. The categorical vocabulary is not
decorative: it lets the same primitives carry several theorem
discharges---elementwise lifting, scalar-into-matrix monomorphism,
LP counit---each as a typed constant in the emitted object file
where elsewhere it would cost a tactics file and a separate
...
$\mathtt{Dual}\circ\mathtt{Rational}\circ\mathtt{Complex}$ layering
...
\paragraph{Worked diagnostic.}
A rank-deficient \lstinline{Invertible2x2<Rat, 1, 2, 2, 4>} input
produces a single named diagnostic naming the broken predicate
(\texttt{Invertible2x2 requires full rank: a*d - b*c != 0}) at the
\texttt{static\_assert} site, with one \texttt{note} pointing at the
instantiation; no substitution-failure cascade.\footnote{Verbatim
\texttt{clang++}~22 output (with \texttt{-fno-color-diagnostics}
\texttt{-ferror-limit=2}):
\texttt{matrix.cppm:120:17: error: static assertion failed due to
requirement '!(det == ...\{0\})':\ Invertible2x2 requires full
rank: a*d - b*c != 0; failed\_proof.cpp:12:46: note: in
instantiation of template class
'dedekind::linear\_algebra::Invertible2x2<Rat, 1, 2, 2, 4>'
requested here; 1 error generated.} The predicate sits inside the
class body as a literal \texttt{static\_assert} rather than as a
\lstinline{requires}-clause that has to be probed, so the diagnostic
fires at the named axiom rather than as a probe failure deep in
overload resolution.}
This is \emph{fail fast at the named axiom}: the
mathematical claim that broke surfaces before the type-system
mechanics that surfaced it.
