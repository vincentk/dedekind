...
\mathbb{B} \arrow[loop above, "\neg"]
  \arrow[r, "\iota_{\mathbb{B}\hookrightarrow\mathbb{N}}"]
\& \mathbb{N} \arrow[r, shift left, "\iota_{\mathbb{N}\hookrightarrow\mathbb{Z}}"]
\& \mathbb{Z} \arrow[l, shift left, "\rho_{\mathbb{Z}\to\mathbb{N}}"]
\end{tikzcd}
\caption{Hub-and-spoke at three carriers, $\mathbb{B}$, $\mathbb{N}$, $\mathbb{Z}$.  Each hub is a single-species category: its objects share a carrier type, and its endomorphisms are arrows within the species (the loop on $\mathbb{B}$ shows one --- negation $\neg: \mathbb{B} \to \mathbb{B}$).  Inter-hub arrows are morphisms between species: the canonical embedding $\iota_{\mathbb{B}\hookrightarrow\mathbb{N}}$ (false $\mapsto$ 0, true $\mapsto$ 1), the embedding $\iota_{\mathbb{N}\hookrightarrow\mathbb{Z}}$, and a partial realisation $\rho_{\mathbb{Z}\to\mathbb{N}}$ in the reverse direction (defined on the non-negative branch; the loose end of the back-arrow is the structural reason \texttt{:partial} exists).  Most arrows on each hub and most inter-hub arrows are suppressed for clarity; the diagram shows one of each kind.}
