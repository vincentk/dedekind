...
front: the trait registry is populated with claims that are
constructively true of the carrier (\lstinline{is_associative_v<T,Op>}
asserted only where the carrier supports it operationally); the
language's escape hatches (\lstinline{reinterpret_cast},
\lstinline{const_cast}, undefined behaviour) are not used in the
public surface; concept formulations are chosen to match the math.
