...
\begin{table}[htbp]
\centering
\caption{\textbf{The algebraic lattice and its carriers.} Each structure of the branching poset (§\ref{sec:algebraic_lattice_carriers}), the concept that certifies it, and the carriers that realise it on the built-in (\cppinline{std} / IEEE) side and the exact-library side. Each carrier is listed under the strongest structure the code \emph{strictly} certifies; \textsuperscript{$\dagger$} marks operator-surface-only certification (the strict axiomatic witness is queued). \cppinline{I} abbreviates \cppinline{SignedExtensionalCardinal<>}.}
\label{tab:lattice-carriers}
\footnotesize
\begin{tabular}{@{}l l p{0.26\textwidth} p{0.30\textwidth}@{}}
\toprule
...
Field & \cppinline{IsField} & \cppinline{bool} under $(\oplus,\land)$ ($\mathbb{F}_2$) & $\mathbb{Q}$: \cppinline{Rational<I>}; $\mathbb{C}$: \cppinline{Complex}\textsuperscript{$\dagger$}; $\mathbb{R}$: \cppinline{ExactReal}\textsuperscript{$\dagger$} \\
Galois field & \cppinline{IsGaloisField} & \cppinline{bool} ($\mathbb{F}_2$, order 2) & \cppinline{𝔽64} (order 64) \\
Module / v.\ space & \cppinline{IsModule} / \cppinline{IsVectorSpace} & \cppinline{uint64_t} under $\oplus$, \cppinline{bool} scalars ($\mathbb{F}_2^{64}$) & \cppinline{Vec2V}; $\mathbb{D}$: \cppinline{Dual<Rational<I>>} \\
\bottomrule
\end{tabular}

\vspace{0.3em}
\begin{flushleft}
{\footnotesize \textsuperscript{$\dagger$} \cppinline{Complex} and \cppinline{ExactReal} certify only the ring/field \emph{operator surface} (\cppinline{HasRingOperators} / \cppinline{HasFieldOperators}); the strict \cppinline{IsField} witness is blocked behind the \cppinline{IsTotal} / species-trait gate.  \cppinline{Dual} is a strict module over $\mathbb{Q}$ carrying ring operators, but $\varepsilon^2 = 0$ has no strict \cppinline{IsRing} witness, so it sits with the modules.}
\end{flushleft}
\end{table}
