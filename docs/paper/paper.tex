...
\subsubsection[IEEE Fast Lane and Approximation Policies]{The IEEE Fast Lane and Numerical Approximation Policies (\texttt{dedekind.numbers.ieee}, \texttt{dedekind.numbers.approx})}
\label{sec:ieee-approx}

Standard floating-point arithmetic pervades scientific computing, yet its algebraic contract is almost entirely implicit: associativity, commutativity, and identity are assumed by convention rather than certified by the type system.
The \texttt{dedekind.numbers.ieee} and \texttt{dedekind.numbers.approx} modules address this gap through an \emph{effect-context} design: arithmetic may be fast by default, but every operation that enters the certified path is algebraically stamped.
...
The propagation formula follows the classical treatment of \citet{ku1966notes}:
\begin{equation}
  \delta z \;\approx\; \bigl|\tfrac{\partial f}{\partial x}\bigr|\,\delta x
                    + \bigl|\tfrac{\partial f}{\partial y}\bigr|\,\delta y ,
\end{equation}
with an additive machine-epsilon contribution modelled after the round-off proxy analysis of \citet{higham2002accuracy}.
...
\begin{itemize}
  ...
  \item \textbf{\texttt{ReportErrorPolicy}}: surface the outcome as a \texttt{TernaryResult} (Regular / NearZero / Huge / NonFinite), providing a structured signal without aborting the pipeline.
