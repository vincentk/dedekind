...
Each carrier is depicted with its underlying set as the single object ($\{\mathrm{false}, \mathrm{true}\}$, $\{0, 1, 2, \ldots\}$, $\{\ldots, -1, 0, 1, \ldots\}$) and selected endo-operations as loops: Boolean negation $\neg$ on $\mathbb{B}$, squaring $x \mapsto x^2$ on $\mathbb{N}$, negation $x \mapsto -x$ on $\mathbb{Z}$.
...
One safe carrier is $\mathbb{B}$ as the Galois field $\mathbb{F}_2$ (Boolean negation, conjunction, exclusive-or). It has finite cardinality, no rounding, no overflow.
...
Arithmetic on $\mathbb{Q}$ is exact, so associativity, distributivity, and the field axioms hold by construction.
...
%          src/main/modules/dedekind/numbers/rational.cppm (line 917);
%          src/main/modules/dedekind/category/species.cppm (line 1068).
\begin{listing}[ht]
\caption{Honest Rejection in code: $\mathbf{Ddk}$ ships the Galois field $\mathbb{F}_2$, the integers $\mathbb{Z}$ as a strict commutative ring, and the field of rationals $\mathbb{Q}$ safe by default; \cppinline{int} is refused at the gate.  All five static\_asserts compile in $\mathbf{Ddk}$ today.}
...
// ℚ = Rational<I>: the bona fide field of fractions Frac(ℤ).
// Exact arithmetic on the (numerator, denominator) pair — associativity,
// distributivity, and the field axioms hold by construction.  The
// literal field-operator surface (+, -, unary -, *, /, T{1}) closes
// mechanically on Rational<default_integer>.
static_assert(HasFieldOperators<Rational<default_integer>>);
...
\paragraph{The fast lane: one trait specialisation away.}
A library user pushes the button by writing a per-carrier trait specialisation that registers \cppinline{is_associative_v<T, Op> = true} (or \cppinline{is_commutative_v}, \cppinline{is_invertible_v}, and so on).
The downstream concepts (\cppinline{IsRing}, \cppinline{IsField}) then discharge.
...
A pre-existing standard-library type such as a \cppinline{std::pair}, an \cppinline{std::ranges::view}, or a \cppinline{std::optional} becomes an inhabitant of $\mathbf{Ddk}$ the moment a \cppinline{concept} accepting it is written, with no modification to the carrier.
...
The body's \cppinline{IsFunctor} concept (Listing~\ref{lst:isfunctor}, §\ref{sec:cpp-stlc-mapping}) encodes the same content structurally; the static\_assert there is the Curry--Howard discharge that \texttt{maybe\_functor} inhabits the concept.
The listing also lands the System-F-recovery rosetta (Table~\ref{tab:structuralist-bridge}) at one concrete site: the \cppinline{template <typename F>} header is the \emph{Constrained Templates} row ($\equiv \forall\alpha.\dots$); the \cppinline{requires}-clause is the \emph{Concept-gating} row discharging parametricity; the \cppinline{static_assert} is the Curry--Howard term.
