...
\midrule
Trsk quantifier & $\forall a\,\exists b$ & $\forall a\,{\le}1\,b$ & $\forall b\,\exists a$ & $\forall b\,{\le}1\,a$ \\
\addlinespace
...
\begin{minipage}[t]{0.49\linewidth}
\begin{cpp}
constexpr auto op = arrow<ℕ,ℕ>(   // opaque λ
  [](ℕ m){ return m + fix(1_c); });
constexpr auto img =
  image(op, Set{in<ℕ> | in<ℕ> > bound<5>});
static_assert(               // honest wall
  img(fix(7)) == Ternary::Unknown);
\end{cpp}
\end{minipage}
\end{listing}
