...
\end{equation}
the ETCS power-object bound $\beth_1$ above (Appendix~\ref{sec:etcs}, Axiom~9) meeting the finite reach of $\mu$ below.
...
\label{def:trsk-rel}
A $\mathbf{Trsk}$ relation is an $\mathbf{Lwv}$ subobject (Definition~\ref{def:lwv-set}) of a Cartesian product $A \times B$ that also carries the dioid structure of this section --- converse, relative product, and the diagonal unit.  Where $\mathbf{Lwv}$ is the set fragment, $\mathbf{Trsk}$ is its point-free relational refinement,
\begin{equation}
\label{eqn:trsk}
