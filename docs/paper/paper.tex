...
\end{equation}
In code the comprehension is \cppinline{set(S,P)} and the emptiness test is an overload of \cppinline{Ø::operator==}, so \cppinline{exists(S,P)} is \cppinline{!(Ø{} == set(S,P))} and \cppinline{forall} its $\neg\exists\neg$ dual (Listing~\ref{lst:quantifiers_def}, top row).
...
\textbf{Top:} \cppinline{exists} is non-emptiness of the comprehension \cppinline{set(S,P)}; \cppinline{forall} its $\neg\exists\neg$ dual.
