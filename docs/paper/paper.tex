...
\begin{table}[htbp]
\centering
\caption{The three Birkhoff constructions H, S, P read across vocabularies.  H and S are the epi and mono of the arrow classification of \S\ref{sec:relations_functions_blueprint} (Table~\ref{tab:relation-quantifier}): every homomorphism factors as an \emph{epi} onto its image (H) followed by a \emph{mono} into the codomain (S), so a subalgebra is the mono half and an \emph{embedding} is its arrow.  P is the product.  The \emph{finitary} row marks that only finite products are type-level constructible.}
...
arrow class          & epi: a surjection                       & mono: an injection ($=$ embedding) & product: a limit \\
...
optimization         & normalise / fold                        & strength-reduce                 & prune (dead lanes) \\
exemplar             & $\mathbb{Z}/N\mathbb{Z}$, $F[\varepsilon]/(\varepsilon^2)$ & $\mathbb{N} \subseteq \mathbb{Z}$ & $T^n$ (vector, matrix) \\
finitary             & yes                                     & yes                             & \emph{finite} products only \\
\bottomrule
\end{tabular}
\end{table}
