...
\begin{itemize}
    ...
    \begin{enumerate}
        \item \textit{Subobject Classifier:} a truth-value object $\Omega$ with an element $\mathrm{true}: 1 \to \Omega$, so every subobject $S \subseteq A$ is named by a characteristic map $\chi: A \to \Omega$.
        \item \textit{Equalizers:} for parallel arrows $f, g: A \to B$, the subobject $\{x \in A \mid f(x) = g(x)\}$.
    \end{enumerate}
    \item \textbf{C++ Reification:} $\Omega$ is a pluggable \emph{logic species}. \cppinline{concept IsLogicalSpecies} asks for a type carrying $\Omega$ with its \cppinline{AND} / \cppinline{OR} / \cppinline{NOT} and \cppinline{True} / \cppinline{False}. Two ship: \cppinline{ClassicalLogic} ($\Omega = $ \cppinline{bool}) and \cppinline{TernaryLogic} ($\Omega = $ \cppinline{Ternary}, the Kleene $K_3$ with an \cppinline{Unknown} value). The characteristic map $\chi$ is the predicate of a \cppinline{Set<T, L, P>}, whose \cppinline{operator()} returns $\Omega$; \cppinline{concept IsSubobject} recognises it. Which $\Omega$ a carrier receives is the countable versus uncountable bifurcation of Section~\ref{sec:ddk_etcs}: $\mathbb{B}$ on the countable track, $K_3$ on the uncountable. Equalizers are named by \cppinline{concept IsEqualizer}, the subobject solving $f = g$.
