...
\begin{listing}[htbp]
\caption{The \emph{net fiber} of the closure operator (Table~\ref{tab:relation-matrix-map}): the single-source semiring closure as a fold over the edge sequence, gated on \cppinline{IsSemiring}, the operators defaulted per carrier.
The fold threads its accumulator in place, but the potential net never escapes the call, so \cppinline{constexpr} evaluation still folds it to the literal IR of Listing~\ref{lst:necklace-ir}.}
\label{lst:closure}
\begin{cpp}
...
The net fold of \S\ref{subsec:semiring_closure} is one \emph{instance} of a single operator, the closure of a seed under a one-step expansion,
\begin{equation}
\label{eqn:closure}
\mathrm{closure}(X, Y) \;=\; \bigsqcup_{n} Y^{n}(X),
\end{equation}
the least fixpoint of $S \mapsto X \sqcup Y(S)$.
Reachability is $R^{*} = \mathrm{closure}(\Delta,\ \lambda S.\, S ; R)$ over the Boolean semiring, and the critical path is the same closure over the tropical dioid; the iteration bound is the finite-subgraph selection a partial evaluation performs (Appendix~\ref{app:partial-eval}).
