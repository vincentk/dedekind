...
Whether an arbitrary carrier obeys a given algebraic law, or an arbitrary program computes a given function, is a non-trivial semantic property, and so undecidable in general (Rice's theorem~\cite{rice1953classes}).
The library therefore does not attempt the universal claim; it supplies mechanical support for the \emph{existential} one.
A specific carrier, a specific law, a specific reduction can be named in the type and discharged by the compiler even where no general procedure could.
Read this way, Rice's theorem is less a wall than an invitation to contribute: it marks the seam between the aspirational (a general, automatic verifier) on the one hand and current best practice (checkable existential demonstrations with mechanical support) on the other.
