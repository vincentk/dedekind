...
\cppinline{materialise} (Table~\ref{tab:relation-matrix-map}) is a \emph{dioid homomorphism} onto the matrix semiring $\mathrm{Mat}(S)$, sending $\varnothing\mapsto 0$, $\Delta\mapsto I$, $\cup\mapsto\oplus$, $\mathbin{;}\mapsto\otimes$, so reachability is this dioid at $S=\mathbb{B}$ and shortest path a semiring apart (Table~\ref{tab:semirings}).}
...
\toprule
 & $\subseteq$ & $+$ & $-$ & $\times$ & $\div$ & $(\cdot)^{\circ}$ & $\mathbf{0}$ & $\mathbf{1}$ \\
\midrule
$\chi\colon A\to\Omega \;(\cong \{R\subseteq\Delta\})$ & $\subseteq$ & $\cup$ & $\triangle$ & $\cap$ & --- & $\chi^{\circ}$ & $\varnothing$ & $\Delta$ \\
\cppinline{IsSet} $\subset$ \cppinline{IsBooleanAlg} & \cppinline{<=} & \cppinline{|} & \cppinline{^} & \cppinline{&} & --- & ($\chi^{\circ}$) & \cppinline{Ø} & \cppinline{Ω} \\
\midrule
dioid $(S,\oplus,\otimes)$ & $\subseteq$ & $\oplus$ & --- & $\otimes$ & --- & $(\cdot)^{\ast}$ & $\bar{0}$ & $\bar{1}$ \\
relation $A\times B$ & $\subseteq$ & $\cup$ & $\triangle$ & $\mathbin{;}$ & --- & $R^{\circ}$ & $\varnothing$ & $\Delta$ \\
$\mathrm{Mat}(S)$ & $\subseteq$ & $\oplus$ & --- & $\otimes$ & --- & $A^{\dagger}$ & $0$ & $I$ \\
\cppinline{IsRelation} $\subset$ \cppinline{IsDioid} & \cppinline{<=} & \cppinline{+} & \cppinline{^} & \cppinline{*} & --- & \cppinline{conv} & \cppinline{Ø} & \cppinline{diagonal} \\
\midrule
monoid $(M,\circ,e)$ & $=$ & --- & --- & $\circ$ & --- & --- & --- & $e$ \\
function $A\to A$ & $=$ & --- & --- & $\mathbin{;}$ & --- & $R^{\circ}$ & $\varnothing$ & $\Delta$ \\
\cppinline{IsFunction} $\subset$ \cppinline{IsMonoid} & \cppinline{==} & --- & --- & \cppinline{*} & --- & \cppinline{conv} & \cppinline{Ø} & \cppinline{diagonal} \\
\midrule
group $(G,\circ,{}^{-1}\!,e)$ & $=$ & --- & --- & $\circ$ & ${}^{-1}$ & ${}^{-1}$ & --- & $e$ \\
permutation $P$ & $=$ & --- & --- & $\mathbin{;}$ & $\mathbin{;}P^{\top}$ & $P^{\top}\!\!=\!P^{\circ}$ & $\varnothing$ & $\Delta$ \\
\cppinline{IsIsomorphism} $\subset$ \cppinline{IsGroup} & \cppinline{==} & --- & --- & \cppinline{*} & \cppinline{/} & \cppinline{inv} & \cppinline{Ø} & \cppinline{diagonal} \\
\midrule
$\mathrm{End}(\mathbf{1})=\mathbb{F}_2$ & $\subseteq$ & $\triangle$ & $\triangle$ & $\cap$ & $\cap$ & $\mathrm{id}$ & $\varnothing$ & $\Delta$ \\
$\mathrm{End}(I)=\mathbb{C}$ & --- & $+$ & $-$ & $\circ$ & ${}^{-1}$ & $\dagger$ & $0$ & $1$ \\
\cppinline{IsScalar} $\subset$ \cppinline{IsField} & \cppinline{<=} & \cppinline{+} & \cppinline{-} & \cppinline{*} & \cppinline{/} & \cppinline{conj} & \cppinline{0} & \cppinline{1} \\
\bottomrule
\end{tabular}
\caption{The relation algebra sliced by concept: each slice pairs a relational concept with an algebraic one (\cppinline{IsX} $\subset$ \cppinline{IsY}), listed over several vocabularies and closed by its \cppinline{Trsk} row; a dash marks a column the slice does not carry.
The columns are the order $\subseteq$, addition $+$ and its inverse $-$, multiplication $\times$ and its inverse $\div$, the involution $(\cdot)^{\circ}$, and the units $\mathbf{0}$, $\mathbf{1}$.
Top to bottom the slices are: sets (the classifier $\chi\colon A\to\Omega$, equivalently the coreflexive relations $R\subseteq\Delta$; its dagger $\chi^{\circ}$ is parenthesised, unreified in Trsk); relations (the dioid, and the matrix semiring $\mathrm{Mat}(S)$); functions (monoid); isomorphisms (group); scalars ($\mathrm{End}(\mathbf{1})=\mathbb{F}_2$ and $\mathrm{End}(I)=\mathbb{C}$~\cite{kellylaplaza1980coherence, abramskycoecke2004categorical}; field).
Addition is union $\cup$ with inverse the symmetric difference $\triangle$; multiplication is composition $\mathbin{;}$ (meet $\cap$ on coreflexives) with inverse $R\mathbin{;}S^{\circ}$; the units are $\mathbf{0}=\varnothing$ and $\mathbf{1}=\Delta$.}
\label{tab:relation-algebra}
\end{table}
...
\toprule
 & $\subseteq$ & $+$ & $-$ & $\times$ & $\div$ & $(\cdot)^{\circ}$ & $\mathbf{0}$ & $\mathbf{1}$ \\
\midrule
$R^{A}$ (set: diagonal algebra) & --- & $+$ & $-$ & $\odot$ & --- & $\overline{(\cdot)}$ & $0$ & $\mathbf{1}$ \\
$\mathrm{Mat}_A(R)=R[C]$ (relation) & --- & $\oplus$ & $\ominus$ & $\ast$ & --- & $(\cdot)^{\top}$ & $0$ & $I$ \\
monomial matrices (function) & --- & --- & --- & $\ast$ & --- & $(\cdot)^{\top}$ & --- & $I$ \\
$R[G]$ (iso: group algebra) & --- & $\oplus$ & $\ominus$ & $\ast$ & $g^{-1}$ & $(\cdot)^{\ast}$ & $0$ & $\delta_e$ \\
$R=R[\mathbf{1}]$ (scalar: field) & --- & $+$ & $-$ & $\cdot$ & $\div$ & $\overline{(\cdot)}$ & $0$ & $1$ \\
\bottomrule
\end{tabular}
\caption{The same ladder \emph{linearised}: each Boolean relation is replaced by its $R$-linearisation, the \emph{category algebra} $R[C]$ --- the free $R$-module on the arrows with \emph{convolution} $\ast$ (the sum $\sum_j R_{ij}\,S_{jk}$, i.e.\ the matrix product) as multiplication; columns as in Table~\ref{tab:relation-algebra}, a dash marking an absent operation.
Rows: the diagonal algebra $R^{A}$ (a set's linearisation), the matrix / category algebra $\mathrm{Mat}_A(R)=R[C]$ (a relation's), its deterministic monomial part (a function's), the group algebra $R[G]$ (an isomorphism's), and the coefficient ring $R=R[\mathbf{1}]$ (a scalar's); the incidence algebra of a poset~\cite{rota1964foundations} and the path algebra of a quiver~\cite{assem2006elements} are the order and graph cases.
Addition is the $R$-linear sum with inverse from $R$; multiplication is convolution $\ast$; the involution is the transpose/adjoint; $\mathbf{0}$ is the zero, $\mathbf{1}$ the identity element $\delta_e=I$.
Read against Table~\ref{tab:relation-algebra}: the linear form carries the additive inverse $-$ but not the order $\subseteq$; the Boolean form the reverse.}
\label{tab:relation-algebra-linear}
\end{table}
