...
The \texttt{dedekind} types and combinators are the inhabitants of $\mathbf{Jlt} \cap \mathbf{Sngl}$\footnote{We use $\mathbf{Sngl}$ as a project-specific mnemonic (\emph{single-species}) naming the engineering-library fragment of $\mathbf{Cat}$ developed in §\ref{sec:sngl} (Burris--Sankappanavar~\cite{burris1981universalalgebra} §II.10).  The relationship to the textbook $\mathbf{Mon}$ (the category of monoids; equivalently, one-object categories under Mac Lane~\cite{maclane1971categories} §I.1's bijection) is one of generalisation: single-species categories admit operations of arbitrary signature on a shared carrier (e.g.\ both $+$ and $\cdot$ on a ring), where a monoid carries only one binary operation.  Concretely, $\mathbf{Mon} \subset \mathbf{Sngl} \subset \mathbf{Cat}$.  We use the project-specific mnemonic to make the engineering generalisation visible at the symbol layer rather than overload $\mathbf{Mon}$ with a meaning it does not carry in the standard textbook reading.}: C++23 types and arrows that simultaneously sit in the disciplined syntactic fragment $\mathbf{Jlt}$ and carry single-species categorical structure from $\mathbf{Sngl}$.
