...
\begin{listing}[ht]
\caption{An \emph{algebraic set} reified, assembled from smaller concepts.  \textbf{Left:} \cppinline{IsAlgebraOnSet} conjoins \cppinline{IsSet} (the $\mathbf{Trsk}$ layer of Listing~\ref{lst:concept_isset}) with \cppinline{IsAlgebra}, the universal-algebra $(A, F)$ pattern that checks the operations close on the carrier.  \textbf{Right:} a sample structure built the same bottom-up way, the monoid: a magma (closure) refined by associativity and a unit.  The per-shape \cppinline{Has*Operators} concepts pin which operator signature each carrier supplies, so $\mathbf{Ddk}$ stays as wide as needed while a concrete variety is one fixed choice.}
...
a sample algebra: the monoid
\begin{cpp}
export template <typename T, typename Op>
concept IsMagma  = IsTotal<T, Op>
                && /* Op closes: A×A → A */;
export template <typename T, typename Op>
concept IsMonoid = IsMagma<T, Op>       // closure
   && IsAssociative<T, Op>   // + assoc
   && IsPointed<T, Op>;      // + unit
\end{cpp}
\end{minipage}
\end{listing}
...
\label{eqn:ddk}
    \mathbf{Ddk} \;=\; \mathbf{Trsk} \,\cap\, \mathbf{Alg}(-).
\end{equation}
Listing~\ref{lst:algebra:boolean_case} exhibits two inhabitants: the primitive \cppinline{bool}, at once a lattice and the field $\mathbb{F}_2$, and the exact rationals $\mathbb{Q}$.

% Source excerpts from dedekind.algebra:boolean and dedekind.numbers:rational.
\begin{listing}[ht]
\caption{Concrete carriers inhabiting the concepts of Listing~\ref{lst:algebra:universal}.  \textbf{Left:} the Boolean universe on the primitive \cppinline{bool}, at once a lattice under the logical operators $(\vee, \wedge)$ and the field $\mathbb{F}_2$ under the bitwise operators $(\oplus, \wedge)$; the carrier is a primitive type.  \textbf{Right:} the exact rationals $\mathbb{Q}$, a field under standard arithmetic.}
\label{lst:algebra:boolean_case}
\noindent
\begin{minipage}[t]{0.52\linewidth}
Boolean universe (primitive \cppinline{bool})
\begin{cpp}
constexpr UniversalSet<bool, ClassicalLogic, Finite>
    𝔹 = sets::Ω<bool>;

// logical (∨, ∧): a lattice
static_assert(IsLattice<bool,
    std::logical_or<bool>, std::logical_and<bool>>);
// bitwise (⊕, ∧): the field F_2
static_assert(IsGaloisField<bool,
    std::bit_xor<bool>, std::bit_and<bool>>);
\end{cpp}
\end{minipage}\hfill
\begin{minipage}[t]{0.45\linewidth}
Rationals $\mathbb{Q}$ (exact)
\begin{cpp}
using ℚ = Rational<default_integer>;
                    // exact Frac(ℤ)

// arithmetic (+, ×): a field
static_assert(IsField<ℚ>);
\end{cpp}
\end{minipage}
\end{listing}

\subsection{The carrier poset}
\label{sec:carrier-lattice}

The carriers form a poset: a spine $\mathbb{B}, \mathbb{N}, \mathbb{Z}, \mathbb{Q}, \mathbb{R}$ that branches at $\mathbb{R}$ into the complex and dual numbers, $\mathbb{C} = \mathbb{R}[i]/(i^2{+}1)$ and $\mathbb{D} = \mathbb{R}[\varepsilon]/(\varepsilon^2)$, and the module carriers $R^n$ and $M_n(R)$.
Each edge is a \emph{universal construction}, a canonical way to make a new carrier from an old one; none is ours to invent, they are textbook, and the library only checks that each type is what it claims.
Three of these constructions preserve algebraic laws for a single reason, one theorem (Birkhoff~\cite{burris1981universalalgebra}): whatever laws the ingredients satisfy, their product, their quotients, and their subalgebras satisfy too.
That one theorem is what the trait engine turns into mechanical propagation, and it is the whole of Rule~2 (§\ref{sec:hsp_varieties}).
The library's other carrier-building moves, fractions ($\mathbb{Z} \to \mathbb{Q}$), completion ($\mathbb{Q} \to \mathbb{R}$), and the matrix algebra ($R^n \to M_n(R)$), are equally canonical but each preserves structure by its own theorem, from a different branch of mathematics, so no single closure rule reaches them (Table~\ref{tab:constructions}).
...
So the engine mechanizes exactly the Birkhoff-closed core, the fragment where one theorem does all the work, and files fractions there too because $\operatorname{Frac}$ behaves enough like a quotient to ride the same machinery.
Completion and the matrix step it does not model as law-preserving constructions at all; it builds them by hand.
The honest statement is therefore narrow and true: \texttt{dedekind} propagates certificates across the Birkhoff-closed fragment of the carrier poset, and leaves the rest as the engineer's obligation, the same seam the rest of §\ref{sec:algebraic_lattice_carriers} works.
...
\subsection{How certificates lift}
\label{sec:hsp_varieties}

A carrier's certificates, the laws it is known to satisfy, are not re-proven for each new type.
They \emph{lift}, by two composing rules: a concept that composes certificates, and an engine that propagates them.

\paragraph{Rule 1: composition on the parameter.}
Some certificates are a weaker certificate conjoined with a property of the parameter.
A vector space is the textbook case, a module whose scalars form a field.
The library says exactly that as a concept rather than a comment (Listing~\ref{lst:rule1-compose}), so it gates a \cppinline{requires} clause the way \cppinline{IsRing} does, and the compiler discharges it with no per-carrier opt-in.

% Source excerpt from dedekind.algebra:vectorspace.
\begin{listing}[ht]
\caption{Rule 1: a vector space is a module over a field, one concept composed from two existing certificates.  \cppinline{is_vector_space_v} is the boolean witness the compiler discharges.}
\label{lst:rule1-compose}
\begin{cpp}
template <typename V, typename F, /* operator defaults */>
concept IsVectorSpace = IsModule<V, F, ...> && category::IsField<F, ...>;

static_assert(is_vector_space_v<Vec2V<ℚ>, ℚ>);  // discharged, no per-carrier code
\end{cpp}
\end{listing}

\paragraph{Rule 2: propagation through a constructor.}
The other rule carries a certificate from a base carrier to one built from it.
A class of algebras closed under homomorphic images, subalgebras, and products is a variety (Birkhoff~\cite{burris1981universalalgebra}), and the library realises that closure as \emph{trait propagation}: a one-line registration names the base, and the species traits (\cppinline{is_associative_v}, \cppinline{is_commutative_v}, and the rest) forward to the derived carrier for every parameter.
The engine, a registration point plus one forwarding specialisation per trait, and a worked closure over $\mathbb{Z}/6\mathbb{Z}$ are in Appendix~\ref{sec:carrier-rosetta} (Listings~\ref{lst:rule2-engine} and~\ref{lst:hsp-closed}).
The gate §\ref{sec:optimization_lp} uses, \cppinline{IsSemiring}, reads this same registry, so its exhibit's gate is a lookup, not a re-proof.

\paragraph{The two compose.}
Rule 2 lifts \cppinline{IsModule} onto \cppinline{Vec2V<T>} for every \cppinline{T}, which is registered as a product algebra, and Rule 1 upgrades that to \cppinline{IsVectorSpace} over any field.
Module and vector space are the first two rungs of a ladder that reaches linear operators, climbed in the subsections that follow.


\subsection{The ladder, and its honest edge}
\label{sec:certificate_ladder}

The two rules compose into a ladder, and over the rationals the whole of it type-checks.
Each rung is a certificate the compiler discharges, none re-proven (Listing~\ref{lst:certificate-ladder}): the scalar $\mathbb{Q}$ as the field of fractions of $\mathbb{Z}$, the rank-2 free module $\mathbb{Q}^2$, that module read as a vector space once its scalars form a field, and the $2\times2$ matrices as the endomorphism ring $M_2(\mathbb{Q}) = \operatorname{End}(\mathbb{Q}^2)$, matrix multiplication being composition of linear maps.
...
\begin{cpp}
using Q = Rational<default_integer>;         // scalar: ℚ = Frac(ℤ)
...
These rungs are existential, and deliberately so.
One would like the universal, a single lift certifying scalar to module to vector space to operator for \emph{every} carrier.
That is the aspiration of §\ref{sec:introduction}, and Rice's theorem is why it stays one: whether an arbitrary carrier satisfies a given law is undecidable, so the library exhibits witnesses and reads the universal as an invitation rather than a theorem.

The edge shows when the same vector-space lift is asked over three scalars (Table~\ref{tab:lift-spectrum}).
Over $\mathbb{Q}$ it is clean.
Over $\mathbb{C}$ the field certificate is withheld on purpose, because $x^2+1$ is reducible for some field parameters (over $\mathbb{F}_2$ it factors as $(x+1)^2$), so a blanket lift would be false.
Over $\mathbb{R}$ the strict field is stopped by the \cppinline{IsTotal} policy gate rather than by any mathematics, a gate $\mathbb{Q}$ clears only because its base registers a saturating totality path and $\mathbb{R}$ registers none.

\begin{table}[ht]
\centering
\caption{One lift, three honest answers: the vector-space certificate asked over three scalar carriers.}
\label{tab:lift-spectrum}
\small
\begin{tabular}{@{}l l l@{}}
\toprule
scalar & vector-space lift & reason \\
\midrule
$\mathbb{Q}$ & clean, witnessed above & a module over a field, no side condition \\
$\mathbb{C}$ & field certificate withheld & $x^2+1$ reducible over $\mathbb{F}_2$; a blanket lift would be false \\
$\mathbb{R}$ & blocked by \cppinline{IsTotal} & a policy gate, not a genuine obstruction \\
\bottomrule
\end{tabular}
\end{table}

The top rung repays a closer look, because an operator wears two faces.
The next subsection separates them.

\subsection{Operators, two faces}
\label{sec:operators_two_faces}
\label{sec:operational_calculus}
\label{sec:geometric_mappings}

The operators at the top of the ladder come in two forms, and \texttt{dedekind} keeps both.

\paragraph{Materialised: matrices.}
The first face is the endomorphism ring $M_2(F) = \operatorname{End}(V)$ of §\ref{sec:certificate_ladder}: an operator is an endofunction on the carrier, written as a grid of coefficients, and composition is matrix multiplication.
Here the honest edge is sharpest, because the rung has three layers and only two are mechanical.
That an operator is \emph{linear} the compiler checks as a concept, \cppinline{IsLinearMorphism} ($f(s \cdot x) = s \cdot f(x)$), and that the matrices carry a ring is another, \cppinline{HasRingOperators}.
But the identity that this ring \emph{is} the endomorphism ring, $M_2(F) = \operatorname{End}(V)$, is registered as a trait rather than derived as a concept, because no concept derives it from the operator surface alone.
It is a declaration the author makes and the compiler records, the honesty obligation of §\ref{sec:introduction} arriving at the top of the ladder.

\paragraph{Coordinate-free: boundary and coboundary.}
The second face never builds the grid.
With the carriers of §\ref{sec:algebraic_lattice_carriers} in place, the linear algebra over them stays matrix-free: the two operators a graph actually needs are kept as structural mappings between functions on its cells.
A graph is a one-dimensional complex, a node potential a 0-form $d : \mathcal{V} \to S$, an edge value a 1-form on $\mathcal{E}$.
...
Neither is materialised; each is an expression over the incidence structure, the intensional reading of §\ref{sec:intensional_sets} carried up from sets to operators.
The mnemonic is calculus: $\delta$ is the differential, $\partial$ is evaluation at the endpoints, and the discrete Stokes theorem $\int_\gamma \delta d = \langle d,\ \partial\gamma\rangle$ ties them.

The two faces meet in the graph Laplacian $\Delta = \delta\partial + \partial\delta$, an endomorphism assembled from the grade-shifting maps that individually are not.
They also pose the problem §\ref{sec:optimization_lp} solves: the critical-path duration is the boundary pairing $\langle d,\ \partial\gamma\rangle = d(t) - d(s)$, and the precedence constraints the coboundary bound $\delta d \ge c$, so the dual program is
\begin{equation}
    \text{minimise}\ \langle d,\ \partial\gamma\rangle \quad\text{s.t.}\quad \delta d \ge c .
\end{equation}
Section~\ref{sec:optimization_lp} is that program's solver in the tropical semiring, its closure the integral that inverts $\delta$, accumulating cost along paths to recover $d$ (a fold, not a reified operator).
So these operators are not adjacent to the exhibit; they are what it is written in.

\subsection{Dual numbers, for the sensitivity exhibit}
\label{sec:dual_numbers_quotient}

The one carrier worth a second look, because §\ref{sec:optimization_lp} differentiates with it, is the dual numbers $\mathbb{D}$, mathematically the quotient ring $F[\varepsilon]/(\varepsilon^2)$ over a coefficient carrier $F$ (classically $\mathbb{R}$; the sensitivity exhibit of §\ref{sec:optimization_lp} takes $F$ the saturating-unsigned cost carrier, the companion matrix listing takes $F = \mathbb{Q}$).
...
% the epic--monic factorization gates (IsSingleton, mono_retract), and the
% :relations / :functions partition surfaces.  Kept minimal for now to pin the
...
\vspace{0.3em}
\begin{flushleft}
{\footnotesize \textsuperscript{$\dagger$} Operator-surface certification only (\cppinline{HasRingOperators} / \cppinline{HasFieldOperators}); the three honest outcomes of the vector-space lift (clean, withheld because false, blocked by policy) are laid out in §\ref{sec:certificate_ladder}, Table~\ref{tab:lift-spectrum}.  \cppinline{Dual} carries ring operators, but $\varepsilon^2 = 0$ leaves it without a strict \cppinline{IsRing} witness, so it sits with the modules.}
\end{flushleft}
\end{table}
...
The two rules of §\ref{sec:hsp_varieties} are shown in full here.
Rule 2, propagation, is a small engine: a registration point, the concept that reads it, and one forwarding specialisation per species trait (Listing~\ref{lst:rule2-engine}), given for the H (quotient) family, with P (product) and S (subalgebra) identical up to the base name.

% Source excerpt from dedekind.algebra:quotient.
\begin{listing}[ht]
\caption{Rule 2, engine core.  The H (quotient) family; P (product) and S (subalgebra) mirror it.  Registering a base is one line, and the forwarding specialisation reads the base's own trait, so no law is re-proven on the derived carrier.}
\label{lst:rule2-engine}
\begin{cpp}
template <typename Q> struct quotient_algebra_base {};      // registration point
template <typename Q>
concept IsQuotientAlgebra = requires { typename quotient_algebra_base<Q>::type; };

// The derived carrier's trait forwards to its base's trait.
template <typename Q> requires IsQuotientAlgebra<Q>
inline constexpr bool is_associative_v<Q, std::plus<Q>> =
    is_associative_v<quotient_algebra_base_t<Q>, std::plus<quotient_algebra_base_t<Q>>>;
\end{cpp}
\end{listing}

Registering \cppinline{Vec2V} is the single line \cppinline{template <typename T> struct product_algebra_base<Vec2V<T>> { using type = T; };}, after which every species trait \cppinline{Vec2V<T>} carries forwards to \cppinline{T}; that is why \cppinline{IsModule} holds for the whole family and Rule 1 upgrades it to \cppinline{IsVectorSpace} over any field.
