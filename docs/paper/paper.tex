...
\begin{itemize}
    ...
    \begin{enumerate}
        ...
        \item \textit{Smallness:} both collections are \emph{sets} rather than proper classes --- here \emph{by construction}, since a single translation unit expresses only finitely many types and lambdas. (Requiring only the hom-collections to be sets gives the weaker notion of a \emph{locally small} category.)
    \end{enumerate}
    \item \textbf{C++ Reification:} The concept \texttt{IsSmallCategory} gates the structure. Per $\mathbf{Jlt}$~\ref{sec:jlt_posture}, objects are \texttt{std::regular} carriers. Morphisms are values of a single arrow type satisfying \texttt{IsArrow}, with \texttt{::Domain} and \texttt{::Codomain} typedefs and a callable \texttt{f(x)} from the domain into the codomain. Composition is the overloaded \texttt{operator>>}, with the identity laws \texttt{id >> g == g} and \texttt{f >> id == f} discharged as overloads; identity arrows come from an \texttt{id\_c(x)} factory.
    \item \textbf{Explanatory Note:} Instead of data-hiding containers, a ``set'' at this entry rung is simply a well-behaved C++ type acting as a domain, and a ``function'' is a side-effect-free arrow mapping inputs to outputs. In its single-object mode --- the carrier type taken as the sole object and its arrows as endomorphisms --- composition makes the category a \textbf{monoid} (a one-object category); these are exactly the endomorphism loops drawn on each carrier hub of Figure~\ref{fig:hub-spoke-bnz}.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{C++ Reification:} The exponential $B^A$ is the \emph{internal hom} --- required to be an \texttt{IsArrow} $A \to B$, reusing A.1 --- a demand \texttt{IsCartesianClosed} makes through the concept \texttt{IsArrowExponential}. Crucially the carrier is a \emph{defaulted, constrained parameter}, not a fixed type: the library ships \texttt{Morphism} as the default, so no concrete type enters the concept, only the default slot --- where \texttt{std::function} sits as the erased callable impl (C++ offers no nameable capturing callable, and \texttt{std::function} is not \texttt{constexpr}-usable). The adjunction arrows \texttt{curry} / \texttt{uncurry} / \texttt{eval} are library-provided.
    \item \textbf{Explanatory Note:} This licenses function \textit{Currying} at compile time via the \texttt{constexpr} lambdas. It guarantees that functions are first-class citizens that can be passed, stored, and transformed by other functions without dropping out of the type-checking engine.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Explanatory Note:} An \textit{intensional} set is defined by a rule, not an array. Instead of searching a block of RAM to check if $x \in S$, the compiler executes the predicate function $\chi(x)$. If $\chi(x)$ evaluates to true, the element belongs to the subobject.
\end{itemize}
