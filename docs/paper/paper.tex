...
\cppinline{Double} is the appetizer: linear, injective, its retract the obvious halving.
The main course keeps the machinery and drops the linearity.
Take $f(x) = x^2$.
On the nonnegative branch $x^2$ is strictly monotone, so it is monic, and its retract is the integer square root: $y \mapsto {+}\sqrt{y}$ when $y$ is a perfect square, \cppinline{nullopt} otherwise (Listing~\ref{lst:functional_image_square}).
Monotonicity is the certificate that this retract is \emph{complete}: $\mathrm{isqrt}$ returns \emph{the} unique nonnegative preimage, so no square in the image is missed, while the soundness check $r^2 = y$ is free.
Membership $y \in x^2(S)$ is then the same local lookup as the appetizer, now nonlinear: take $\sqrt{y}$, test that it squares back and lands in $S$, and never search the domain.
This is also what discharges the $\arg\max$ of §\ref{sec:relations_functions_blueprint} over an infinite domain: an optimiser's extreme value and its fibre are the same $\exists x.\,f(x){=}y$, and for $x^2$ the fibre of any $y$ is $\{\pm\,\mathrm{isqrt}(y)\}$, computed by the retract rather than searched for.

\begin{listing}[htbp]
\caption{The nonlinear main course: $x^2$ with an \cppinline{isqrt} retract.  On the nonnegative branch $x^2$ is monic, and image membership $y \in x^2(S)$ is decided by the closed-form root ($25 \mapsto 5$, a perfect square in \cppinline{positive}; $7 \mapsto$ \cppinline{nullopt}, not a square; $0 \mapsto 0 \notin$ \cppinline{positive}), never a search of the domain (abridged from the checked-in test).}
\label{lst:functional_image_square}
\begin{cpp}
constexpr int isqrt(int y) {                     // principal root, y ≥ 0
  int r = 0; while ((r+1)*(r+1) <= y) ++r; return r; }
struct Square { using Domain = int; using Codomain = int;
  constexpr int operator()(int x) const { return x*x; } };
constexpr auto retract(Square) {
  return [](int y) -> std::optional<int> { int r = isqrt(y);
    return (y >= 0 && r*r == y) ? std::optional{r} : std::nullopt; }; }

constexpr auto img = image(Square{}, positive);
\end{cpp}
\end{listing}

On all of $\mathbb{Z}$ the same $x^2$ is two-to-one: the fibre of $y$ is $\{{+}\sqrt{y}, {-}\sqrt{y}\}$, and no single-valued retract can name the preimage.
That is precisely the non-injective case the factorization is built for.
