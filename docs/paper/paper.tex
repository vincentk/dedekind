...
\cppinline{Set{...}} is a variadic constructor; the combinators \cppinline{in} and \cppinline{bound} construct a scout variable and a half-space predicate.
The scout variable appears on both sides of the bar: introduced on the left, referred back to on the right, so the predicate reads as the constraint $x > 5$.
\cppinline{operator()} is shorthand for \cppinline{a.contains(e)}.}
\label{lst:pruning}
\noindent
