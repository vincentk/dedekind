...
\begin{listing}[htbp]
\caption{The image of an intensional set under an arrow, and the honest indecision it carries.  \cppinline{image(succ, {n>5})} realises \cppinline{IsImageOf} (it is a subobject of the codomain), yet its membership is \cppinline{TernaryLogic::Unknown}: the existential $\exists x.\,x>5 \wedge y = \mathrm{succ}(x)$ is undecidable on the transfinite carrier without a factorization.  The Rice boundary, made visible (abridged from the checked-in showcase).}
\label{lst:functional_image_factorization}
\begin{cpp}
constexpr auto succ = arrow<ℕ, ℕ>(
  [](ℕ m){ return m + finite_cardinality(1); });

constexpr auto img =
  image(succ, Set{in<ℕ> | in<ℕ> > bound<5>});
static_assert(IsImageOf<decltype(img), decltype(succ)>);
