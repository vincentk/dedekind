...
\end{tabular}
\caption{
Operator-mapping table: each column is one construction spelled in four languages, the rows ascending from the Boolean lattice $\mathbb{B}$ through the powerset $\mathcal{P}(A)$ and the abstract Cartesian-closed category $\mathbf{CCC}$ to the reification $\mathbf{Trsk} \subset \mathbf{Cpp}$ (Section~\ref{sec:ddk_etcs}, Eqn~\ref{eqn:trsk}).
The lattice columns are the $\mathbf{CCC}$'s universal constructions realised concretely: the meet \emph{is} the categorical product, the implication the exponential.
The \emph{member} column is the odd one out: membership $x \in S$ is not a lattice operation but the characteristic morphism $\chi : A \to \Omega$, the relation the whole lattice is built on.
It is that one relation at four universes: $\in$ set membership over a general $A$, $\chi$ the classifier, \cppinline{s(x)} the set applied to a point, and, since $\mathbb{B} = \mathcal{P}(1)$ is the powerset of a single point, $\models$ satisfaction (membership of the sole point is just whether the proposition holds).
The set lattice \emph{is} the Boolean lattice lifted pointwise ($\mathcal{P}(A) \cong \mathbb{B}^A$), and the collapse in Listing~\ref{lst:pruning} is that lift at work in the form of type-checked \Cpp{23}.
}
...
\begin{listing}[htbp]
\caption{$\mathbf{Trsk}$ is strictly more expressive than \cppinline{std::set}, the extensional container it generalises, and the two readings compose.
\textbf{Left, top} ($\iota$, total): every container \emph{is} a $\mathbf{Trsk}$ set, its \cppinline{contains} the characteristic morphism, so \cppinline{ambient_set(from_std(·))} lifts any \cppinline{std::set} to an \cppinline{IsSet} object.
\textbf{Left, bottom} ($\mu$, partial): \cppinline{materialise} enumerates the universe and keeps the $\chi$-matches, defined only for a finite, enumerable universe with a decidable predicate, so the naturals do not materialise.
\textbf{Right} (composition): an extensional set bounds an intensional halfspace, making the universe finite, so their intersection \emph{always} materialises ($\{2,3,5\} \cap \{x>3\} = \{5\}$, $\cap\, \{x>5\} = \varnothing$).
The price of staying purely intensional is discharging membership by proof, not by enumeration.}
...
That is what rescues $\forall$ over an infinite carrier: \cppinline{isDivisibleBy3} is periodic, and it factors through a finite codomain, the modular arithmetic $\mathbb{Z}/3\mathbb{Z}$ (Section~\ref{sec:functions_morphism_factorization}), so the collapse is settled on three residues rather than the countably many naturals, and both $\exists$ and $\forall$ return in finite time (Listing~\ref{lst:quantifiers_def}, bottom row).
...
\begin{listing}[htbp]
\caption{The quantifiers \emph{are} set operations (Eqn~\ref{eqn:quantifiers}), decided \emph{by type} through a finite quotient.
\textbf{Top:} the generic quantifiers over an \cppinline{IsSet} domain \cppinline{S} and an \cppinline{IsPredicate} query \cppinline{P}, as the two lattice bounds: (A) \cppinline{exists} tests \cppinline{set(S,P)} against $\varnothing$, (B) \cppinline{forall} against the input set $S$; each recovers its partner by complementing the predicate, \cppinline{!P}.
\textbf{Bottom:} over $\mathbb{B}$ (a finite carrier) the structural predicate \cppinline{Singleton<true>} materialises over the two truth values; over $\mathbb{N}$, \cppinline{isDivisibleBy3} factors through $\mathbb{Z}/3\mathbb{Z} = $ \cppinline{Modular<3>}, so $\exists$ and $\forall$ exhaust its three residues.  Neither enumerates $\mathbb{N}$.}
