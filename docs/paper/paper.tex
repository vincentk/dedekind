...
\cppinline{MaxPlus<T>} aliases the longest-path direction, with $\infty$ the saturating sentinel the extended cardinal's $\aleph_0$ already plays.\footnote{The finite carrier is \cppinline{unsigned}: its $+$ is a \emph{periodic} magma (defined wraparound, not signed-overflow UB), so the $\otimes$-totality the \cppinline{IsSemiring} gate requires is certified \emph{honestly} rather than asserted over a hazard; signed \cppinline{long long} is correctly not certifiable.  This is the same discipline the library applies to \textsc{ieee} and signed integers: an operation is certified total only where the hardware genuinely closes it.}
...
int64_t critical_between(std::size_t source, std::size_t sink) {
  if (source >= Cap || sink >= Cap) return -1;  // bound is compile-time; folds
  return semiring_closure<MaxPlus<unsigned long long>, Cap>(
