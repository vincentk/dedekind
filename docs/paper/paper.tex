...
The concrete carriers\footnote{We use ``carrier'' for the underlying set $A$ of a $\Sigma$-algebra in the strict universal-algebra sense (Burris--Sankappanavar~\cite{burris1981universalalgebra}), distinct from the algebra $(A, \Sigma_A)$ itself.  In Liskov / OO terms: the textbook posture is HAS-A (an algebra is the pair $(A, \Sigma_A)$, carrying its underlying set as a separate component), and the project converges on this spelling --- the canonical $(A, F)$ form is reified as \cppinline{IsAlgebra<T, Ops...>} in \texttt{algebra:universal} (and the strict categorical variants \cppinline{IsRing<T, Add, Mult>}, \cppinline{IsField<T, Add, Mult>}, \dots in \texttt{category:total}).  Where the prose says ``the carrier $\mathrm{T}$'' it is shorthand for the pair $(\mathrm{T}, \Sigma_\mathrm{T})$ inhabited under those concepts.  The named carrier types of the engineering surface (\cppinline{Rational<I>}, \cppinline{Complex<F>}, \dots) bake their operator overloads into the type as ergonomic sugar accommodating the C++ legacy of per-type \cppinline{operator+} / \cppinline{operator*} resolution; the strict $(A, \Sigma_A)$ pair view is the one the concepts and trait specialisations witness.  Section~\ref{sec:alg-sigma} makes the layering precise.} a programming language offers violate those laws conditionally, sometimes silently:
...
We chose elements from C++ guided by two criteria: alignment with the universal-algebra fragment $\mathbf{Alg}(\Sigma)$ identified in §\ref{sec:alg-sigma}, and the desire to integrate functionality the standard library already ships --- in particular \cppinline{std::regular} value-types and the primitive carriers (\cppinline{int}, \cppinline{double}, \cppinline{bool}, \dots) --- without wrapping them in \cppinline{struct} boilerplate or paying a runtime cost.
...
The first two tiers are mechanically decidable on the disciplined fragment the library polices (template instantiation is Turing-complete in general --- Veldhuizen~\cite{veldhuizen2003templates} --- and decidability is recovered specifically by the bounded-instantiation-depth and no-template-template-recursion rules collected in Appendix~\ref{app:jlt-systemf}); the third is the manual-certification path the C++ standard distinguishes in [concepts.lib] as \emph{satisfying} (syntactic) versus \emph{modelling} (semantic) a concept~\cite{cpp23standard}, and that C++ Core Guideline T.20~\cite{cppcoreguidelines} prescribes for the semantic-requirements clause the spec deliberately refuses to enforce mechanically (the standard's \emph{ill-formed, no diagnostic required} clause being its bite --- the C++ analogue of \emph{ex falso quodlibet}; the C++0x \texttt{axiom} keyword was considered for in-language semantic certification and ultimately withdrawn~\cite{isocpp_concepts_history}, leaving the manual-certification pattern as the path the committee left open).\footnote{The C++ standard's \emph{syntactic} / \emph{semantic} split is fundamentally \textbf{decidable vs.\ undecidable} --- what the spec commits to enforcing mechanically vs.\ what it deliberately refuses to enforce --- and does not coincide with the static-vs-dynamic semantics split familiar from PL theory~\cite{pierce2002tapl}.  C++'s \emph{syntactic} bucket subsumes that tradition's \emph{static semantics} (type checking, concept resolution, auto-derived theorems via \cppinline{static_assert}) because those are all things the compiler verifies mechanically; the C++ \emph{semantic} bucket is roughly what resists mechanical decision in the spec's chosen target structure.  The committee's vocabulary optimises for responsibility allocation (compiler vs.\ user) rather than for type-theoretic precision; the mismatch is real but learnable.}
The trait-registry mechanics --- which axioms propagate under which constructors, how per-carrier specialisations are organised, how the \emph{Honest Rejection} policy maps to varieties, and how the algebra-side machinery composes with the C++-side concept layer --- live in §\ref{sec:pruning} (\emph{Algebraic Lattice}), where the page budget for the compare-and-contrast actually fits.

A welcome side effect: the disciplined subset that aligns with $\mathbf{Alg}(\Sigma)$ also turns out to be a faithful image of System~F (Pierce~\cite{pierce2002tapl} ch.~23, 29) and of the simply-typed $\lambda$-calculus under Lambek--Scott~\cite{lambek1988introduction}, which lets the same surface read in either tradition.
This was not the design goal: the paper began as an engineering exercise on $\mathbf{Alg}(\Sigma)$, and the System-F reading announced itself afterwards.
The triangulation across System~F, the Juliet Posture, and Bourbaki structuralism~\cite{marquis2022structuralist} is reported as an observation rather than a guarantee --- some discipline is required of the engineer for the reading to hold; the precise admissibility rules and a row-by-row mapping across the three vocabularies are deferred to Appendix~\ref{app:jlt-systemf}.

Call this disciplined subset $\mathbf{Jlt} \subset \mathbf{Cpp}$ (mnemonically: \emph{Juliet}, after the \emph{rose by any other name} reading of structural-over-nominal typing); in $\mathbf{Jlt}$ the objects are \cppinline{std::regular} types and the values they classify, and the morphisms are the arrows the language admits between them.

\subsection{The Structuralist Intersection: $\mathbf{Ddk} = \mathbf{Jlt} \cap \mathbf{Alg}(-)$}
\label{sec:cpp-stlc-mapping}

The \texttt{dedekind} types and combinators are the inhabitants of $\mathbf{Jlt} \cap \mathbf{Alg}(-)$: C++23 types in the disciplined fragment $\mathbf{Jlt}$ (§\ref{sec:jlt}) that simultaneously carry universal-algebra structure $\mathbf{Alg}(\Sigma)$ (§\ref{sec:alg-sigma}) for some signature $\Sigma$ the carrier supplies operators for; the dash notation $\mathbf{Alg}(-)$ is the family $\bigcup_\Sigma \mathbf{Alg}(\Sigma)$ over the signatures the library names (per §\ref{sec:alg-sigma}, $\Sigma$ stays unbound here, and the per-shape \cppinline{Has*Operators} concepts pin it later).
The intersection is small and that is the point.

\paragraph{Honest Rejection: the headline policy.}
$\mathbf{Ddk}$ ships with two carriers certified safe by default --- $\mathbb{B}$ as the Galois field $\mathbb{F}_2$ (Boolean negation, conjunction, exclusive-or; finite cardinality, no rounding, no overflow --- laws hold trivially) and $\mathbb{Q}$ as \cppinline{Rational<I>} for any wide-enough integer carrier $I$ (exact arithmetic; associativity, distributivity, and the field axioms hold by construction) --- and refuses everything else at the gate.
\cppinline{int} satisfies \cppinline{HasRingOperators} (the four arithmetic operators close on \cppinline{int}), but signed-overflow undefined behaviour defeats associativity, so the strict \cppinline{IsRing} concept refuses it.
IEEE \cppinline{double} satisfies \cppinline{HasFieldOperators}, but rounding non-associativity defeats the field axioms, so \cppinline{IsField} refuses it.
Structural typing (§\ref{sec:jlt}) sees the operator surface mechanically; the laws are a separate axis the C++ standard library, principled in its silence~\cite{cpp23standard}, refuses to commit to.

% Sources: src/main/modules/dedekind/algebra/galois.cppm (line 335);
%          src/main/modules/dedekind/category/species.cppm (line 1068).
\begin{listing}[ht]
\caption{Honest Rejection in code: $\mathbb{B} = \mathbb{F}_2$ ships safe; \cppinline{int} is refused at the gate.  All four static\_asserts compile in $\mathbf{Ddk}$ today.}
...
A library user who has audited their inputs to a regime where the law actually holds --- integer arithmetic on a domain where overflow cannot occur, IEEE \cppinline{double} accepted with associativity-by-fiat for a specific application, a custom carrier whose modular wrap-around the user proves consistent with the surrounding logic --- pushes the button: a per-carrier trait specialisation registers \cppinline{is_associative_v<T, Op> = true} (or \cppinline{is_commutative_v}, \cppinline{is_invertible_v}, \dots), and the downstream concepts (\cppinline{IsRing}, \cppinline{IsField}) discharge.
The opt-in is the user's pinky-swear in their part of the multiverse: they own the proof that the swear is honest, and IFNDR is the standard's bite if the swear was a lie.
This is the operational discharge of the satisfies/models distinction §\ref{sec:jlt} introduces: closure on the operator surface is what \cppinline{Has*Operators} records mechanically; lawfulness is what the trait registry records intentionally.
The mechanics --- which axioms propagate under which constructors, how per-carrier specialisations are organised, how the \emph{Honest Rejection} default composes with the universal-algebra reading $\mathbf{Mon} \subset \mathbf{Var}(\Sigma) \subset \mathbf{Alg}(\Sigma) \subset \mathbf{Alg}(-) \subset \mathbf{Cat}$~\cite{burris1981universalalgebra} (§II.10--II.11; the paper's $\mathbf{Mon}$-side framing is in §\ref{sec:alg-sigma}) --- live in §\ref{sec:pruning} (\emph{Algebraic Lattice}).

\paragraph{Two consequences worth naming.}
First, structural typing makes membership definable \emph{after the fact}: a pre-existing standard-library type --- a \cppinline{std::pair}, an \cppinline{std::ranges::view}, a \cppinline{std::optional} --- becomes an inhabitant of $\mathbf{Ddk}$ the moment a \cppinline{concept} accepting it is written, with no modification to the carrier.
Nominal-typing alternatives (declared lineage, base classes, virtual interfaces) would require carriers shipped in the C++ standard a decade ago to opt in upstream --- impossible.
Second, the disciplined fragment under which $\mathbf{Ddk}$ inhabits also reads as a faithful image of the simply-typed $\lambda$-calculus and System~F under Lambek--Scott~\cite{lambek1988introduction} and Pierce~\cite{pierce2002tapl} --- a verification artefact rather than a flow-relevant claim, but worth naming for the reader who wants to chase the bridge: the row-by-row C++23 $\to$ STLC mapping is in Appendix~\ref{app:ddk-rosetta}, alongside the operator-shape rosetta and a worked Functor exhibit.
...
The intersection $\mathbf{Jlt} \cap \mathbf{Alg}(-)$ becomes concrete on a single non-trivial example.
