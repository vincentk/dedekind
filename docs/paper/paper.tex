...
Reading $\le$ through a map $f\colon \mathrm{dom}\to C$ turns $\max$ into $\arg\max$: not the greatest \emph{value} but every \emph{argument} that attains it.
The definition is the same forward filter as Equation~\ref{eqn:extrema}, with the carrier's own order replaced by the order $f$ pulls back onto its domain, and it asks nothing of $f$ beyond a comparable codomain: no inverse, no injectivity.
\begin{equation}
\label{eqn:argmax}
\begin{aligned}
...
The objective $f$ is generic, any map into a comparable codomain; supply a point-free $f$ and the whole relation stays compiler-inspectable, but the filter asks only for its values, never its inverse.
Because the answer is a set, the (domain, codomain, constraint) triple hands back the degenerate cases as a \emph{cardinality} rather than as special code: the fibre $f^{-1}(\max f(\mathrm{dom}))$ is empty when there is no maximum, a singleton exactly when the optimiser is unique (so $\arg\max$ is a function), and larger under ties (so $\arg\max$ is a proper relation), the same fibre bookkeeping Table~\ref{tab:relation-quantifier} reads off a projection (Listing~\ref{lst:argmax_def}, bottom).

\begin{listing}[htbp]
\caption{$\arg\max$ / $\arg\min$ as Equation~\ref{eqn:extrema}'s extremum with $\le$ pulled back through $f$.
\textbf{Top}: the forward filter, a \cppinline{forall} over $f(a)\le f(x)$; no inverse of $f$ enters, and the result is again an \cppinline{IsSet}.
\textbf{Bottom}: over $\{0,\dots,6\}$ the concave \cppinline{cap} has the unique peak $x=3$ (a singleton fibre, so $\arg\max$ is a function), while \cppinline{parity} ties across every odd argument (fibre $\{1,3,5\}$, a proper relation); membership \cppinline{argmax(D,f)(x)} reads the fibre.}
\label{lst:argmax_def}
\noindent
\begin{minipage}[t]{0.99\linewidth}
\begin{cpp}
template <IsEnumerableSet S, class F>
constexpr auto argmax(S dom, F f) {     // { x ∈ dom | ∀ a ∈ dom. f(a) ≤ f(x) }
  return Set{dom | forall(dom, f(a) <= f(x))};
}
\end{cpp}
\end{minipage}
...
\begin{minipage}[t]{0.99\linewidth}
\begin{cpp}
constexpr int cap(int x)    { return x*(6-x); }  // concave, unique peak at 3
constexpr int parity(int x) { return x % 2;    }  // ties across the odds
constexpr auto D = iota(0, 7);                     // {0,…,6}
static_assert(argmax(D, cap)(3));                 // {3}: unique optimiser (a fn)
static_assert(argmax(D, parity)(1) && argmax(D, parity)(5));  // {1,3,5}: a tie
\end{cpp}
\end{minipage}
\end{listing}

The filter types in for any domain and any $f$; it \emph{decides} only where §\ref{sec:ddk_comprehension}'s certificates reach.
A finite or finite-quotient domain is walked, but an unbounded one leaves both the codomain maximum and its winning fibre as existentials over $f$, decidable only through $f$'s own structure.
That closed-form certificate, the retract, is the next slice.

\subsubsection{Functions and Morphism Factorization Gates}
\label{sec:functions_morphism_factorization}

Image membership, $y \in \mathrm{image}(f)$, is the same existential once more, now on the domain side: $\exists x.\, f(x) = y$.
...
\begin{listing}[htbp]
\caption{A function \emph{is} its graph.  \cppinline{f = ℤ*ℤ | π1+fix(3_c)==π2} is the point-free relation $\{(x, x{+}3)\}$, a bona fide function: functional and entire, the two bounds of Table~\ref{tab:relation-quantifier}'s $\pi_A$ column.  Its \cppinline{inverse} is the graph read backwards, $x{-}3$, applied as a function; \cppinline{image} is the range, the whole line for a total (surjective) function, and a domain bound pushes forward through the graph, $\{x\le5\}$ to $\{y\le8\}$, again a halfspace of the same shape (abridged from the checked-in witnesses).}
...
static_assert(inverse(f)(10) == 7);                // the graph read backwards, x−3
