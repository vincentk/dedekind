...
The body's \cppinline{IsFunctor} concept (Listing~\ref{lst:isfunctor}, §\ref{sec:cpp-stlc-mapping}) encodes the same content structurally; the Curry--Howard discharge that \texttt{maybe\_functor} inhabits the concept is the \cppinline{static_assert} in the worked-instance listing immediately following Listing~\ref{lst:isfunctor}.
Together, Listing~\ref{lst:isfunctor} and that worked-instance listing land the System-F-recovery rosetta (Table~\ref{tab:structuralist-bridge}) at one concrete site: the \cppinline{template <typename F>} header is the \emph{Constrained Templates} row ($\equiv \forall\alpha.\dots$); the \cppinline{requires}-clause is the \emph{Concept-gating} row discharging parametricity; and the worked-instance listing's \cppinline{static_assert} is the Curry--Howard term.
