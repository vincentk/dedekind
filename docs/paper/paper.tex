...
Subobject Classifier ($\Omega$) & \texttt{enum class Omega} & \texttt{LogicalSpecies} \\
\addlinespace
...
While the module graph serves as an external meta-logical constraint ensuring build-time termination, we later introduce a pluggable \textit{Subobject Classifier} ($\Omega$) in the \ref{sec:algebraic_logic} section which internalizes the notion of undecidability within the C++ type system itself.
...
These partitions represent the maturation of mathematical species from raw mappings (\texttt{:morphisms}) to structured universes (\texttt{:logic}).
