...
Cartesian-closed categories are not ETCS; the library's \texttt{IsSet}
concept is intended---aspirationally---to entail the canonical CCC
witness on its ambient species, so that any ETCS-set carrier (for
instance the finite field $\mathbb{F}_{64}$) inherits the CCC guarantee
structurally rather than by per-carrier opt-in. What this alignment buys
is the Lambek--Scott
correspondence~\cite{lambek1988introduction}: a Cartesian-closed
category \emph{is} a simply-typed lambda calculus. Realising the
ETCS~$\subset$~CCC subsumption at the concept level therefore promotes
\texttt{dedekind}'s set-builder DSL into a compile-time typed lambda
calculus---function spaces, currying, evaluation, and product/coproduct
introduction all witnessed by the same type-system machinery that
discharges the algebraic laws of Section~\ref{sec:carrier-rosetta}.
