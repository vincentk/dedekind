...
\begin{table}[htbp]
\centering
\caption{\textbf{The algebraic lattice and its carriers.} Each structure of the branching poset (§\ref{sec:algebraic_lattice_carriers}), the concept that certifies it, and the carriers that realise it on the built-in (\cppinline{std} / IEEE) side and the exact-library side. \cppinline{I} abbreviates \cppinline{SignedExtensionalCardinal<>}.}
...
Comm.\ monoid & \cppinline{IsMonoid} & \cppinline{unsigned} under $+$ ($\mathbb{Z}/2^n$) & $\mathbb{N}$: \cppinline{Cardinality} \\
Abelian group & \cppinline{IsAbelianGroup} & \cppinline{unsigned} under $+$ (cyclic) & $\mathbb{Z}$: \cppinline{SignedExtensionalCardinal<N>} \\
Ring & \cppinline{IsRing} & \cppinline{unsigned} under $(+,*)$; signed \cppinline{int} \emph{refused} (overflow UB) & $\mathbb{Z}$; $\mathbb{D}$: \cppinline{Dual<Rational<I>>}; \cppinline{RigPolynomial} \\
Field & \cppinline{IsField} & \cppinline{bool} under $(\oplus,\land)$ ($\mathbb{F}_2$) & $\mathbb{Q}$: \cppinline{Rational<I>}; $\mathbb{C}$: \cppinline{Complex}; $\mathbb{R}$: \cppinline{ExactReal} \\
Galois field & \cppinline{IsGaloisField} & \cppinline{bool} ($\mathbb{F}_2$, order 2) & \cppinline{𝔽64} (order 64) \\
Module / v.\ space & \cppinline{IsModule} / \cppinline{IsVectorSpace} & \cppinline{uint64_t} under $\oplus$, \cppinline{bool} scalars ($\mathbb{F}_2^{64}$) & \cppinline{Vec2V} \\
\bottomrule
\end{tabular}
\end{table}
