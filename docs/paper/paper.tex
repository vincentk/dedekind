...
The answer is a set, so the degenerate cases need no special casing: an empty carrier gives $\varnothing$, a total order gives a singleton whose sole element is the bound, and ties give a larger set (Listing~\ref{lst:extrema_def}, bottom).
...
Because $R$ is only a \emph{preorder} (reflexive and transitive, not antisymmetric), ties and incomparable optima are held by the relation rather than collapsed, so the partial case is the Pareto front; this is argmax as the set-valued correspondence of Berge's maximum theorem~\cite{berge1963topological}.
