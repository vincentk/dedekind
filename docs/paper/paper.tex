...
Crucially, the library does not yet bind these axioms to specific hardware operators like \texttt{std::less\_equal}.
This abstraction allows the \texttt{:logic} partition (\ref{dedekind.category::logic}) to address the "Reflexivity Failure" observed in IEEE 754 floating-point species, where the singularity \texttt{NaN <= NaN} evaluates to \texttt{false}. By isolating these axioms in Level 0, we ensure that the subsequent synthesis of Set Theory in Level 4 inherits a verified, algebraic definition of "part-of" relationships.
...
Before a ``Body'' (Set) or a ``Path'' (Sequence) can be reified, the \texttt{dedekind} library establishes the ``Rules of Presence'' through the \textbf{Subobject Classifier} $\Omega$. Following the algebraic logic of Rasiowa~\cite{rasiowa1974algebraic}, we treat every logical system as a specific type of abstract algebra where truth-values act as the objects of a \textbf{Topos}. By reifying these systems via the \texttt{LogicalSpecies} concept, we prevent the ``leaky abstractions'' inherent in machine-level \texttt{bool} types—such as implicit integral promotion—while allowing for pluggable logical universes.
...
\subsection{The Registry of Formal Correspondences}
