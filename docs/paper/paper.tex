...
\item \textbf{A Zero-Overhead Parametric Solver (specified):} A synthetic source-and-IR specification of a network linear program that evaluates entirely at compile time, collapsing into literal constant returns within the emitted LLVM IR, built on the set-level type-directed collapse demonstrated in §\ref{sec:ddk_comprehension}.
...
Section~\ref{sec:optimization_lp} specifies the chain-network dual linear program as the compile-time centrepiece; Section~\ref{sec:evaluation_benchmarks} models its cost, and Section~\ref{sec:discussion} sets out the limitations and follow-up.
...
With the carrier tower of §\ref{sec:algebraic_lattice_carriers} established, linear algebra over those carriers is the next layer.
...
// Define a finite, enumerable coordinate index set as a library extensional carrier
constexpr ExtensionalSet BaseDomain{1u, 2u, 3u};
