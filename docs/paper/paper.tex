...
The step budget also bounds where the boundary can fall.
An exact carrier admits constant evaluation but is step-heavy, so a deep enough fold exhausts the translator's constant-evaluation budget; forced past it a fold is ill-formed rather than merely slow, so such a term is left to run time.
Because the carrier is exact that costs only the folding and never the answer: the run-time value is the very constant a fold would have produced, computed a moment later rather than during translation.
