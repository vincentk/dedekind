...
\begin{tikzpicture}[scale=1.05, every node/.style={font=\small},
  ...
  \draw (ph)--(psh)--(sh);
  % axes (the three edges from the origin)
...
\end{tikzpicture}
\caption{The construction cube: the three Birkhoff axes H (quotient), S (subalgebra), P (product) span a space of carriers, a lattice reading of Table~\ref{tab:hsp-rosetta} in the spirit of the lambda cube.  The origin is a base magma ($\mathbb{B}$); each axis applies one construction; composites sit at the corners, up to a matrix of dual numbers $M_n(\mathbb{D})$ at the far vertex.  Labels are representative carriers, not a functor on a fixed base.}
\label{fig:hsp-cube}
\end{figure}
