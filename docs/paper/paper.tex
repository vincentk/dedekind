...
The \texttt{dedekind} types and combinators are the inhabitants of $\mathbf{Jlt} \cap \mathbf{Alg}(-)$: C++23 types and arrows that simultaneously sit in the disciplined syntactic fragment $\mathbf{Jlt}$ and carry universal-algebra structure from $\mathbf{Alg}(\Sigma)$ for some signature $\Sigma$ that the carrier supplies operators for.
...
\paragraph{Why $\mathbf{Alg}(\Sigma)$ rather than $\mathbf{Mon}$.}
On the CT side we work in the universal-algebra category $\mathbf{Alg}(\Sigma)$ (Burris--Sankappanavar~\cite{burris1981universalalgebra} §II.10), introduced in §\ref{sec:alg-sigma} above.
The CS-side reading is the isomorphism $\mathbf{Alg}(\Sigma) \cong F_\Sigma\text{-}\mathbf{Alg}$ to the category of $F_\Sigma$-algebras for the polynomial endofunctor $F_\Sigma$ generated by the signature~\cite{pierce1991basic} (§2.2); the same fragment is therefore reachable from either tradition.
The relationship to the textbook $\mathbf{Mon}$ (the category of monoids; equivalently, one-object categories under Mac Lane~\cite{maclane1971categories} §I.1's bijection) is generalisation along two axes: \textbf{(a)~signature}: $\mathbf{Alg}(\Sigma)$ admits operations of arbitrary signature on a shared carrier (e.g.\ both $+$ and $\cdot$ on a ring), where $\mathbf{Mon}$ carries exactly one binary operation; and \textbf{(b)~laws}: $\mathbf{Alg}(\Sigma)$ does not require the monoid axioms (associativity, two-sided identity), leaving lawfulness to per-species trait specialisations (\cppinline{IsRing}, \cppinline{IsField}, \dots).
Concretely, the chain reads $\mathbf{Mon} \subset \mathbf{Var}(\Sigma_{\text{mon}}) \subset \mathbf{Alg}(\Sigma_{\text{mon}}) \subset \mathbf{Alg}(-) \subset \mathbf{Cat}$, where $\mathbf{Var}(\Sigma) \subset \mathbf{Alg}(\Sigma)$ is the variety~\cite{burris1981universalalgebra} (§II.11) --- the equational sub-class of $\Sigma$-algebras that satisfy a given set of laws --- and $\mathbf{Alg}(-)$ denotes the family $\bigcup_\Sigma \mathbf{Alg}(\Sigma)$ over the signatures the library names; the project's \emph{Honest Rejection} policy maps cleanly onto this chain by admitting carriers in $\mathbf{Alg}(\Sigma)$ that fall outside $\mathbf{Var}(\Sigma)$ (the operator surface holds; some equation does not).

The reason for the generalisation is both engineering-pragmatic and categorical-aesthetic: $\mathbf{Alg}(-)$ is sized precisely to admit the carriers and combinators the C++ standard library already ships --- the primitive types (\cppinline{int}, \cppinline{bool}, \cppinline{double}, \dots), \cppinline{std::regular} value-types, \cppinline{std::pair} and \cppinline{std::tuple}, the \cppinline{std::ranges} families, and so on --- even when these carriers fail one or both of the monoid axioms (\cppinline{int} addition is non-associative under signed-overflow UB; IEEE-754 \cppinline{double} addition is non-associative under rounding; \cppinline{std::ranges::view} has no obvious neutral element): each such carrier sits in $\mathbf{Alg}(\Sigma)$ for the surface signature it supplies, but outside the variety $\mathbf{Var}(\Sigma)$ that the corresponding equations would carve out.
Working in $\mathbf{Alg}(-)$ is what lets the library lift these standard-library carriers into a categorical reading without first having to refactor or wrap them; the trait-specialisation registry (the carrier-side opt-in via \cppinline{is_associative_v} / \cppinline{is_commutative_v} / \cppinline{is_invertible_v} and friends, developed in §\ref{sec:pruning}) then refines individual carriers to stricter algebraic concepts (\cppinline{IsRing}, \cppinline{IsField}, \cppinline{IsTotallyOrdered}, \dots) --- in $\mathbf{Var}(\Sigma)$-vocabulary, registers the carrier as inhabiting the relevant equational sub-variety.
...
The textbook spelling $\mathbf{Alg}(-)$ makes both motivations visible at the symbol layer (the generalisation beyond $\mathbf{Mon}$ along the signature and law axes; the after-the-fact-membership opening that lets the library absorb existing types) rather than overload $\mathbf{Mon}$ with a meaning it does not carry in the standard textbook reading.

The remainder of the section develops the two restrictions in turn --- §\ref{sec:alg-sigma} for $\mathbf{Alg}(\Sigma)$, §\ref{sec:jlt} for $\mathbf{Jlt}$ --- and the subsequent pillars exhibit the intersection through worked examples.
