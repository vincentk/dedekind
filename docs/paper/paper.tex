...
A common implementation approach follows Zermelo-Frenkel set theory, where the discussion often starts from a set membership operation $\in$.
Software implementations such as \cppinline{std::set} then model sets as extensional memory containers where the membership test is carried out by checking if a given value can be found in the container. 
...
By contrast, the categorical comprehension view pioneered by Lawvere~\cite{lawvere1964etcs}, defines a set as an intensional rule mapped over an ambient species.
Here, a set $S$ is specified by a characteristic predicate $P : A \to \Omega$, where $A$ is the underlying type and $\Omega$ is a subobject classifier specifying the logic regime:
\begin{equation}
  S = \{x \in A \mid P(x)\}.
\end{equation}
