...
\paragraph{Infinite and infinitesimal data, natively.}
A second motivation runs through the project: handling
\emph{infinite} (unbounded sets, lazy streams, transfinite cardinals)
and \emph{infinitesimal} (dual numbers, formal series, Newton-quotient
differentials) data structures in everyday programming, without each
project reinventing its own ad-hoc encoding.  Three host-language
properties make such objects legible: \emph{lazy evaluation} (an
infinite predicate or a formal series can be carried as a typed
intension, never as a finite materialisation), \emph{strong typing}
(the distinction between a Dedekind cut and a \texttt{double} survives
all the way through composition), and a \emph{compositional surface}
(new structures arrive by application of named functors, not by
copy-and-paste).  C++23 supplies a workable subset of all three ---
\lstinline{constexpr} for staged evaluation, \lstinline{concept}s for
disciplined typing, named modules for compositional layering --- and
the library leans on each.  Figure~\ref{fig:carrier-lattice}'s
carrier lattice and the dual-number / tangent-bundle identification
(§\ref{sec:ad-elliott}) are the load-bearing exhibits.
...
qualifier ``poor man's'' is structural, not economic.  \texttt{clang++}
...
The body splits across three pillars: \emph{An Axiomatic Subset of C++23}, \emph{Sets and Sequences}, \emph{Algebraic Lattice}.

Section~\ref{sec:axiomatic} (\emph{An Axiomatic Subset of C++23}) motivates the choice of C++23 as the implementation substrate, introduces the \emph{Axiomatic Systems Programming} paradigm, and develops the intensional-first design rule (§\ref{sec:intensional}) including the tension between abstract algebraic numerics and IEEE~754 hardware floating-point and the carrier Rosetta from exact rationals to packed integers.
...
% ============================================================
\section{A Categorified Subset of C++23}
\label{sec:axiomatic}
% ============================================================
...
\centering
\scalebox{1.5}{%
\begin{tikzcd}[column sep=4em, row sep=4em, ampersand replacement=\&]
\mathbb{K}_3 \arrow[loop above, "\neg_{\mathbb{K}_3}"]
   \arrow[drr, "\mathfrak{s}"]
  \&[-1em] \&[-1em] \\[-0.5em]
\mathbb{B} \arrow[u, "\mathfrak{k}"]
   \arrow[loop left, "\neg"]
   \arrow[r, "\mathfrak{i}"]
...
\end{tikzcd}}
\caption{Hub-and-spoke at four carriers, $\mathbb{B}$, $\mathbb{K}_3$ (the Kleene 3-valued truth carrier), $\mathbb{N}$, and $\mathbb{Z}$.  Each hub is a single-species category; loops are endomorphisms within the species (Boolean negation $\neg$ on $\mathbb{B}$; Kleene negation $\neg_{\mathbb{K}_3}$ on $\mathbb{K}_3$; squaring $x \mapsto x^2$ on $\mathbb{N}$, which closes on the carrier; negation $x \mapsto -x$ on $\mathbb{Z}$, which closes because $\mathbb{Z}$ is closed under additive inverse).  Inter-hub arrows are functors between species: the structural embeddings are drawn in fraktur ($\mathfrak{i}: \mathbb{B} \to \mathbb{N}$, the canonical embedding $\text{false} \mapsto 0$, $\text{true} \mapsto 1$; $\mathfrak{k}: \mathbb{B} \to \mathbb{K}_3$, classical into Kleene; $\mathfrak{s}: \mathbb{K}_3 \to \mathbb{Z}$, the signed-truth realisation $\text{true} \mapsto +1$, $\text{false} \mapsto -1$, $\text{unknown} \mapsto 0$, under which Kleene negation $\neg_{\mathbb{K}_3}$ commutes with integer negation $-x$); the concrete pair $\mathbb{N} \rightleftarrows \mathbb{Z}$ is labelled by the operation it performs, $x \mapsto -x$ in one direction and $x \mapsto x^2$ in the reverse, the latter exhibiting the symmetry that the same operation appears as an endomorphism on $\mathbb{N}$ and as a structurally-typed restriction $\mathbb{Z} \to \mathbb{N}$ (squaring closes on $\mathbb{N}$ and lands in $\mathbb{N}$ from any integer input).  Most arrows on each hub and most inter-hub arrows are suppressed for clarity; the diagram shows one of each kind.}
