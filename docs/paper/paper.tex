...
The lowering changes an expression's \emph{representation}, from the exact ring to floating-point, and that map is lossy; changing only \emph{when} the expression is evaluated, from translation time to run time, is not (Appendix~\ref{app:partial-eval}).
...
Such a call the compiler folds when it is cheap enough and emits as run-time code when it is not; because the carrier is exact either choice yields the same value, so the fallback costs only the folding and never the answer.
