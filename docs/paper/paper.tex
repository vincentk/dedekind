...
They \emph{lift}, by two composing rules, and the section spends a listing on each because they are the mechanism everything downstream leans on.
...
A class of algebras closed under homomorphic images, subalgebras, and products is a variety (Birkhoff~\cite{burris1981universalalgebra}), and the library realises that closure as \emph{trait propagation}.
A one-line registration names the base, and the species traits (\cppinline{is_associative_v}, \cppinline{is_commutative_v}, and the rest) forward to the derived carrier for every parameter.
The engine is small enough to show in full (Listing~\ref{lst:rule2-engine}): a registration point, the concept that reads it, and one forwarding specialisation standing for the family.
...
Listing~\ref{lst:hsp-closed} pins the outcome at the cyclic base $\mathbb{Z}/6\mathbb{Z}$: the variety ``associative and commutative $+$'' stays closed across its H, S, and P images, witnessed by eight \cppinline{static_assert}s.
...
\vspace{0.3em}
\begin{flushleft}
{\footnotesize \textsuperscript{$\dagger$} \cppinline{Complex<Rational<I>>} and \cppinline{ExactReal<I>} certify only the ring/field \emph{operator surface} (\cppinline{HasRingOperators} / \cppinline{HasFieldOperators}), for two \emph{different} reasons.  \cppinline{Complex<R>} gets no generic field witness because $x^2 + 1$ is reducible over some field parameters (notably $\mathbb{F}_2$, where it is $(x+1)^2$), so field-ness is deliberately not lifted through the constructor.  \cppinline{ExactReal} is instead blocked by the architectural \cppinline{IsTotal} gate (no exact totality path is registered).  \cppinline{Dual} is a strict module over $\mathbb{Q}$ carrying ring operators, but $\varepsilon^2 = 0$ leaves it without a strict \cppinline{IsRing} witness, so it sits with the modules.}
\end{flushleft}
\end{table}
