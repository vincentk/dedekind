...
A parametric two-dimensional linear program was implemented whose input polytope and output optimum are both carried as Sets in the project's set-comprehension DSL.
...
\subsection{A Well-Behaved Structure in C++: the Juliet Posture $\mathbf{Jlt} \subset (\mathbf{CCC} \cap \mathbf{Cpp})$}
\label{sec:jlt}

Section~\ref{sec:alg-sigma} carved out a well-behaved structure on the mathematics side: $\mathbf{Alg}(-)_{\beth_1}$.
Here we want the engineering counterpart.
The idea is to find a fragment of C++ that, suitably disciplined, matches the structure our algebraic hubs need to inhabit.
The following two criteria were used in this largely empirical development process:
First, alignment with the universal-algebra fragment $\mathbf{Alg}(\Sigma)$ identified in §\ref{sec:alg-sigma}.
Second, the desire to integrate functionality the standard library already ships, in particular \cppinline{std::regular} value-types and the primitive carriers without wrapping them in \cppinline{struct} boilerplate or paying a runtime cost.
The findings are presented below as a proof sketch for clarity of exposition.
...
\begin{proof}[Sketch]
To show this intersection isn't empty, we exhibit one of its inhabitants: a mapping from a disciplined slice of $\mathbf{Cpp}$ to what a Cartesian Closed Category asks for (equivalently, the internal language of $\mathbf{STLC}$).
The slice is what \cppinline{std::regular} together with the admissibility rules of Appendix~\ref{app:jlt-systemf} carves out; the mapping is the five steps below.

\begin{enumerate}
    \item \textbf{Objects.}
    Take the types satisfying \cppinline{std::regular}: the ones with default-construction, copy-semantics, and an honest \cppinline{operator==}.
    We can read these as sets of values with stable identity, as long as nothing else inspects type internals to break that read.
    The admissibility rules of Appendix~\ref{app:jlt-systemf} (no public-API specialisation, no behaviour-branching SFINAE) are exactly what keeps the \emph{as long as} honest, and they're what gives us equational extensionality $a = b \implies f(a) = f(b)$ on the morphisms below.

    \item \textbf{Morphisms.}
    Take \cppinline{constexpr} lambda expressions, again on the disciplined fragment.
    By itself \cppinline{constexpr} doesn't get us pure and terminating arrows --- captures can be mutable, and template instantiation is Turing-complete in the general case~\cite{veldhuizen2003templates}.
    The same admissibility rules from Appendix~\ref{app:jlt-systemf} (no \cppinline{reinterpret_cast}, no SFINAE branching, bounded instantiation depth) recover the pure, terminating, effect-free reading; identity and associativity of composition hold by inspection.

    \item \textbf{Products.}
    \cppinline{std::pair} with structured bindings hands us the universal property of products: for any $A, B$ we have a product $A \times B$ with projections $\pi_1, \pi_2$, and the mediating-arrow construction is the obvious tuple-build.
    This is the \emph{Cartesian} half of the structure.

    \item \textbf{Exponentials.}
    First-class lambdas hand us the internal Hom-object $B^A$: a lambda value \cppinline{[](A) -> B { ... }} \emph{is} an arrow $A \to B$, packaged as a C++ value.
    Currying --- mapping $f : A \times B \to C$ to $\hat{f} : A \to C^B$ --- is what lambdas-capturing-arguments do, and on the disciplined fragment it gives us the bijection $\mathrm{Hom}(A \times B, C) \cong \mathrm{Hom}(A, C^B)$ that \emph{closed} asks for.

    \item \textbf{The admissibility gate.}
    \cppinline{concept} does the work at compile time: it mechanically checks that a given type $T \in \mathbf{Cpp}$ has the operator-shape and member-API surface to inhabit $\mathbf{CCC}$.
    Note this is the \emph{C++-side gate}, not the topos-level subobject classifier $\Omega$ of §\ref{sec:alg-sigma} --- the bona fide $\Omega$ lives one layer up in the codebase's \texttt{category:topoi} partition, and \cppinline{concept} is the type-system's complement to it.
\end{enumerate}

Putting these together: a terminal object (\cppinline{void} / \cppinline{std::monostate}), finite products, exponentials, and an admissibility gate, all inside the disciplined \cppinline{constexpr} subset Appendix~\ref{app:jlt-systemf} carves out.
The intersection $\mathbf{CCC} \cap \mathbf{Cpp}$ is inhabited.
\end{proof}

We can compare and contrast this fragment with three textbook schools~\cite{pierce2002tapl, cardelli1996typesystems} of type identification:
i) \textbf{nominal typing} (declared lineage, often inheritance; checked at compile time but blind to behaviour),
ii) \textbf{duck typing} (structural by call-site method probe; checked at runtime, too late for the optimiser),
iii) \textbf{static structural typing} (concept-based; behaviour checked at compile time without a declared lineage).
C++23 \cppinline{concept}, \cppinline{constexpr}, and \cppinline{std::regular} realise the third school.
Call this disciplined subset $\mathbf{Jlt} \subset \mathbf{CCC} \cap \mathbf{Cpp}$ (mnemonically: \emph{Juliet}), after the observation that a rose by any other name would smell as sweet~\cite{shakespeare1597romeo}.
The library cares about the \emph{scent} (structural invariants), not the \emph{lineage} (declared name).
In $\mathbf{Jlt}$ the objects are well-behaved types and the values they classify, and the morphisms are the arrows the language admits between them.
...
\begin{enumerate}
\item~\cppinline{constexpr} branching via template specialisation (the C++<20 baseline);
\item~\cppinline{concept}-constrained templates, which lift specialisation from a \emph{compiles-or-doesn't} probe to a named structural predicate (the C++20/23 step the Juliet Posture rests on);
\item~developer-supplied claims and axioms via \emph{trait specialisations} that downstream concepts consult to discharge a single \cppinline{static_assert} obligation.
\end{enumerate}
An example is shown in Listing~\ref{lst:jlt}.
The first two tiers are mechanically decidable on the disciplined fragment the library polices.
The third tier is a manual-certification path.

The motivation for the third tier is as follows:
...
The library thus pre-defines traits (\cppinline{is_associative_v<T, Op>}, \cppinline{is_commutative_v<T, Op>}, \dots) and concepts (\cppinline{IsRing}, \cppinline{IsField}, \ldots), with safe defaults.
Overrides or custom user-supplied traits in the downstream are explicitly allowed.
Here, we mean safe defaults to be read in the bit-perfect sense: \cppinline{bool} is \emph{recognized} as \cppinline{IsField} (the Galois Field $\mathbb F_2$) and as \cppinline{IsLattice}, while \cppinline{int} is \emph{rejected} as \cppinline{IsRing} because addition is not associative in general (overflow).
We call this safe-by-default configuration the \emph{Honest Rejection} policy.
The trait-registry mechanics --- which axioms propagate under which constructors, how per-carrier specialisations are organised, how the \emph{Honest Rejection} policy maps to varieties, and how the algebra-side machinery composes with the C++-side concept layer --- are deferred to §\ref{sec:pruning} (\emph{Algebraic Lattice}) and §\ref{sec:numeric-tension}.
...
\begin{table}[htbp]
\centering
\caption{\textbf{The chain $\mathbf{Cat} \to \mathbf{Ddk}$ as named refinement steps.}  Each row adds one structural commitment over the previous; each is cashed out by an existing C++23 partition / concept.  Steps 0--6 are internal to category theory; step 7 is the move out of pure CT into the C++ realisation.  The chain is approximately linear at the granularity shown; the honest reading is more lattice-shaped, with concreteness (step 5) and the size axis (step 1) sitting orthogonally to the CCC and topos refinements (steps 2--4) rather than strictly downstream of them.}
...
5 & concreteness (objects \emph{are} sets; faithful $U: \mathcal{C} \to \mathbf{Set}$) & \cppinline{:concrete} / \cppinline{IsConcrete}, \cppinline{IsSetObject} & concrete topos        \\
6 & ETCS axioms (well-pointedness, NNO, choice)  & \cppinline{:etcs} / \cppinline{IsSet}                   & Set-modeling topos             \\
\midrule
...
If a set is a predicate, a set operation is whatever operation you would apply to the predicates --- intersection is conjunction, union is disjunction, complement is negation:
\begin{align*}
...
\end{align*}
The combinators compose at compile time and produce a new type --- a new rule.
The same dictionary extends to a sequence's graph relation in §\ref{sec:sequences-relational} (Table~\ref{tab:sql-rosetta}); §\ref{sec:pruning-payoff} closes the page-1 hinge with the structural-pruning payoff.
...
\subsection{Structural Pruning: the Page-1 Payoff}
\label{sec:pruning-payoff}

The payoff of the comprehension view is that the LLVM backend can \emph{prune} a set expression to a no-op when its predicate is statically unsatisfiable.
\emph{Algebraic} properties of the predicates, recorded in the \texttt{:species} registry, provide additional pruning opportunities --- $S \cap \varnothing = \varnothing$, $S \cap \overline{S} = \varnothing$, $S_1 \cap S_2 = S_1$ when $S_1 \subseteq S_2$ --- each discharged as a \cppinline{static_assert} once the relevant algebraic witnesses are in the trait registry.

% Source: src/test/cpp/modules/dedekind/python/showcase_alpha_prime.cpp
\begin{listing}[htbp]
\caption{α′ — a single listing across the §3 narrative arc \emph{rule\,→\,tested\,→\,trimmed\,→\,collapsed\,→\,contradicted\,→\,certified\,→\,transformed} on $\mathbb{N}$ (the \emph{transformed} nugget parallel-sections on $\mathbb{Q}$, the smallest carrier where the scout-algebra additive and multiplicative gates both fire).  Each line is one structural reduction the compiler discharges.  Post-\#622 the carrier-axis decidability resolver routes countable carriers ($\mathbb{N}$, $\mathbb{Z}$, $\mathbb{Q}$) to \cppinline{ClassicalLogic}, so \cppinline{S.contains(7u)} reads as bare \cppinline{static_assert}-eligible \cppinline{bool} without a Kleene lift.  The cardinality-1 collapse \cppinline{S & {<7}} → \cppinline{Singleton<6>} and the law-of-excluded-middle reduction \cppinline{S & !S} → $\varnothing$ are both type-level discharges; Listing~\ref{lst:pruning_ir} demonstrates the same empty-meet structural pattern at the IR level via a sibling halfspace-contradiction witness in \texttt{showcase\_03\_halfspace\_contradiction.cpp}.  The \emph{transformed} nugget is the in-line scout-algebra surface (\#664): \cppinline{in<ℚ> + bound<3>} shifts the source halfspace's pivot by $+3$ at the type level; \cppinline{in<ℚ> * bound<2>} scales the pivot by $\times 2$ (positive-scalar branch; the negative-scalar branch flips the halfspace's direction).  $\mathbb{Q}$ admits both because \cppinline{IsField<Rational<default_integer>>} fires structurally (PR \#674), inheriting the additive and multiplicative gates from the field axioms.}
\label{lst:pruning}
\begin{cpp}
constexpr auto S = Set{in<ℕ> | in<ℕ> > bound<5>};                         // rule
static_assert(S.contains(7u));                                              // tested
...
static_assert(IsExtensional<decltype(a)> && IsExtensional<decltype(b)>);     // certified
constexpr auto Sₜ = Set{in<ℚ> + bound<3> | in<ℚ> > bound<2>};                // transformed (+): {y | y > 5}
constexpr auto S× = Set{in<ℚ> * bound<2> | in<ℚ> > bound<5>};                // transformed (×): {y | y > 10}
\end{cpp}
\end{listing}
