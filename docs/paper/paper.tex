...
Over an \emph{infinite} intermediate the enumeration would not terminate, so the library also carries the \emph{monadic} presentation of a relation, $R : A \to \mathcal{P}(B)$: composition is then Kleisli arrow composition over the power-set monad, $(R \mathbin{\gg} S)(a) = \bigcup_{b \in R(a)} S(b)$, a lazy symbolic pipeline that never materialises the middle set.
The two are the currying transposes of one relation, so the same \cppinline{>>} spells both; the choice of presentation is the choice of which intermediate you can afford to enumerate.
