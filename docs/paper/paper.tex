...
\begin{lstlisting}[caption={Compile-time reduction of contradictory set intersections.}]
using 
 = Set<
, [](auto x) { return x > 0.0; }>;
using 
...
% --- Table with Endofunctor Constraint ---
\begin{table}[h]
\centering
\begin{tabular}{lllll}
\hline
\textbf{Structure} & \textbf{Endofunctor $T$} & \textbf{Unit ($\eta$)} & \textbf{Mult. ($\mu$)} & \textbf{Species Interaction} \\ \hline
\texttt{std::identity\_t} & $Id: \mathcal{C} \to \mathcal{C}$ & \texttt{x} & \texttt{x} & Identity \\
\texttt{std::optional} & $T(A) = A + 1$ & \texttt{just(x)} & \texttt{flatten} & Coproduct ($+$) \\
\texttt{std::variant} & $T(A) = \sum A_i$ & \texttt{in\_i(x)} & \texttt{join} & Sum ($\sum$) \\
\texttt{std::vector} & $T(A) = A^*$ & \texttt{\{x\}} & \texttt{concat} & List Monad \\
\texttt{std::pair<W, A>} & $T(A) = W \times A$ & \texttt{\{id\_w, x\}} & \texttt{combine\_w} & Product ($\times$) \\
\hline
\end{tabular}
\caption{C++ Standard Types as Endofunctors in the Category of Species}
\label{tab:cpp_endofunctor_mapping}
\end{table}

\begin{itemize}
    \item \textbf{Frobenius Algebra:} The \texttt{std::vector} (List) and finite Coproducts (\texttt{std::variant}) can be viewed as Frobenius algebras in the category of sets, as they possess compatible monoid and comonoid structures (e.g., \texttt{concat} and \texttt{duplicate}).
    \item \textbf{Comonadic Applicability:} \texttt{std::pair} acts as the Product Comonad (often used for local environment state), while \texttt{std::vector} can be comonadic if treated as a non-empty focused stream (Zipper).
\end{itemize}
...
TODO: include some benchmarks e.g. from the bengchmarks game. Pidigits, SpectralNorm, \ldots maybe something "memory-bound" to feature sparsity and lazy evaluation.
