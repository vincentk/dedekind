...
\begin{itemize}
    \item \textbf{Mathematical Axiom:} A collection of objects and arrows (morphisms) closed under an associative composition with identities. \emph{Smallness} is the size condition that both collections are \emph{sets} rather than proper classes; when only the hom-collections are required to be sets the category is \emph{locally small}. The partition names both rungs --- \texttt{IsLocallySmallCategory} (the arrow-collection is a single C++ type, hence a set) and \texttt{IsSmallCategory} (additionally the objects form a set).
    \item \textbf{C++ Reification:} Objects are \texttt{std::regular} carriers --- the Juliet-Posture substrate of Section~\ref{sec:jlt_posture} --- registered as a \texttt{Species} (guaranteeing stable value identity). Morphisms are values of a single arrow type satisfying \texttt{IsArrow} --- which exposes \texttt{::Domain} and \texttt{::Codomain} typedefs and a callable \texttt{f(x)} into the codomain (\emph{not} \texttt{std::regular\_invocable}, which cannot name the arrow's domain and codomain). Composition is the overloaded \texttt{operator>>}, with the identity laws \texttt{id >> g == g} and \texttt{f >> id == f} discharged as overloads; identity arrows come from an \texttt{id\_c(x)} factory.
    \item \textbf{Clarity Note for Engineers:} Instead of data-hiding containers, a ``set'' at this entry rung is simply a well-behaved C++ type acting as a domain, and a ``function'' is a side-effect-free arrow mapping inputs to outputs. In its single-object mode --- the carrier type taken as the sole object and its arrows as endomorphisms --- composition makes the category a \textbf{monoid} (a one-object category); these are exactly the endomorphism loops drawn on each carrier hub of Figure~\ref{fig:hub-spoke-bnz}.
\end{itemize}
...
\begin{itemize}
    ...
    \begin{enumerate}
        \item \textit{Terminal Object:} There exists a unique set $1$ with exactly one element, and every set $A$ has exactly one arrow $A \to 1$.
    ...
    \end{enumerate}
    \item \textbf{C++ Reification:} The concepts \texttt{IsTerminalObject} and \texttt{IsProduct} gate the structure, and the standard library satisfies them out of the box: \texttt{std::monostate} \emph{is} the terminal object (exported as \texttt{One}), and \texttt{std::pair} \emph{is} the binary product (\texttt{std::tuple} the $n$-ary case), with projections $\pi_1 / \pi_2$ reading \texttt{.first} / \texttt{.second}. The universal factorisation $\langle f, g\rangle$ is the one piece \texttt{std} does not provide; the library supplies it as the default \texttt{mediate\_product} arrow.
    \item \textbf{Clarity Note for Engineers:} This layer turns the relational coordinate tables of Section~\ref{sec:relations_functions_blueprint} into type-safe tuples. The terminal object lets us define ``elements'' of a set $A$ structurally, as arrows \texttt{One} $\to A$.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{C++ Reification:} Modeled via value-capturing \texttt{constexpr} lambda expressions (the compile-time exponential), with \texttt{std::function} as the type-erased \emph{runtime} carrier.
    \item \textbf{Clarity Note for Engineers:} This licenses function \textit{Currying} at compile time via the \texttt{constexpr} lambdas (\texttt{std::function} is not \texttt{constexpr}-usable and serves only the erased runtime form). It guarantees that functions are first-class citizens that can be passed, stored, and transformed by other functions without dropping out of the type-checking engine.
\end{itemize}
