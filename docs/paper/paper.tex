...
\end{equation}
The membership condition is a bounded $\forall$, so an extremum is literally \cppinline{forall} composed with the order (Listing~\ref{lst:extrema_def}, top), and $\min$ is $\max$ read in the dual order, the way $\forall$ was $\exists$ under $\neg$.
No inverse, product, or projection enters yet; a bare comparison suffices, which is why the extremum of a set lands before relations do.
The answer is a set, so the degenerate cases need no special casing: an empty carrier gives $\varnothing$, and a total or partial order gives a singleton (a greatest element, when it exists, is unique by antisymmetry) or $\varnothing$; a larger set arises only under a \emph{preorder}, where distinct elements can be equivalent and tie for the top (Listing~\ref{lst:extrema_def}, bottom).
Being a comprehension, the extremum is lazy and inherits the quantifier's call-site bargain: it types in for any set and order, but decides only where the carrier is finite (walk it) or the order hands over a finite quotient.
The dominance $\forall$ over an unbounded halfspace on $\mathbb{N}$ that carries no such handle stays undecided, the same Rice wall as before, and $\max$ over it returns \texttt{Unknown} rather than a fabricated bound.
...
\begin{listing}[htbp]
\caption{The extremal sets.
\textbf{Top}: \cppinline{max} / \cppinline{min} over an enumerable set \cppinline{S}, each the comprehension whose membership test is a bounded \cppinline{forall} over the order (\cppinline{min} is \cppinline{max} in the dual order); the result is itself an \cppinline{IsSet}.
\textbf{Bottom}: over the lifted range $[2,6)$ each extremum is a singleton, so membership \cppinline{max(D)(x)} is the extremum test.}
\label{lst:extrema_def}
\noindent
\begin{minipage}[t]{0.49\linewidth}
\begin{cpp}
// 𝔹 (finite): the top / bottom, via the where-clause
auto max(Ω<bool> s) { return s | (π == fix(true_c)); }
auto min(Ω<bool> s) { return s | (π == fix(false_c)); }
\end{cpp}
\end{minipage}\hfill
\begin{minipage}[t]{0.49\linewidth}
\begin{cpp}
// a halfspace: the pivot, read off the type — no walk
auto max(Halfspace<T,p,Down,NonStrict>) {
  return Singleton<p>{};   // greatest = p
}
\end{cpp}
\end{minipage}
...
\begin{cpp}
static_assert(max(𝔹)(true));   static_assert(min(𝔹)(false)); // finite: {true} / {false}
static_assert(max(ℤ | (π <= fix(5_c)))(5));   // intensional (infinite): max {x ≤ 5} = {5}
static_assert(min(ℤ | (π >= fix(5_c)))(5));   //                         min {x ≥ 5} = {5}
\end{cpp}
\end{minipage}
\end{listing}
