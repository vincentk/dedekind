...
$\mathbf{Jlt}$ was then used to bootstrap first, a lazy, intensional set comprehension engine under an ETCS reading ($\mathbf{Trsk} = \mathbf{Jlt} \cap \mathbf{Set}$), and second, an intersection with universal algebraic signatures, forming the core category $\mathbf{Ddk} = \mathbf{Trsk} \cap \mathbf{Alg}$.
The resulting embedded DSL allows for the reification of coordinate-free analytic graphs as relations on sets, augmented with algebraic laws.
...
If all arguments can be applied at compile-time, the compiler folds the optimum and the emitted IR is simply a constant result, typed to the semiring used to describe the cost function ($\mathbb B$ for reachability, $\mathbb N$ for cost estimates, dual numbers for sensitivity analysis).
...
Listing~\ref{lst:algebra:universal} is the whole of the membership test.
A type inhabits the category $\mathbf{Ddk}$ exactly when it type-checks against \cppinline{IsAlgebraOnSet}, which imposes universal-algebra structure on the Tarski set layer $\mathbf{Trsk}$ of §\ref{sec:intensional_sets}:
\begin{equation}
    \mathbf{Ddk} \;=\; \mathbf{Trsk} \,\cap\, \mathbf{Alg}(-),
\end{equation}
the algebraic sets expressible as C++ types.
Because membership is structural, a pre-existing type such as \cppinline{std::pair} joins $\mathbf{Ddk}$ the moment a concept accepts it, with no change to the carrier.

Figure~\ref{fig:hub-spoke-bnz} is this category drawn out, and it is worth keeping in hand for the rest of the section.
Its objects are the algebraic sets, the hubs $\mathbb{B}, \mathbb{N}, \mathbb{Z}$.
The loop on each hub is that carrier's own algebraic operations, $\wedge$ and $\vee$ on $\mathbb{B}$, $+$ and $\times$ on $\mathbb{Z}$: the arrows a carrier composes with itself.
The arrows between hubs are the embeddings, the canonical inclusions $\mathbb{B} \hookrightarrow \mathbb{N} \hookrightarrow \mathbb{Z} \hookrightarrow \mathbb{Q}$ that the \cppinline{embed_*_*} family ships.
...
The quotient rung is backed by the First Isomorphism Theorem, $A/\ker f \cong \operatorname{im} f$~\cite{burris1981universalalgebra}, which the library discharges as a typed witness in \texttt{category:image} rather than as prose.
...
These are the same traits §\ref{sec:optimization_lp}'s \cppinline{IsSemiring} gate reads, so the exhibit there does not re-establish that its carrier is a dioid; it inherits the certification from here.
...
The one carrier worth a second look, because §\ref{sec:optimization_lp} differentiates with it, is the dual numbers $\mathbb{D} = \mathbb{R}[\varepsilon]/(\varepsilon^2)$.
A dual number is a pair $(a, b)$ whose second coordinate carries the coefficient of $\varepsilon$, and the truncation $\varepsilon^2 = 0$ is enforced as a compile-time reduction, so a product collapses to $(a, b)(c, d) = (ac,\, ad + bc)$ during translation.
That is forward-mode automatic differentiation obtained as a ring law rather than a hand-written container: the $\varepsilon$-coordinate is the derivative and the truncation is the product rule (Listing~\ref{lst:lin-alg-embeddings}).
