...
\textbf{Right, the set layer} (\cppinline{algebra}): a set \emph{object} whose carrier is such a model, the subobject-over-carrier pattern familiar from proof assistants~\cite{mathlib2020}.  \cppinline{IsAlgebraOnSet} is the generic $(A,F)$ form available today; the named rungs \cppinline{algebra::IsMonoid<X>} are the set-indexed lift of the left column, the target the library is converging on.}
...
concept IsMonoid = IsSet<X>
  && category::IsMonoid<
       typename X::Domain, Op>;
\end{cpp}
\end{minipage}
\end{listing}
...
\begin{listing}[!ht]
\caption{Concrete carriers inhabiting Listing~\ref{lst:algebra:universal}, shown at the \emph{pointwise} (element) layer --- every check is on the element type \cppinline{X::Domain}.  The complementary \emph{set} layer for these carriers (membership and the $\cup,\cap$ subobject lattice) is Listing~\ref{lst:pruning}.
...
pointwise on \cppinline{decltype(𝔹)::Domain}
\begin{cpp}
static_assert(
  IsLattice<decltype(𝔹)::Domain,
    std::logical_or<bool>,
    std::logical_and<bool>>);
static_assert(
  IsGaloisField<decltype(𝔹)::Domain,
    std::bit_xor<bool>,
    std::bit_and<bool>>);
\end{cpp}
\end{minipage}\hfill
\begin{minipage}[t]{0.45\linewidth}
pointwise on \cppinline{decltype(𝕊)::Domain}
\begin{cpp}
...
static_assert(IsField<Q>);                   // scalar: ℚ is a field
