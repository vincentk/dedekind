...
\begin{itemize}
    \item \textbf{Axioms of Categories and Smallness:}
    \begin{enumerate}
    ...
    \end{enumerate}
    \item \textbf{C++ Reification:} The concepts \texttt{IsSmallCategory} and its weaker sibling \texttt{IsLocallySmallCategory} gate the structure. Per $\mathbf{Jlt}$~\ref{sec:jlt_posture}, objects are \texttt{std::regular} carriers. Morphisms are values of a single arrow type satisfying \texttt{IsArrow}, with \texttt{::Domain} and \texttt{::Codomain} typedefs and a callable \texttt{f(x)} from the domain into the codomain. Composition is the overloaded \texttt{operator>>}, with the identity laws \texttt{id >> g == g} and \texttt{f >> id == f} discharged as overloads; identity arrows come from an \texttt{id\_c(x)} factory.
