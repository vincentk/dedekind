...
\begin{minipage}[t]{0.49\linewidth}
\begin{cpp}
template <IsEnumerableSet S>
constexpr auto max(S s) {
  // { x ∈ s | ∀ a ∈ s. a ≤ x }
  return Set{s | forall(s, a <= x)};
}
\end{cpp}
\end{minipage}\hfill
\begin{minipage}[t]{0.49\linewidth}
...
\begin{cpp}
constexpr auto D = iota(2, 6);   // the finite set [2,6), a set by its membership
static_assert(max(D)(5));        // 5 is the greatest element of [2,6)
static_assert(min(D)(2));        // 2 is the least
\end{cpp}
\end{minipage}
\end{listing}
