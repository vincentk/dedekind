...
\toprule
set & meet & join & complement & bottom & top & order \\
\midrule
...
\end{tabular}
\caption{
Operator mapping table relating pointwise operations on three lattices: the Booleans (with logical operations), the powersets (with set operations), the reification of powersets $\mathbf{Trsk} \subset \mathbf{Cpp}$ discussed here (Section~\ref{sec:ddk_etcs}, Eqn~\ref{eqn:trsk}).
The set lattice \emph{is} the Boolean lattice, lifted pointwise, and the collapse in Listing~\ref{lst:pruning} is that lift at work in the form of type-checked \Cpp{23}.
}
...
\label{def:etcs-set}
A \emph{set} over a carrier type $A$ is presented by its characteristic morphism $\chi_S : A \to \Omega$ into the truth object $\Omega$, with membership $x \in S$ meaning $\chi_S(x) = \top$.
Sets and the functions between them form the category $\mathbf{Set}$ of the elementary theory of the category of sets: a well-pointed topos with a natural-numbers object and choice.
Its intensional realisation inside $\mathbf{Jlt}$ is
\begin{equation}
\label{eqn:trsk}
