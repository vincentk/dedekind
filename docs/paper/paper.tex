...
The first pillar.
We restrict to an axiomatic subset of C++23 — types as objects, the type-respecting maps the language admits as arrows — and let the compiler check our claims.
The disciplined fragment is Cartesian-closed under Lambek--Scott, so the surface reads as a faithful image of STLC: a \lstinline|concept| picks out the inhabitants satisfying a predicate (a subobject classifier on the underlying category $\mathbf{Cpp}$), and type composition reads as arrow composition.
