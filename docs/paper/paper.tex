...
\begin{listing}[htbp]
\caption{A function \emph{is} its graph.  \cppinline{f = ℤ*ℤ | π1+fix(3_c)==π2} is the point-free relation $\{(x, x{+}3)\}$, a bona fide function: functional and entire, the two bounds of Table~\ref{tab:relation-quantifier}'s $\pi_A$ column.  Its \cppinline{inverse} is the graph read backwards, the converse relation $x{-}3$; \cppinline{image} is the range, the whole line for a total (surjective) function, and a domain bound pushes forward through the graph, $\{x\le5\}$ to $\{y\le8\}$, again a halfspace of the same shape (abridged from the checked-in witnesses).}
\label{lst:functional_image_bijection}
\begin{cpp}
constexpr auto f = ℤ * ℤ | π1 + fix(3_c) == π2;   // x ↦ x+3 as its graph
static_assert(is_function(f));                     // functional ∧ entire
static_assert(inverse(f) == (ℤ*ℤ | π1+fix(-3_c)==π2));  // converse graph: x−3
static_assert(image(f) == ℤ);                      // total ⇒ onto ℤ
static_assert(image(f | π1 <= fix(5_c))            // {x≤5} pushes forward
              == (ℤ | π <= fix(8_c)));             //   ... to {y≤8}
\end{cpp}
\end{listing}
