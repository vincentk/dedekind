...
%%%
\clearpage
\section{The Trait Registry and the Carriers}
\label{sec:trait-registry}

Where Appendix~\ref{app:jlt-systemf} grounds the structural surface, this appendix records what the type system can certify about the algebra of each carrier: the built-in primitives, the abstract-versus-IEEE tension that forces hand-certification, and the per-carrier rosetta.

\subsection{The Algebra of the Built-in Types}
\label{sec:builtin-algebra}

Table~\ref{tab:primitive-algebra} gives the headline; the four tables below break the principal built-in types down by operator surface, grouped by the standard concept each family satisfies (\cppinline{bool} on its own), split because the built-in primitives do not share a single operator surface.
The library carriers built on these primitives, and the strict/functor/literal tiers they are graded by, are catalogued in §\ref{sec:carrier-rosetta}.
...
\phantomsection\label{par:concept-tiers}%
