...
However, \emph{we deliberately leave $\Sigma$ unbound for now}, the intent is to defer the formalization of operations.
The net result are sets equipped with operations realising as yet unspecified signatures $\Sigma$ such that the subset relations read
$\mathbf{Mon} \subset \mathbf{Alg}(\Sigma) \subset \mathbf{Alg}(-) \subset \mathbf{Set} \subset \mathbf{Cat}$.
While the binding of concrete operator-shape signatures such as such as $\neg, \vee, \wedge, \cup, \cap, +, \times, \ldots$ is discussed in later sections~\ref{sec:cpp-stlc-mapping}, \ref{sec:sets-rules},
we can already name some familiar examples: The Booleans $\mathbb{B}$ with logical operations, the integers $\mathbb{Z}$ with addition and subtraction and so on.
...
\begin{enumerate}
    ...
    We select the sub-universe of types satisfying \cppinline{std::regular}. By requiring copy-semantics and equational extensionality ($a = b \implies f(a) = f(b)$), we satisfy the requirement that \textbf{Objects} behave as sets of values with stable identity. 
    \item \textbf{Selection of Morphisms (The Arrows):} 
    We select the set of \cppinline{constexpr} lambda expressions. By enforcing a pure, terminating, and effect-free evaluation context, we ensure that every function in this sub-universe is a \textbf{Morphism} in the categorical sense, satisfying the identity and associativity laws of composition without the interference of side effects. 
    ...
    The existence of first-class lambdas provides the \textbf{Internal Hom-object} ($B^A$). This allows for the \textbf{Currying} transformation, mapping $f: (A \times B) \to C$ to $\hat{f}: A \to C^B$, satisfying the requirement for a closed category.
    \item \textbf{The Classifier (The Gate):} 
    The C++23 \cppinline{concept} mechanism acts as the \textbf{Sieve}. It allows us to mechanically verify that a given type $T \in \mathbf{Cpp}$ possesses the internal structure required to inhabit the $\mathbf{CCC}$.
\end{enumerate}

Because we have exhibited a terminal object (\cppinline{void} / \cppinline{monostate}), finite products, and exponentials within the \cppinline{constexpr} subset of C++, the intersection $\mathbf{CCC} \cap \mathbf{Cpp}$ is inhabited.
The \textbf{Juliet Posture} is defined as the intentional selection of this inhabitant.
The requirements are recapitulated in Table \ref{tab:cpp23-ccc-mapping}.
\end{proof}
...
\cppinline{std::regular} & \textbf{Objects as Sets} & Provides equational extensionality and identity. \\
\cppinline{constexpr} & \textbf{Pure Morphisms} & Guarantees side-effect-free, terminating composition. \\
\cppinline{concept} & \textbf{Structural Logic} & Filters \(\mathbf{Cpp}\) noise to identify \(\mathbf{CCC}\) structures. \\
Lambdas / \cppinline{std::pair} & \textbf{Exponentials / Products} & Reifies the Cartesian and Closed properties. \\ \bottomrule
\end{tabular}
\end{table}
