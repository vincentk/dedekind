...
The non-injective case keeps the machinery and drops the injectivity.
Take $f(x) = 2|x|$.
On $\mathbb{Z}$ it is two-to-one, the fibre of $y$ being $\{{+}y/2, {-}y/2\}$, so $f$ is \emph{not} monic and cannot simply ship a retract.
It ships its \emph{factorization} instead, $\mathbb{Z} \twoheadrightarrow \mathbb{N} \hookrightarrow \mathbb{Z}$ (fold the sign, then double): the mono leg's retract names one preimage, the canonical ${+}y/2$, and the epi leg's \emph{cofibre} expands a point to its whole class $\{{+}x, {-}x\}$ (Listing~\ref{lst:functional_image_fold}).
Membership $y \in f(S)$ then ORs the source over the \emph{entire} fibre, and that is exactly what keeps it sound.
A lone retract would answer $6 \in f(\{x<0\})$ with the canonical ${+}3 \notin \{x<0\}$ and wrongly say no, where the fibre also holds ${-}3$, which is in; the epi leg's cofibre is what supplies it.
This is also what discharges the $\arg\max$ of §\ref{sec:relations_functions_blueprint} over an infinite domain: an optimiser and its fibre are the same $\exists x.\,f(x){=}y$, read off the factorization rather than searched for.

\begin{listing}[htbp]
\caption{A non-injective map decided through its (regular epi, mono) factorization: $2|x|$.  It is not monic, so it ships a \cppinline{retract} (the mono leg: the canonical preimage ${+}y/2$) and a \cppinline{cofibre} (the epi leg: the class $\{{+}x,{-}x\}$), and \cppinline{image} ORs the source over the whole fibre.  Sound where a lone retract is not: $6 \in 2|x|(\{x<0\})$ holds via ${-}3$, though the canonical ${+}3$ is not in the source (abridged from the checked-in test).}
\label{lst:functional_image_fold}
\begin{cpp}
struct AbsDouble { using Domain = int; using Codomain = int;
  constexpr int operator()(int x) const { return 2*(x<0?-x:x); } };
constexpr auto retract(AbsDouble) {          // mono leg: canonical +y/2
  return [](int y) -> std::optional<int> {
    return (y>=0 && y%2==0) ? std::optional{y/2} : std::nullopt; }; }
constexpr auto cofibre(AbsDouble) {          // epi leg: the class {+x, -x}
  return [](int x) { return std::array{x, -x}; }; }

constexpr auto img = image(AbsDouble{}, negative);   // img(6) == true, via -3
\end{cpp}
\end{listing}

The factorization here is \emph{consumed}, not narrated: \cppinline{image} routes a non-monic arrow through its two legs, so the non-injective case is decided rather than deferred.
For general or non-bijective functions, validating whether an arbitrary element $b \in B$ possesses a valid preimage in the domain $A$ presents a severe decidability barrier if evaluated via standard search techniques.
