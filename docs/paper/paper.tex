...
In a dagger category the dagger is itself the one-sided inverse, so the isometry, coisometry and unitary conditions ($f^\dagger f = \mathrm{id}$, $f f^\dagger = \mathrm{id}$, both) name the mono, epi and iso classes and supply their witness at no cost: ``the converse is the inverse'' (a bijective relation) and ``the transpose is the inverse'' (an orthogonal matrix) become one theorem.
...
\addlinespace
general $m\circ e$ & retract of $m$, section of $e$          & a union of branches                               & converse composes contravariantly & --- \\
\bottomrule
\end{tabular}%
}
...
The final column names the arrow's $\dagger$-class when a dagger is present: the dagger then \emph{is} the one-sided inverse (unitary $f^{-1}=f^\dagger$, isometry retract $r=f^\dagger$, coisometry section $s=f^\dagger$), a witness a general $m\circ e$ arrow has no claim to.}
