...
\small
\begin{tabular}{l|cc|ccc|ccc|cc|ccc|ccc|cc|ccc|cc}
\toprule
\textbf{Species} & \multicolumn{5}{c|}{\textbf{bool}} & \multicolumn{8}{c|}{\textbf{uint}} & \multicolumn{8}{c|}{\textbf{int}} & \multicolumn{2}{c}{\textbf{double}} \\ \midrule
\textbf{Category} & \multicolumn{2}{c|}{Logical} & \multicolumn{3}{c|}{Bitwise} & \multicolumn{3}{c|}{Arithmetic} & \multicolumn{2}{c|}{Order} & \multicolumn{3}{c|}{Bitwise} & \multicolumn{3}{c|}{Arithmetic} & \multicolumn{2}{c|}{Order} & \multicolumn{3}{c|}{Bitwise} & \multicolumn{2}{c}{Arithmetic} \\ \midrule
\textbf{Operator} & \texttt{||} & \texttt{\&\&} & \texttt{|} & \texttt{\&} & \texttt{\^{}} & \texttt{+} & \texttt{*} & \texttt{\%} & \texttt{min} & \texttt{max} & \texttt{|} & \texttt{\&} & \texttt{\^{}} & \texttt{+} & \texttt{*} & \texttt{\%} & \texttt{min} & \texttt{max} & \texttt{|} & \texttt{\&} & \texttt{\^{}} & \texttt{+} & \texttt{*} \\ \midrule

\textbf{Identity}   & \texttt{f} & \texttt{t} & \texttt{f} & \texttt{t} & \texttt{f} & \texttt{0} & \texttt{1} & --- & \texttt{\textasciitilde 0u} & \texttt{0} & \texttt{0} & \texttt{\textasciitilde 0u} & \texttt{0} & \texttt{0} & \texttt{1} & --- & \texttt{INT\_MAX} & \texttt{INT\_MIN} & \texttt{0} & \texttt{-1} & \texttt{0} & \texttt{0.0} & \texttt{1.0} \\ 
\textbf{Absorber}   & \texttt{t} & \texttt{f} & \texttt{t} & \texttt{f} & --- & --- & \texttt{0} & \textbf{Hz}\textsuperscript{D} & \texttt{0} & \texttt{\textasciitilde 0u} & \texttt{\textasciitilde 0u} & \texttt{0} & --- & --- & \texttt{0} & \textbf{Hz}\textsuperscript{D} & \texttt{INT\_MIN} & \texttt{INT\_MAX} & \texttt{-1} & \texttt{0} & --- & \texttt{Inf} & \texttt{NaN} \\ 
\textbf{Associative}& \checkmark & \checkmark & \checkmark & \checkmark & \checkmark & \checkmark\textsuperscript{M} & \checkmark\textsuperscript{M} & --- & \checkmark & \checkmark & \checkmark & \checkmark & \checkmark & \textbf{Hz}\textsuperscript{U} & \checkmark & --- & \checkmark & \checkmark & \checkmark & \checkmark & \checkmark & \textasteriskcentered & \textasteriskcentered \\ 
\textbf{Commutative}& \checkmark & \checkmark & \checkmark & \checkmark & \checkmark & \checkmark & \checkmark & --- & \checkmark & \checkmark & \checkmark & \checkmark & \checkmark & \checkmark & \checkmark & --- & \checkmark & \checkmark & \checkmark & \checkmark & \checkmark & \checkmark & \checkmark \\ 
\textbf{Idempotent} & \checkmark & \checkmark & \checkmark & \checkmark & --- & --- & --- & --- & \checkmark & \checkmark & \checkmark & \checkmark & --- & --- & --- & --- & \checkmark & \checkmark & \checkmark & \checkmark & --- & --- & --- \\ 
\textbf{Inverse}    & --- & --- & --- & --- & \checkmark & \checkmark\textsuperscript{M} & --- & --- & --- & --- & --- & --- & \checkmark & \textbf{Hz}\textsuperscript{A} & --- & --- & --- & --- & --- & --- & \checkmark & \checkmark & \checkmark \\ 
\textbf{Monotonic}  & \checkmark & \checkmark & \checkmark & \checkmark & --- & \textbf{Hz}\textsuperscript{W} & \textbf{Hz}\textsuperscript{W} & --- & \checkmark & \checkmark & --- & --- & --- & \checkmark\textsuperscript{P} & \checkmark\textsuperscript{P} & --- & \checkmark & \checkmark & --- & --- & --- & \checkmark & \checkmark \\ \midrule
\textbf{Logic ($\Omega$)} & \multicolumn{5}{c|}{Classical ($\Omega_{\mathbb{B}}$)} & \multicolumn{8}{c|}{Classical ($\Omega_{\mathbb{B}}$)} & \multicolumn{8}{c|}{Classical ($\Omega_{\mathbb{B}}$)} & \multicolumn{2}{c}{Ternary ($\Omega_{K3}$)} \\ \bottomrule
\end{tabular}

\begin{flushleft}
\scriptsize
\textsuperscript{M} \textbf{Modular success}: Inverses and associativity hold in the modular ring $\mathbb{Z}/2^n\mathbb{Z}$. \\
\textsuperscript{A} \textbf{Asymmetry hazard}: Signed inverse (\texttt{-x}) fails at \texttt{INT\_MIN} due to range asymmetry. \\
\textsuperscript{W} \textbf{Wrap-around hazard}: Monotonicity fails at the Lipschitz boundary. \\
\textsuperscript{U} \textbf{Undefined behavior}: Associativity rejected due to non-deterministic overflow. \\
\textsuperscript{D} \textbf{Division hazard}: Modulo by zero triggers hardware halt (Absorber of Doom). \\
\textasteriskcentered \textit{Fails bit-perfect associativity due to rounding or NaN interactions.}
\end{flushleft}
\end{table}
\end{landscape}
