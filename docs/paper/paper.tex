...
constexpr Singleton<true> 
  T{};
constexpr Singleton<false>
  F = ~T;
static_assert((F | T) == 𝔹);
constexpr Ø<bool>
  empty = F & T;
...
static_assert(IsSet<decltype(ℕ)>);
constexpr Above<5>
  gt_5 = in<ℕ> > bound<5>;
constexpr AtMost<5>
  le_5 = ~gt_5;
static_assert((le_5 | gt_5) == ℕ);
constexpr Ø<Cardinality>
  empty = le_5 & gt_5;
\end{cpp}
\end{minipage}
\begin{cpp}
// membership on that empty meet, at -O2 — the collapse, made observable:
...
A predicate on the scout \emph{is} the set, since a set is its own characteristic morphism: \cppinline{in<ℕ> > bound<5>} already denotes $\{x \in \mathbb{N} \mid x > 5\}$, a \cppinline{Halfspace}, with no wrapper (Listing~\ref{lst:pruning}).
...
\begin{minipage}[t]{0.49\linewidth}
\begin{cpp}
// ι : std::set ↪ Trsk   (total)
std::set<int> c{2, 3, 5};  // contains IS χ
static_assert(IsSet<decltype(
  ambient_set<int>(from_std(c)))>);
\end{cpp}
\vspace{0.4em}
\begin{cpp}
// μ : Trsk ⇀ std::set   (partial)
auto isEven = [](int x){ return x % 2 == 0; };
auto evens = materialise(
  std::views::iota(0, 10), isEven);
assert((evens == std::set{0,2,4,6,8}));
// {x∈ℕ|isEven}: ℕ unbounded → no μ
\end{cpp}
\end{minipage}\hfill
\begin{minipage}[t]{0.49\linewidth}
\begin{cpp}
// the two worlds COMPOSE: an extensional
// set bounds an intensional one, so their
// intersection always materialises.
std::set<int> ext{2, 3, 5};
auto gt3 = in<ℕ> > bound<3>;  // {x>3}
auto gt5 = in<ℕ> > bound<5>;  // {x>5}
assert(materialise(ext, gt3)  // ext ∩ int1
       == std::set{5});
assert(materialise(ext, gt5)  // ext ∩ int2
       == std::set<int>{});
\end{cpp}
\end{minipage}
\end{listing}
...
\begin{minipage}[t]{0.49\linewidth}
\begin{cpp}
// 𝔹 = Ω<bool> : finite carrier
constexpr auto 𝔹 = Ω<bool>;
Singleton<true> isTrue;  // the {true}
static_assert( exists(𝔹, isTrue));
static_assert(!forall(𝔹, isTrue));
// set → FiniteBooleanSet, 2 truths
\end{cpp}
\end{minipage}\hfill
\begin{minipage}[t]{0.49\linewidth}
\begin{cpp}
// ℕ=Ω<Cardinality>: isDivisibleBy3 is
// periodic → factors thru Modular<3>
constexpr auto ℕ = Ω<Cardinality>;
...
Table~\ref{tab:relation-quantifier} organises these four properties as one structure: the quantifiers $\exists$ ($\ge 1$) and $\forall$ ($\le 1$) applied to each projection of $R \subseteq A \times B$, read across the quantifier, set-theoretic, and categorical vocabularies.
Here $\exists$ and $\forall$ are the indexed join $\bigvee$ and meet $\bigwedge$ of Table~\ref{tab:lattice-lift}, now ranging over a projection fibre rather than a binary pair.
A projection turning into an isomorphism marks the two composite notions: $\pi_A$ an iso is a function, $\pi_B$ an iso a bijection.
...
\bottomrule
\end{tabular}
\caption{The four properties of a relation $R \subseteq A \times B$ as $\exists$ ($\ge 1$) and $\forall$ ($\le 1$) on its two projections, in three vocabularies.
...
\begin{minipage}[t]{0.49\linewidth}
\begin{cpp}
// R ⊆ A×B is a predicate on the
// product, χ_R : A×B → Ω, so a
// relation IS A Set on A×B:
template <class R,class A,class B>
concept IsRelation =
  IsSet<R> && IsProduct<Dom<R>,A,B>;

// a Set is its own predicate:
static_assert(IsPredicate<
  Set<pair<A,B>,Classical,P>>);

// entire ∧ functional = function
// (⟺ IsArrow<A,B>); the epi/mono
// of the projection π_A (Table 3):
...
// converse  R˘(b,a) = R(a,b)
template <IsRelation R>
constexpr auto converse(R);

// composition = ∃-over-the-middle,
// the SAME fish functions use:
//  (R>>S)(a,c)=∃b.R(a,b)∧S(b,c)
template <class R,class S>
constexpr auto operator>>(R,S);

// codomain-side axis, π_B (Table 3):
template <class R> concept
IsSurjective = is_right_total_v<R>;
template <class R> concept
IsInjective = is_left_unique_v<R>;

// bijection: a function that is
// surjective ∧ injective (π_B iso).
\end{cpp}
\end{minipage}
\end{listing}
...
\begin{cpp}
// R = { (a, b) ∈ ℕ × ℕ | b % a == 0 } — a general relation (set of pairs)
using P = std::pair<unsigned, unsigned>;
constexpr auto R = Set{ element<Ω<P>> % UniversalSet<P>{} |
    [](const P& p){ return p.first != 0 && p.second % p.first == 0; } };

// pointwise membership constant-folds:
static_assert( R(std::pair{2u, 6u})); // 6 % 2 == 0
static_assert(!R(std::pair{4u, 6u})); // 6 % 4 != 0
\end{cpp}
\end{listing}
...
Rather than relying on explicit, verbose template-parameter bindings, the expression binds a scout element over the product domain $(\mathbb{N} \times \mathbb{N})$ and refines it by a predicate on the pair.
This scout binds the relational invariant as a native type-directed predicate.
...
To circumvent this translation-time search bottleneck, the library's \emph{compositional} presentation of a relation is not the static subobject pair $R \subseteq A \times B$ (which it still reifies, as a \texttt{Set} of pairs) but a monadic one, where a relation is an active functional morphism. Concretely, a binary relation $R$ from a domain set $A$ to a codomain set $B$ is defined as a total arrow of the form $R : A \to \mathcal{P}(B)$, where $\mathcal{P}(B)$ represents the intensional power set of the target space. This architectural choice ensures that when an element $a \in A$ is supplied to the relation, the compiler immediately evaluates the image as a native, optimized intensional set type in the destination space.

Under this functional presentation, Tarski's relative composition resolves elegantly as standard Kleisli arrow composition over the power set monad. We reify this step within the \texttt{:relations} partition as the \emph{fish} operator $R \gg S$, realised in code as the overloaded \cppinline{>>} (Listing~\ref{lst:kleisli_fish_composition}), so the point-free composition is written:
\begin{equation}
...
Listing~\ref{lst:relation_examples} works the surface over the two carriers: on each of $\mathbb{B}$ and $\mathbb{N}$ a relation that is not a function and a function, with functions composing through the same \cppinline{>>} that composed relations above.

\begin{listing}[htbp]
\caption{Worked examples.
\textbf{Left} ($\mathbb{B}$): implication as a relation (not single-valued), negation as a function, the two negations composing to the identity.
\textbf{Right} ($\mathbb{N}$): divisibility as a relation (not a function), successor as a function, both composing with the same \cppinline{>>}.}
\label{lst:relation_examples}
\noindent
\begin{minipage}[t]{0.49\linewidth}
\begin{cpp}
// Relations on 𝔹, decided by
// exhausting the two values.
using P = pair<bool,bool>;

// ⇒ as a relation, a ≤ b:
constexpr auto imp = Set{
  element<Ω<P>> % UniversalSet<P>{}
  | [](P p){ return !p.first
             || p.second; } };
static_assert(IsRelation<
  decltype(imp),bool,bool>);
// false ⇒ {false,true}: many images
static_assert(
  !IsFunctional<decltype(imp)>);

// ¬ as a FUNCTION, via graph:
constexpr auto neg =
  graph([](bool b){ return !b; });
static_assert(IsFunction<
  decltype(neg),bool,bool>);
// functions compose via >> : ¬∘¬=id
static_assert(
  (neg>>neg)(pair{true,true}));
\end{cpp}
\end{minipage}\hfill
\begin{minipage}[t]{0.49\linewidth}
\begin{cpp}
// Relations on ℕ, decidable where
// the predicate factors finitely.
using Q = pair<ℕ,ℕ>;

// divides: a relation, NOT a fn
// (1 divides every n):
constexpr auto div = Set{
  element<Ω<Q>> % UniversalSet<Q>{}
  | [](Q p){ return p.first
    && p.second%p.first==0; } };
static_assert(
  !IsFunctional<decltype(div)>);

// succ: a FUNCTION, composes via >>
constexpr auto succ =
  graph([](ℕ n){ return n+1; });
static_assert(IsFunction<
  decltype(succ),ℕ,ℕ>);
// succ ∘ succ = "+2":
static_assert(
  (succ>>succ)(pair{2u,4u}));
\end{cpp}
\end{minipage}
\end{listing}
...
If a function lacks a strict structural inverse or is evaluated over an unconstrained, transfinite domain, constructively determining image containment or mapping preimages demands an unbounded search of the domain and is undecidable. Consequently, if a generic result-set construction is impossible for general functions, it is similarly precluded for general relations.
...
\begin{cpp}
// f = succ : ℕ → ℕ, the NNO successor arrow
constexpr auto succ = arrow<ℕ, ℕ>(
  [](ℕ m){ return m + finite_cardinality(1); });

// image of {n > 5} under succ: a Subobject of the codomain ℕ
constexpr auto img =
  image(succ, Set{in<ℕ> | in<ℕ> > bound<5>});
static_assert(IsImageOf<decltype(img), decltype(succ)>);

// ...yet membership is honestly Unknown: the existential
// ∃x. x > 5 ∧ y == succ(x) is undecidable on the transfinite
// carrier without a factorization / retract:
