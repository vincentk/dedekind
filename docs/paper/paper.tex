...
Elliott~\cite{elliott2018simple}.\footnote{The discipline extends to
polynomial calculus: the formal derivative of a rational polynomial
is a \texttt{static\_assert}-checked identity, and the Fundamental
Theorem of Calculus (Part~II) is discharged at translation time
against a closed-form antiderivative. Worked exhibits in the
companion repository's
\texttt{dedekind.algebra:polynomial} and \texttt{dedekind.analysis:ftc}
partitions; \texttt{static\_assert} witnesses pin
$\frac{d}{dx}P(x) = P'(x)$ on rational polynomials and the
trapezoidal-quadrature equality with the antiderivative.}
...
M_2(\mathbb{Q})$ — see, e.g., the standard treatment in any
abstract-algebra reference. The library claims both identities at
the type level over \lstinline{Rational<SignedExtensionalCardinal<>>}
and the compiler discharges them as bit-perfect
\texttt{static\_assert}s, with the defining relation $\varepsilon^2
= 0$ lifting to
...
\begin{lstlisting}[language=C++, caption={Ring-homomorphism identities $\mathbb{C}, \mathbb{D} \hookrightarrow M_2(\mathbb{Q})$, discharged as \lstinline{static_assert}s. From \texttt{embeddings\_test.cpp}.}, label={lst:lin-alg-embeddings}]
