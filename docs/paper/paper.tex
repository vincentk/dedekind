...
The concrete carriers\footnote{We use ``carrier'' loosely throughout to mean the C++ type $\mathrm{T}$ that realises both the underlying set of a $\Sigma$-algebra and the operator-shape concept fired against it.  In strict universal algebra (Burris--Sankappanavar~\cite{burris1981universalalgebra}), \emph{carrier} names only the underlying set $A$, not the algebra $(A, \Sigma_A)$.  In Liskov / OO terms: textbook universal algebra adopts a HAS-A posture (an algebra is the pair $(A, \Sigma_A)$, carrying its underlying set as a separate component); this paper adopts an IS-A posture (the C++ type $\mathrm{T}$ \emph{is} the algebra, with its underlying-set inhabitation and operator-shape concept both attached as properties of $\mathrm{T}$ via concept opt-ins and trait specialisations --- no \cppinline{Algebra<Set>} wrapper around a separate carrier member).  A sentence like ``the carrier $\mathbb{Z}$ is closed under additive inverse'' is therefore a claim about $(\mathbb{Z}, +, -)$ as a $\Sigma$-algebra, not about the bare set.  Section~\ref{sec:alg-sigma} makes the layering precise.} a programming language offers violate those laws conditionally, sometimes silently:
