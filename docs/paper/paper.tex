...
Loosely spoken and one paragraph: where nlab\cite{nlab} attempt to categorify physics and summarize the results in a wiki, this library aims to take their output as axiomatic and translate it into a mainstream programming language.
If the compiler accepts their axioms then, the compiler should also be able validate or perhaps even derive the corresponding theorems\cite{wadler}.
...
The authoritative textbook in the context of programming might be `Types and Programming Languages`\cite{pierce}.
\footnote{
...
\item Any arrow $f$ with domain $A$ can compose with $id_a$, but that's the same as doing nothing: $f \circ id_A = f$, in particular $id_A \circ id_A = id_A$
\item If two distinct arrows $g$ and $f$ happen to have the same domain and codomain, then they are said to \emph{commute}: $f = g$.
Trivially, this is the case for $id_A = id_A$, but as one starts to compose arrows into complex structures, these equations start to form interesting constraints.
\end{enumerate}
}
The appeal of category theory in programming language research should be obvious: a minimalist type checker does not need to do much more that check whether the output of one function matches the input of another.
A ``sufficiently smart'' optimizing compiler might know about a fair amount of commutation rules and re-write the arrows in a sensible sort of way.

Once this basic rapport is established, once can build out the path to 
\begin{enumerate}
\item cartesian closed categories\cite{insert} (in a way a generalization of sets equipped with singly typed $\lambda$-calculi\cite{insert}), 
\item the elementary theory of the category of sets (ETCS, the siamese twin of conventional ZF-set theory) \cite{insert}, and, 
\item to universal algebra and abstract algebra \cite{insert}.
\end{enumerate}
...
If one wants to target a programming environment that imposes no extra runtime requirements above and beyond the host systems natively supported features, it becomes interesting to look at systems programming languages where the runtime is exactly the one provided for by the host system.
One such language is C++23 \cite{cpp23standard} which increasingly offers a pragmatic middle ground: 
\texttt{module} enforce a strict directed-acyclic build graph,
\texttt{consteval} and \texttt{constexpr} allow for a fair amount of compile-time logic,
\texttt{concepts} are compile-time structural checks,
\emph{non-type template parameters} (NTTP---template arguments that are values, not
...
The desired outcome is to enable checks and processes about as follows: 
...
iv) an element test in an empty set will always return false;
v) transcendental number can not be computed to the last digit, but they can be represented \emph{exactly}.
...
\begin{description}
  \item[\texttt{module}]: named modules enforce a strict directed-acyclic build graph, preventing circular
