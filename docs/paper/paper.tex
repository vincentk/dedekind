...
\begin{enumerate}
  \item $\varnothing$, when $\max_i a_i > \min_j b_j$;
  \item a \lstinline{Singleton<v>}, when $T$ is integral and
        $\max_i a_i = \min_j b_j = v$;
  \item an \lstinline{OrderInterval}~$[\max_i a_i, \min_j b_j]$
        otherwise (where the lower and upper bounds default to the
        carrier's $\bot$ and $\top$ when $m = 0$ or $n = 0$).
\end{enumerate}
\end{theorem}
...
one of the three forms, completing the induction.\qed
\end{proof}
