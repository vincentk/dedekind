...
Where the compiler can decide $x \in S$, membership is two-valued and the subobject classifier is Boolean ($\Omega = \mathbb{B}$); the enumerable carriers such as $\mathbb{N}$ and $\mathbb{Z}$ are of this kind, since a bounded search decides membership.
Where it cannot decide, by Rice's theorem for an opaque predicate, or because the carrier cannot be faithfully enumerated (a float carrier does not denote its reals), the characteristic map is a \emph{partial} function.
...
When it is, for instance an enumerable carrier where a bounded search decides $x \in S$, the classifier is Boolean, $\Omega = \mathbb{B}$.
When it is not, by Rice's theorem or because the carrier cannot be enumerated, the characteristic map is partial and its value lands in the Boolean lifted with an \emph{undecided} value \texttt{U}.
...
\begin{itemize}
    ...
    \end{enumerate}
    \item \textbf{C++ Reification:} $\Omega$ is a pluggable \emph{logic species}. \cppinline{concept IsLogicalSpecies} asks for a type carrying $\Omega$ with its \cppinline{AND} / \cppinline{OR} / \cppinline{NOT} and \cppinline{True} / \cppinline{False}. Two ship: \cppinline{ClassicalLogic} ($\Omega = $ \cppinline{bool}) and \cppinline{TernaryLogic} ($\Omega = $ \cppinline{Ternary}, the Boolean lifted with an \cppinline{Unknown} value, under strong-Kleene connectives). The characteristic map $\chi$ is the predicate of a \cppinline{Set<T, L, P>}, whose \cppinline{operator()} returns $\Omega$; \cppinline{concept IsSubobject} recognises it. Which a carrier receives is the decidability split of Section~\ref{sec:ddk_etcs}: $\mathbb{B}$ where membership is decidable, the lifted Boolean where it is not. Equalizers are named by \cppinline{concept IsEqualizer}, the subobject solving $f = g$.
