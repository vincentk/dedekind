...
The \texttt{:species} partition serves as the foundational \textit{Algebraic Atlas} of the library, providing a stratified registry of structural invariants for C++ machine types and standard library operators. By reifying categorical axioms—associativity, commutativity, and idempotency—as compile-time constants, the library transforms the C++ type system into a formal verification engine. This design follows the combinatorial species theory of Joyal~\cite{joyal1981species, joyal1986analytic}, treating fundamental types not merely as passive sets of values, but as active rules for generating mathematical structure.

\begin{lstlisting}[language=C++, caption={The Morphic Registry: Elevating machine types to formal Species.}]
/** 
...
// Verification of structural maturation
static_assert(IsCommutative<int, std::plus<int>>,
              "Commutative: Integer addition is commutative.");
\end{lstlisting}

As illustrated in Table~\ref{tab:structural_invariants}, the ``Feature Cube'' acts as a formal ledger mapping algebraic properties to the language's fundamental atoms. This explicit mapping allows the \texttt{dedekind} dispatcher to distinguish between idealised algebraic structures (e.g., \texttt{unsigned int} modular addition) and the messy inhabitants of the hardware, such as floating-point operations which exhibit numerical non-associativity under the IEEE 754 standard~\cite{ieee754_2019}.

The core objective is to anchor the C++ type system as a substrate for modeling truths obtained from category theory and abstract algebra. In this structuralist setup, providing the axiomatic truths (e.g., that $a \lor b$ is commutative) or documenting their absence (e.g., $NaN$ singularities or rounding errors) becomes the explicit responsibility of the registry. By anchoring these operators as first-class categorical entities, we enable the compiler to act as a physical guardrail, ensuring that structural integrity is verified before the final binary is generated.
...
The \texttt{:morphisms} partition establishes the primary inhabitant of the \texttt{dedekind} topos: the \textbf{Arrow} (or Morphism). While traditional C++ development treats functions as opaque, executable blocks, this partition reifies them as formal mappings $f: A \to B$ equipped with domain and codomain metadata. By elevating the function object into a structural entity, we move from the procedural "how" to the categorical "what," enabling the library to reason about the composition and validity of transformations at compile-time.

A core mechanism of this partition is \emph{Signature Discovery}. Utilizing C++23 \texttt{std::invoke\_result\_t} and \texttt{template} metaprogramming, the library automatically extracts the domain and codomain of raw lambdas, function pointers, and functors, registering them into the categorical atlas without intrusive boilerplate. This metadata acts as the ``connective tissue'' for the algebraic structures in Level 1 and the functorial lift in Level 2. By applying \texttt{static\_assert} against the \texttt{IsArrow} concept, we ensure that every mapping is well-formed—possessing a verifiable signature—before it can be used as a taxonomic brick in the broader ontology.

\begin{lstlisting}[language=C++, caption={Signature Discovery and the Morphic Metadata.}, label={lst:morphism_discovery}]
// Automatic inference of f: Domain -> Codomain
template <typename F, typename... Args>
struct infer_morphism {
  using Domain = std::tuple_element_t<0, std::tuple<Args...>>;
  using Codomain = std::invoke_result_t<F, Args...>;
...
// Reification of a raw lambda into a formal Arrow
auto logic_gate = [](int x) -> bool { return x > 0; };
static_assert(IsArrow<decltype(logic_gate), int, bool>);
\end{lstlisting}
