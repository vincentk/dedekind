...
On this basis the paper specifies its target exhibit: the dual of a shortest-path problem over a directed chain network, cast entirely in the type system.
The dual potentials become a compile-time set comprehension and the coboundary constraints unroll over the totally unimodular network polytope, so the feasible region is a type rather than a runtime data structure.
Because the chain's coboundary telescopes, the optimum is expected to fold at translation time to a typed constant, exactly as the set-comprehension exhibits already collapse an empty intersection to a literal in the emitted IR.
Re-running the same reduction over a dual-number ring is specified to carry first-order edge-cost sensitivity alongside the optimum, as a side effect of the ring laws.
A synthetic source-and-IR exhibit fixes this target concretely, so the narrative arc doubles as the specification the implementation is written to meet.
...
While the binding of concrete operator-shape signatures such as $\neg, \vee, \wedge, \cup, \cap, +, \times, \ldots$ is discussed in later sections~\ref{sec:cpp-stlc-mapping}, \ref{sec:ddk_comprehension}, we can already name some familiar examples: the Booleans $\mathbb{B}$ with logical operations, the integers $\mathbb{Z}$ with addition and subtraction, and so on.
...
These are C++23 types and values in the disciplined fragment $\mathbf{Jlt}$ (§\ref{sec:jlt_posture}) that simultaneously carry universal-algebra structure $\mathbf{Alg}(\Sigma)$ (§\ref{sec:alg-sigma}) for some signature $\Sigma$ the carrier supplies operators for, with the size restriction from §\ref{sec:alg-sigma} carried through.
...
Each step adds one structural commitment, and each commitment is reified as an existing C++23 partition or concept, so the chain is reproducible by reading the partition DAG.
...
The no-type-erasure stance from §\ref{sec:jlt_posture} is \emph{mechanical}: it preserves the concrete type-level information the concept checker and the LLVM backend rely on, and is what buys the compile-time decidability and pruning power the rest of the paper trades on.
...
Where the Birkhoff closure above is the \emph{what}, the propagation mechanism is the \emph{how}.
...
Listing~\ref{lst:extensional_function_application} exhibits how the inner product bracket $\langle y \mid x \rangle$, transfinite function kets, and right-multiplied dyadic projection vectors are reified cleanly via natural operator overloading under the \emph{Juliet Posture}.

The design patterns governing these functional vector geometries manifest concretely through compile-time expression templates.
Listing~\ref{lst:extensional_function_application} exhibits how transfinite functional fields, adjoint transposition ($\sim$), and point-free dyadic operators ($\lvert x \rangle \langle y \rvert$) are reified cleanly via operator overloading within the \emph{Juliet Posture}.
...
The \cppinline{Dual<F>} carrier drops into any NTTP-parameterised carrier without special-casing.

\paragraph{Specification: compile-time resolution and its sensitivity.}
The blueprint above declares the feasible potentials; the exhibit that \emph{resolves} them is the target this paper specifies rather than a checked-in result.
Its shape is fixed synthetically in Listings~\ref{lst:chain-dual-spec-src} and~\ref{lst:chain-dual-spec-ir}.
Because the chain's coboundary telescopes, the optimum $d_N - d_0$ equals the sum of edge costs, so with NTTP costs it must fold at translation time to a typed constant, in the same way the empty-meet witness of §\ref{sec:ddk_comprehension} folds to \texttt{ret i1 false}.
Swapping the cost ring $\mathbb{Q} \to \mathbb{Q}[\varepsilon]/(\varepsilon^2)$ carries the first-order edge-cost sensitivity in that same typed constant, since each cost enters the telescoped sum linearly.

% SPEC / IMPLEMENTATION TODO (2026-08-17): Listings lst:chain-dual-spec-src and
% lst:chain-dual-spec-ir are SYNTHETIC. They fix the target the chain-network
% kernel must meet; they are NOT checked-in exhibits. The maximize / perturb_edge
% surface and both @witness_chain_dual_* IR functions are not yet implemented.
% This is the deliberate arc-as-spec seam. Copilot / reviewers: do not flag as a
% claim-vs-code mismatch; it is intentionally forward-looking implementation work.
\begin{listing}[htbp]
\caption{\textbf{Specification (synthetic, not yet implemented).}  Compile-time resolution of the chain-network dual and its edge-cost sensitivity.  The optimum $d_N - d_0$ folds to a typed constant because the coboundary telescopes; swapping the cost ring to \cppinline{Dual<Rat>} carries the first-order sensitivity in the same constant.}
\label{lst:chain-dual-spec-src}
\begin{cpp}
// SPECIFICATION (synthetic): the target the implementation must meet.
// Objective: the terminal potential gap d_N - d_0, maximised over the
// feasible dual potentials DualLPBasis of the listing above.
constexpr auto gap     = [](auto d) { return d(bound<4u>) - d(bound<0u>); };
constexpr auto optimum = maximize(DualLPBasis, gap);

// The chain's coboundary telescopes: d_N - d_0 = sum of edge costs.
// Unit costs, N = 4 edges -> the optimum folds to a typed constant.
static_assert(optimum == bound<4u>);

// Ring swap Q -> Dual<Q>: first-order edge-cost sensitivity for free.
using D = Dual<Rat>;   // Rat = Rational<...>, exact Q
// Perturb the (2 -> 3) edge cost by epsilon; all other costs stay unit.
constexpr auto optimum_eps =
    maximize<D>(perturb_edge<2u, 3u>(DualLPBasis), gap);

// primal d_N - d_0 = 4;  tangent  d(d_N - d_0)/dc_{2,3} = 1
static_assert(optimum_eps == D{Rat{4}, Rat{1}});
\end{cpp}
\end{listing}

\begin{listing}[htbp]
\caption{\textbf{Specification (synthetic).}  The LLVM IR the kernel of Listing~\ref{lst:chain-dual-spec-src} must fold to at \texttt{-O2}: the optimum and its edge-cost sensitivity as literal constants, with no solver left in the emitted binary.}
\label{lst:chain-dual-spec-ir}
\begin{cpp}
; optimum d_N - d_0 = sum of unit edge costs = 4
define noundef i64 @witness_chain_dual_optimum() {
  ret i64 4
}

; primal + tangent packed as one constant: (d_N - d_0, d/dc_{2,3})
define { i64, i64 } @witness_chain_dual_sensitivity() {
  ret { i64, i64 } { i64 4, i64 1 }
}
\end{cpp}
\end{listing}

The two listings are synthetic: they name the \cppinline{maximize} and \cppinline{perturb_edge} surface and the two IR witnesses the implementation is written to satisfy, not results already checked in.
This is the arc-as-specification seam.
The reduction is fully determined by the telescoping structure and the ring laws; building the kernel that emits exactly this IR is the remaining implementation work.
...
Table~\ref{tab:benchmarks} details the representative performance metrics collected during these trials, contrasting the total compilation duration and peak compiler memory footprint against the final execution clock cycles per optimization pass.
