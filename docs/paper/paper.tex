...
The \emph{Juliet Posture} ($Jlt$) is therefore defined as a disciplined fragment of standard \Cpp{23} which is characterized by constraints that force standard code to behave like an ideal type system:
...
The Juliet Posture embraces the third school, leveraging language features like \texttt{concept}, \texttt{constexpr}, and non-type template parameters (NTTPs).
...
A sketch of some desired functionality to be supported by the set comprehension DSL is provided in Figure~\ref{fig:hub-spoke-bnz}.
...
A common implementation approach follows the material Zermelo-Frenkel set theory, where the discussion often starts from a set membership operation $\in$.
Common software implementations such as \cppinline{std::set} use this as a starting point and proceed to model sets as extensional memory containers where the membership test is carried out by checking if a given value can be found in the container. 
...
\begin{enumerate}
\item A \textbf{Small Category} is modeled by instances of a fixed base type \cppinline{A} with stable identity beyond pointer equality (e.g. \cppinline{std::regular}) such that \cppinline{a == b} and morphisms are pure functions \cppinline{(f(a) == f(b)) == (a == b)} as a base-line implementation\footnote{The category is automatically small because physical constraints on the runtime environment will result in a finite runtime size.}. 
This way, a notion of equational reasoning is enabled.
\item Generalized predicates are modeled as arrows $P : A \to \Omega$, where $A$. In $\mathbf{Set}$, $\Omega = \{\mathrm{true}, \mathrm{false}\}$, i.e. a mapping to the boolean lattice $\mathbb{B}(\land, \lor, \neg)$.\footnote{Additionally, \texttt{dedekind} supports a generalization to logical lattices such as $\mathbb{K}_3(\mathrm{min}, \mathrm{max}, \neg) = \{-1, 0, 1\}$ (to capture the notion of computability) or probability measures, although a detailed discussion is beyond the scope of this report.} In the context of a given small category, $P$ is called the \textbf{Characteristic Predicate} and $\Omega$ is called the \textbf{Subobject Classifier}.
\item \ldots
\end{enumerate}
