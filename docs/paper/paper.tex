...
3D oblique-projection lattice; only the eight cube corners are drawn
explicitly.  Three orthogonal axes, each parameterised by a Fraktur-letter
functor: $\mathfrak{A}$ (carrier-strength) — the chain $\mathbb{B} \to
\mathbb{N} \to \mathbb{Z} \to \mathbb{Q} \to \mathbb{R} \to \mathbb{C}$
via successive functors (Frac, Cuts, Cplx;
Tables~\ref{tab:carriers-exact}--\ref{tab:rosetta-textbook-vs-cpp});
$\mathfrak{R}$ (rank) — Scalar $\to V_n(R) \to M_n(R)$ via
$\mathrm{Free}_n$ and $\mathrm{End}_R$ (Table~\ref{tab:rosetta-rank});
$\mathfrak{Q}$ (quotient) — base ring $R \to X(R)$ at a degree-2
polynomial ideal, $X \in \{D, C, D \circ C, \ldots\}$
(Table~\ref{tab:rosetta-quotient}).
The bottom-front-left corner $\mathbb{B}$ is the lattice bottom
(weakest carrier, no rank lift, no extension); the top-right-back corner
$M_2(X(\mathbb{C}))$ is the lattice top.  Intermediate lattice points
(e.g.\ $V_2(\mathbb{Z})$, $D(\mathbb{Q})$, $\mathbb{F}_{64}$) live along
the edges and are itemised in the per-axis rosetta tables.
\emph{Interpretive choice:} where the carrier axis stops is partly
conventional.  Placing $\mathbb{C}$ at the front-right corner treats
$\mathbb{C}$ as a textbook carrier in its own right; placing $\mathbb{R}$
there would treat $\mathbb{C}$ as a back-face quotient
$C(\mathbb{R}) = \mathbb{R}[i]/(i^2 + 1)$; placing $\mathbb{Q}$ there
would treat both $\mathbb{R}$ (via $\mathrm{Cuts}$) and $\mathbb{C}$
(via $\mathrm{Cplx} \circ \mathrm{Cuts}$) as quotient-axis nodes.
All three readings are equally valid; the figure encodes one such
choice (ℂ-as-carrier).  The carrier-vs-quotient line is partly
interpretive --- §\ref{sec:carrier-rosetta} discusses both readings;
the universal-algebra (A, F) anchor of
Burris-Sankappanavar~\cite{burris1981universalalgebra} treats
\emph{carrier} formally as the underlying-set component, leaving the
construction-vs-primitive choice to the modeller.
\emph{Commutation:} not all axes commute --- \textit{(i)} $\mathfrak{R}$
and $\mathfrak{Q}$ always commute ($M_n(X(R)) \cong X(M_n(R))$ when
both are defined); \textit{(ii)} different $\mathfrak{Q}$ values commute
when their generating variables are independent
($D \circ C \cong C \circ D$); \textit{(iii)} $\mathfrak{A}$ and
$\mathfrak{Q}$ commute partially --- e.g.\ $\mathrm{Frac}(D(R))$ is
undefined ($D(R)$ has nilpotents) but
$\mathrm{Frac}(C(R)) \cong C(\mathrm{Frac}(R))$; \textit{(iv)}
$\mathfrak{A}$ and $\mathfrak{R}$ commute restrictively
($\mathfrak{R}$ produces modules, not rings, so subsequent
$\mathfrak{A}$ steps may not type-check).  Specific compile-status
per cell is pinned in the trait registry (\#499); the figure shows
the structural shape.
\emph{Off-figure dimensions:} higher rank ($n > 2$; \#368),
predicate-set / order-theoretic constructions (candidate sibling
Figure~1b), iterated $\mathfrak{Q}$ (jet bundles; \#504), Honest-Rejection
machine-substrate boundary (\texttt{bool} / \texttt{uint} / \texttt{int}
/ \texttt{double} embed into $\mathbb{B} / \mathbb{N} / \mathbb{Z} /
\mathbb{Q}$ via named \texttt{embed\_*} arrows in
\texttt{dedekind.numbers}).  The 2D projection is necessarily lossy.
\emph{Structural caveat:} the algebraic lattice has a unique bottom
($\mathbb{B}$) but \emph{no} unique top --- iterated quotients, tensor
products, Witt vectors, $p$-adic completions and similar each yield a
strictly larger object without a canonical maximum.  Binary meets
(largest shared substructure) are easy to identify; binary joins
(smallest common extension) exist categorically as pushouts but are
not unique without ambient choice, so in practice the structure is
better thought of as a bounded-below lattice (or, less formally, a
meet-semilattice with constructive joins).  The visual "top-right-back"
corner $M_2(X(\mathbb{C}))$ is therefore an \emph{illustrative
stopping point}, not a categorical maximum --- the figure is a finite
slice for clarity of exposition.}
...
distinct axes; the rosetta tables that follow itemise each axis.
