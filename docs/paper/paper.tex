...
$\mathbb{N}$ unbounded & \lstinline|Cardinality| (variant over $\aleph_0$) & \lstinline|IsAbelianGroup<T,+>| (saturating) \\
...
\lstinline|int|, \lstinline|long| & Partial (UB on signed overflow) & \lstinline|HasRingOperators|; strict \lstinline|IsRing| refuses (math-wins). Use \lstinline|SignedCardinality| / \lstinline|SignedExtensionalCardinal<>| / \lstinline|:partial| Maybe-monad for total alternatives. \\
\lstinline|int8_t|, \lstinline|int16_t| & Partial + SIMD-vectorisable & Same as above; SIMD lanes use the partial-group reading. \\
