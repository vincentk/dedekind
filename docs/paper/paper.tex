...
On this basis the paper works its exhibit: the critical path of a directed acyclic network, computed by semiring closure and cast entirely in the type system.
The graph is a relation, expressed intensionally as a predicate rather than a stored edge list, and its closure over a semiring resolves the critical path, so the feasible structure is a type rather than a runtime data matrix.
Because that closure is a bounded fold over the semiring, the optimum folds at translation time to a typed constant, exactly as the set-comprehension exhibits already collapse an empty intersection to a literal in the emitted IR.
Re-running the same reduction over a dual-number ring carries first-order edge-cost sensitivity alongside the optimum, the critical activities recovered as a side effect of the ring laws.
Crucially, this compile-time collapse is \emph{optional}: the identical strongly-typed reduction runs at run time when the graph arrives as runtime data, so the compiler acts as a partial evaluator, specialising one expression across many graph problems while leaving a runtime-parametric continuation.
...
Pinning the object-size at $\beth_1$ admits the transfinite carriers required for Dedekind-cut arithmetic ($\mathbb{R}$, §\ref{sec:numeric-tension}) and the continuum-sized $\mathbb{C}$ and $\mathbb{D}$, while keeping a foundational ceiling that forbids the self-referential paradoxes of the proper-class meta-categories $\mathbf{Set}$ and $\mathbf{Cat}$.
...
Each edge relaxes one potential, $d[\mathrm{head}] := d[\mathrm{head}] \oplus d[\mathrm{tail}] \otimes c$; one pass in topological order suffices on a DAG.
...
constexpr S semiring_closure(std::size_t source, std::size_t sink,
  ...
  FiniteNet<S, Cap> d{};                     // the potential net, 0-bar everywhere
...
\cppinline{MaxPlus<T>} aliases the longest-path direction, with $\infty$ the saturating sentinel the extended cardinal's $\aleph_0$ already plays.
...
\end{equation}
The cost of arriving at $(x', y')$ is $y' \cdot (K - x')$, which is $0$ on the axis, so merges are free, and $\pm(K - x')$ at the peaks and valleys.
...
constexpr FiniteNet<size_t, Cap> pred = annotate<MaxPlus<long long>, Cap>(src, edges, cost);
...
The necklace has $2^{k}$ distinct source-to-sink paths for its $k$ diamonds, exponential in the graph, yet the closure resolves the critical path in polynomial time and never enumerates a path ($\mathcal{O}(V^{3})$ as written; $\mathcal{O}(V + E)$ with a topological pass).
...
constexpr auto opt = semiring_closure<MaxPlus<long long>, Cap>(
...
int64_t critical_between(std::size_t source, std::size_t sink) {
  return semiring_closure<MaxPlus<long long>, Cap>(
      source, sink, edges, cpm_cost).val;    // source, sink runtime
}
\end{cpp}
\end{listing}
