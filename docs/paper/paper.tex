...
Building upon the total mappings established in Level 1a, the \texttt{:algebra\_total} partition materializes the ``Pure Ideals'' of abstract algebra.
...
A key architectural decision in \texttt{dedekind} is the decoupling of \emph{associativity} from \emph{invertibility}: while a \textbf{Monoid} provides an identity to an associative Semigroup, a \textbf{Loop} provides an identity to a Quasigroup \cite{bruck1971survey}.
A \textbf{Group} is thus reified as the perfect synthesis of these two paths, as shown in Table~\ref{tab:algebraic_hierarchy}.

By mapping these structures to \texttt{C++23} concepts, the library transforms the compiler into a \textbf{Constraint Satisfaction Engine} (CSE) that verifies mathematical truth as a physical invariant of the binary \cite{benton1998computational}.
This leads to the \textbf{Honest Rejection} illustrated in Listing~\ref{lst:abelian_group}: while \texttt{unsigned int} addition is verified as a total Abelian Group ($\mathbb{Z}/2^n\mathbb{Z}, +$) due to its bit-perfect periodicity, signed \texttt{int} addition is rejected even as a total Magma.
Because signed overflow constitutes undefined behavior, the operation fails to satisfy the requirements for periodicity or idempotency within the total domain.
The library correctly identifies such operations as \textbf{Hazards} \cite{pomerance2001smooth}, shifting them from the Platonic realm of Level 1 into the pragmatic ``Rescue'' logic of the \texttt{:partial} partition.
...
// No Periodicity, No Idempotency; identified as a Hazard/Partial function.
static_assert(!IsMagma<int, std::plus<int>>);

// The compiler now "knows" the algebra of the machine.
\end{lstlisting}


\subsubsection{Monoid Actions and Modules (\texttt{:actions})}

The \texttt{:actions} partition materializes the concept of a Species being acted upon by an Algebraic Structure. This partition defines the \texttt{IsSemiModule} and \texttt{IsModule} concepts, where a \textbf{Monoid} (from \texttt{:algebra\_total}) defines a transformation rule over a target Species. 
...
Following the functional tradition of the ``Fish Operator'' (\texttt{>=>}) \cite{milewski2018}, \texttt{dedekind} adapts this to the C++23 context by leveraging the left-associativity of \texttt{operator>>} to model the directional vector of computation across the ontology.
...
The final stage of the foundational build anchors Lawvere's Elementary Theory of the Category of Sets (ETCS) \cite{lawvere1964}. This partition defines a set not by its internal elements, but by its universal properties, utilizing C++23 Concepts to enforce well-pointedness and the existence of power objects.
...
// Materialize the set at compile-time as a structural type
using EvenGreaterTen = dedekind::set<int, P>;

static_assert(dedekind::contains<EvenGreaterTen>(12));
\end{lstlisting}
