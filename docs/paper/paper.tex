...
The optimum is the filtration of Equation~\ref{eqn:argmax}, realized to an extensional set only at the end by \cppinline{materialize}, gated on \cppinline{IsExtensional}: finite and addressable, not merely finite.
\cppinline{materialize} is the realize boundary, our \cppinline{runST}~\cite{launchbury1994st}: the enumeration runs inside and \cppinline{constexpr} seals it, and \cppinline{IsExtensional} is the type-level guarantee that makes extraction sound, the role the rank-2 $\forall s$ plays for state locality played here for finiteness.
...
constexpr auto opt = materialize(argmax(omega_N | interval(0,6), cap));  // {3}
\end{cpp}
\end{listing}

The joint is not a black box: which matrix a relation materialises to is fixed by its \emph{classification}, so \cppinline{materialize} is a structure-preserving functor rather than a dense dump (Table~\ref{tab:relation-matrix-map}).
...
\begin{table}[htbp]
\centering
\caption{The relation-to-matrix map: \cppinline{materialize} as a structure-preserving functor, the relation's classification fixing the matrix type. The dense and rank-1 fibers are implemented; the rest are read off the inferred four properties.}
