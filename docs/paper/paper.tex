...
Right: the characteristic-map axiom (\cppinline{IsSubobject}, the classifier $\chi$ recognised as a carrier's call shape into $\Omega$) and the composite predicate \cppinline{IsSet}, which conjoins the ten ETCS axiom witnesses.
...
\begin{cpp}
// characteristic-map axiom: a subobject
// whose call shape is the classifier chi
template <typename S, typename A>
concept IsSubobject = requires(S s, A a) {
  { s(a) } -> LogicalValue;  // chi: A -> Omega
};

// a Jlt carrier that is an ETCS set:
// ten axiom witnesses, plus Set is a CCC
template <typename A>
concept IsSet =
  HasETCSAxioms<A> &&
