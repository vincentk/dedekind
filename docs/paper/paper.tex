...
\end{aligned}
\end{equation}
In code the comprehension is \cppinline{set(S,P)} and each bound is an \cppinline{operator==} overload: scheme (A) reads \cppinline{exists(S,P)} as \cppinline{!(Ø{} == set(S,P))}, scheme (B) reads \cppinline{forall(S,P)} as \cppinline{Ω == set(S,P)}, and each derives its partner by complementing the predicate, \cppinline{!P} (Listing~\ref{lst:quantifiers_def}, top row).
There is no fold: the reduction lives entirely in \cppinline{Ø::operator==}, so the Rice boundary is a \emph{type check}, an operand with no matching overload being a compile error rather than a fabricated answer.
Both quantifiers are decidable in finite time exactly when the predicate factors through a \emph{finite quotient} of the domain, and the emptiness overload is then a proof by exhausting that quotient.
A finite carrier is the trivial case: over $\mathbb{B}$, \cppinline{set} materialises the two truth values and \cppinline{Ø ==} reads them off.
A periodic predicate earns the same finiteness on an infinite carrier: \cppinline{isEven} factors through $\mathbb{N} \twoheadrightarrow \mathbb{Z}/2\mathbb{Z} = $ \cppinline{Modular<2>}, so $\exists$ and $\forall$ reduce to its two residues (Listing~\ref{lst:quantifiers_def}, bottom row).
The opaque lambda cannot be seen as periodic; the structural congruence, carrying its modulus in the type, can.

\begin{listing}[htbp]
\caption{The quantifiers \emph{are} set operations (Eqn~\ref{eqn:quantifiers}), decided \emph{by type} through a finite quotient.
\textbf{Top:} the two lattice bounds: \cppinline{exists} tests \cppinline{set(S,P)} against $\varnothing$ (A), \cppinline{forall} against $\Omega$ (B); each derives its partner by complementing the predicate, \cppinline{!P}.
\textbf{Bottom:} over $\mathbb{B}$ (a finite carrier) \cppinline{set} materialises the two truth values; over $\mathbb{N}$, \cppinline{isEven} factors through $\mathbb{Z}/2\mathbb{Z} = $ \cppinline{Modular<2>}, so $\exists$ and $\forall$ exhaust its two residues.  Neither enumerates $\mathbb{N}$.}
...
\begin{minipage}[t]{0.49\linewidth}
\begin{cpp}
// ℕ = Ω<Cardinality> : isEven is
// periodic → factors thru Modular<2>
constexpr auto ℕ = Ω<Cardinality>;
constexpr auto isEven =
  in<ℕ> % Modular<2>{} == bound<0>;
static_assert( exists(ℕ, isEven));
static_assert(!forall(ℕ, isEven));
\end{cpp}
\end{minipage}
\end{listing}
