...
static_assert(max(ℤ | (π <= fix(5_c)))(5));
static_assert(min(ℤ | (π >= fix(5_c)))(5));
\end{cpp}
\end{minipage}
\end{listing}
...
\cppinline{preimage} and \cppinline{dom} read the $\pi_A$ side and never leave the domain, so they cost nothing beyond a forward call: $\mathrm{preimage}(f, S) = \{a \mid f(a) \in S\}$ is apply-and-test, decided by single-valuedness alone, and $\mathrm{dom}$ is all of $A$ the moment $f$ is entire.
\cppinline{image} and \cppinline{cod} read the $\pi_B$ side and have to cross back: $\mathrm{cod}$ is all of $B$ once $f$ is surjective, and $b \in \mathrm{image}(f, S)$ is the question $\exists x \in S.\, f(x) = b$, which injectivity turns into the single lookup $f^{-1}(b) \in S$.
