...
\usepackage{threeparttable}             % table notes that travel with the table (survive landscape floats)
\usepackage{tabularx}
\usepackage{booktabs}
...
\begin{table}[p]
\centering
\caption{The structuralist bridge.  Each row reads three ways: a System F property (logic / type-theory tradition, Pierce~\cite{pierce2002tapl}~ch.~23, 29 + Wadler~\cite{wadler1989theorems}); the Juliet-Posture restriction in C++23 that recovers it; and the corresponding Bourbaki-structuralist move (Marquis~\cite{marquis2022structuralist}; Bourbaki's \emph{L'Architecture des mathématiques} 1948).  Each row is a \emph{correspondence}, not a strict equivalence: the C++ discipline recovers the \emph{flavour} of the System F property on the disciplined fragment.  Some rows are close (type erasure, universal quantification); others are partial, and the footnotes mark where the analogy stops.  The C++ column states the \emph{discipline} an engineer adopts; the consequences live in the outer columns.}
\label{tab:structuralist-bridge}
\begin{threeparttable}
\begin{tabular}{@{}p{0.4\textwidth}p{0.7\textwidth}p{0.4\textwidth}@{}}
\toprule
\textbf{System F property} & \textbf{Juliet Posture (C++)} & \textbf{Structuralist style (Bourbaki)} \\ \midrule
\textbf{Parametricity} (\emph{Theorems for Free!}~\cite{wadler1989theorems})\tnote{a}  &
...
\textbf{Strong normalisation} (termination)\tnote{b}  &
...
\textbf{Impredicativity} (types quantified over types)\tnote{c}  &
...
\item[a] C++23 concepts realise parametricity in an \emph{implicit-style} discipline: the constraint is checked at instantiation but no concept witness is passed at runtime.  This is closer to Haskell-style type classes than to pure System F's explicit type abstraction.  The admissibility rules exclude ad-hoc \emph{dispatch} (SFINAE, ADL beyond textbook operators, public specialisation; Table~\ref{tab:admissibility-rules}), but not body-level type inspection: a constrained function may still branch on \cppinline{std::is_integral_v<T>}.  Full parametricity is therefore a discipline on the disciplined fragment, not a property the compiler discharges.
\item[b] C++ template instantiation is Turing-complete in general~\cite{veldhuizen2003templates}; Table~\ref{tab:admissibility-rules}'s bounded-instantiation-depth rule caps \emph{type-level} expansion; it is not full strong normalisation.  Over-deep instantiation ends in a compiler depth error rather than a normal form, and \cppinline{constexpr} / runtime recursion is outside the rule's scope.  The analogy is to the terminating type-level fragment, not to System F's normalisation theorem.
\item[c] System F impredicativity instantiates type variables with universally-quantified types.  The C++ analogue here is weaker: concepts \emph{composed from} concepts (higher-order constraints), and these range over concrete types (\cppinline{IsFunctor<F>} takes a \cppinline{typename F}), not over $\forall$-types.  We keep the row as a structural analogy (``structures built on structures''), not an identity.
\end{tablenotes}
\end{threeparttable}
\end{table}
\end{landscape}
