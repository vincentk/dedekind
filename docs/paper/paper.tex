...
The library reifies the H and P operations of the HSP theorem~\cite{burris1981universalalgebra} (§II.10)\footnote{H, S, P are the three structure-preserving operations on algebras: \textbf{H}omomorphic images, \textbf{S}ubalgebras, direct \textbf{P}roducts.  A class is a variety iff it is closed under all three.  S is not heavily exercised by today's carriers and is left absent.  $\operatorname{End}_R$ at the rank axis is \emph{not} an HSP operation: it is the internal hom in $\operatorname{Mod}_R$, which preserves only some axioms (matrix multiplication is non-commutative even when $R$ is commutative).  Its partial trait propagation lives behind a separate \cppinline{is_endomorphism_ring_v} trait.  The arrow-side cousin to \cppinline{IsQuotientAlgebra} is \cppinline{IsQuotientMorphism} in \texttt{algebra:universal} (a declared homomorphism + declared surjectivity, naming the canonical projection $\pi: A \to A/{\sim}$).} as marker-trait + detection-concept pairs in \texttt{algebra:quotient} (Table~\ref{tab:hsp-reification}).
...
\begin{table}[ht]
\caption{HSP reification in \texttt{algebra:quotient}.  Each row maps a structure-preserving operation to its marker trait, detection concept, and a representative carrier-site witness.  S is not heavily exercised today; $\operatorname{End}_R$ at the rank axis is not an HSP operation (see footnote).}
