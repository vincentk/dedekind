...
\begin{listing}[!ht]
\caption{The two layers of $\mathbf{Ddk} = \mathbf{Trsk} \cap \mathbf{Alg}$.
\textbf{Left, the type layer} (\cppinline{category}): the pointwise axioms on a carrier \emph{type} --- a model of a Birkhoff/Lawvere theory~\cite{burris1981universalalgebra,hylandpower2007}, a magma refined to a monoid by associativity and a unit.
\textbf{Right, the set layer} (\cppinline{algebra}): a set \emph{object} whose carrier is such a model, the subobject-over-carrier pattern familiar from proof assistants~\cite{mathlib2020}.  \cppinline{IsAlgebraOnSet} is the generic $(A,F)$ base ($\mathbf{Trsk}$ met with carrier closure); each named rung --- here \cppinline{algebra::IsMagma<X>} --- is that base refined by the pointwise axioms of the left column.}
...
set layer (\cppinline{algebra}): the lift
\begin{cpp}
// generic (A,F) base:
...
// the named magma rung:
template <typename X,
  typename Op = std::plus<typename X::Domain>>
concept IsMagma = IsAlgebraOnSet<X, Op>
  && category::IsMagma<typename X::Domain, Op>;
\end{cpp}
\end{minipage}
\end{listing}
...
\begin{listing}[!ht]
\caption{Primitive carriers as \emph{algebraic sets}, inhabiting the set-indexed concepts of Listing~\ref{lst:algebra:universal}.  \textbf{Left:} $\mathbb{B} = \Omega\langle\texttt{bool}\rangle$ needs \emph{explicit} operators --- a Boolean rig under logical $(\vee,\wedge)$, its carrier the Galois field $\mathbb{F}_2$ under bitwise $(\oplus,\wedge)$.  \textbf{Right:} $\mathbb{S} = \Omega\langle\texttt{size\_t}\rangle$ is a commutative ring under the \emph{default} arithmetic $(+,\times)$, so the operators are omitted.  The complementary set-lattice layer ($\cup,\cap$) is Listing~\ref{lst:pruning}.
...
// carrier under bitwise (⊕, ∧): 𝔽₂
static_assert(IsGaloisField<decltype(𝔹)::Domain,
    std::bit_xor<bool>,
    std::bit_and<bool>>);
\end{cpp}
\end{minipage}\hfill
