...
More generally, if the mono leg of the factorization carries a \emph{retract} (a partial inverse returning the preimage of a codomain element when one exists), the type system decides membership as a local lookup on the image boundary, with no search of the domain (Listing~\ref{lst:functional_image_bijection}).
...
\begin{listing}[htbp]
\caption{A retract makes image membership decidable.  A monic arrow \cppinline{Double} carries a \cppinline{retract} (a partial inverse $y \mapsto$ preimage, if any); with it, $y \in \mathrm{image}(f, S)$ is decided as ``the retract has a value that lies in $S$'': a local lookup on the image boundary, never a search of the domain (here \cppinline{positive} is the halfspace $\{x>0\}$; $10 \mapsto 5 \in$ \cppinline{positive}, $7 \mapsto$ \cppinline{nullopt}; abridged from the checked-in test).}
\label{lst:functional_image_bijection}
\begin{cpp}
struct Double { using Domain = int; using Codomain = int;
  constexpr int operator()(int x) const { return 2*x; } };
constexpr auto retract(Double) {
  return [](int y) -> std::optional<int> {
    return (y%2==0) ? std::optional{y/2} : std::nullopt; }; }

constexpr auto img = image(Double{}, positive);
\end{cpp}
\end{listing}

\cppinline{Double} is the injective case: its retract is the obvious halving.
