...
\toprule
 & meet & join & complement & bottom & top & order \\
\midrule
$\mathbf{CCC}$ & $\times$ & $\sqcup$ & $\neg$ & $\mathbf{0}$ & $\mathbf{1}$ & $B^A$ \\
$\mathbb{B}$ & $\wedge$ & $\vee$ & $\neg$ & $\bot$ & $\top$ & $\Rightarrow$ \\
$\mathcal{P}(A)$ & $\cap$ & $\cup$ & $\complement$ & $\varnothing$ & $A$ & $\subseteq$ \\
$\mathbf{Trsk}$ & \cppinline{&} & \cppinline{|} & \cppinline{~} & \cppinline{Ø} & \cppinline{Ω} & \cppinline{<=} \\
\bottomrule
\end{tabular}
\caption{
Operator-mapping table: each column is one construction spelled in four languages.
The top row names the universal constructions of a Cartesian-closed category (product, coproduct, negation, initial and terminal objects, exponential).
Each lattice is itself a Cartesian-closed poset, so it realises those constructions concretely: the meet \emph{is} the categorical product, the implication the exponential.
The rows below give the Boolean (logical), powerset (set-theoretic), and $\mathbf{Trsk} \subset \mathbf{Cpp}$ (Section~\ref{sec:ddk_etcs}, Eqn~\ref{eqn:trsk}) readings.
...
\textbf{Top:} the two lattice bounds: \cppinline{exists} tests \cppinline{set(S,P)} against $\varnothing$ (A), \cppinline{forall} against the input set $S$ (B); each recovers its partner by complementing the predicate, \cppinline{!P}.
\textbf{Bottom:} over $\mathbb{B}$ (a finite carrier) \cppinline{set} materialises the two truth values; over $\mathbb{N}$, \cppinline{isDivisibleBy3} factors through $\mathbb{Z}/3\mathbb{Z} = $ \cppinline{Modular<3>}, so $\exists$ and $\forall$ exhaust its three residues.  Neither enumerates $\mathbb{N}$.}
...
The combinators \cppinline{Exists} and \cppinline{ForAll} build on the set-operation definition of §\ref{sec:ddk_comprehension} (Listing~\ref{lst:quantifiers_def}): they bind the inner variable over the domain and leave a predicate in the outer variable, exactly the shape a comprehension turns into a new set (Listing~\ref{lst:quantifiers}).

\begin{listing}[htbp]
\caption{The bounded quantifiers as combinators.  A finite domain given as a \cppinline{std::views} range is itself an \cppinline{IsSet}, since a set is its membership test, so the lift is \cppinline{std::ranges::find}.  \cppinline{Exists} binds the inner variable over that domain, leaving a predicate in the outer variable; every line is a compile-time witness.}
...
Which overload resolves the test is where the two regimes, at two layers, show.
A transparent halfspace meet collapses to $\varnothing$ by type through the \cppinline{dedekind.order} specialization of \cppinline{structured_and}, so $\forall x>5.\,x\ge 3$ is a \cppinline{static_assert} on the empty counterexample $\{x>5 \wedge x<3\}$, reached through the \cppinline{\&} meet rather than \cppinline{set} since it needs the halfspace types.
Over an enumerable domain \cppinline{set(S,P)} is a lazy \cppinline{std::views::filter}, and the same \cppinline{==} is \cppinline{begin == end}, short-circuiting at the first witness (Listing~\ref{lst:quantifier_regimes}).

\begin{listing}[htbp]
\caption{One definition, two regimes selected by \cppinline{Ø::operator==}.  Compile time: the order-specialized \cppinline{\&} meet collapses disjoint halfspaces to $\varnothing$ by type.  Run time: \cppinline{set} filters and emptiness is \cppinline{begin == end}.  An opaque, non-enumerable operand matches no overload: the Rice wall.}
\label{lst:quantifier_regimes}
\begin{cpp}
// (a) transparent: the order-specialized & meet collapses BY TYPE
constexpr auto gt5 = Set{in<ℕ> | in<ℕ> > bound<5>};
constexpr auto lt3 = Set{in<ℕ> | in<ℕ> < bound<3>};
static_assert(Ø<Cardinality>{} == (gt5 & lt3));  // {x>5 ∧ x<3} = ∅

// (b) enumerable: set() is a lazy filter; Ø== is begin==end
std::unordered_set<int> dom{1, 2, 3, 4, 5};
assert( exists(dom, [](int x){ return x >  3; }));  // 4 ∈ dom
assert(!forall(dom, [](int x){ return x >  2; }));  // 1,2 counter
\end{cpp}
\end{listing}
...
\end{equation}
On a finite (\cppinline{IsExtensional}) intermediate domain $B$, that $\exists b$ is exactly the \cppinline{Exists} combinator of \S\ref{sec:intensional_sets} (Listing~\ref{lst:quantifiers}): relative composition is a bounded existential over the middle set.
