...
The split is by the \emph{decidability} of membership, not by cardinality.
Where the membership predicate is decidable, so the compiler can evaluate $\chi(x)$ to a definite $\top$ or $\bot$, membership is two-valued and the subobject classifier is Boolean ($\Omega = \mathbb{B}$).
A transparent rule, such as a comparison or an arithmetic test, is of this kind.
Where the predicate is not decidable, by Rice's theorem for an opaque lambda the compiler cannot inspect, or for an existential over a domain it cannot search, the characteristic map is a \emph{partial} function.
We represent that in the standard way, as a total map into the Boolean extended with one \emph{undecided} value, the lift (partial-map classifier) of $\mathbb{B}$~\cite{moggi1991notions, rosolini1986continuity}.
The truth object $\Omega$ stays Boolean; undecidability lives in the lift, not in a third truth value.
...
\& \text{decidable?} \arrow[dl, "\text{yes}"'] \arrow[dr, "\text{no}"] \& \\
\mathbb{B} \arrow[d, "\chi"] \& \& \mathbb{B}+\text{U} \arrow[d, "\chi"] \\
\{\top, \bot\} \& \& \{\top, \bot, \text{U}\}
\end{tikzcd}
\caption{
The evaluation tracks the \texttt{dedekind} compiler navigates on type inspection.
Carriers failing the $\mathbf{Jlt}$ gate (non-\texttt{std::regular}) are rejected by concept constraints.
The rest are routed by whether membership is \emph{decidable}.
When it is, for instance a transparent rule the compiler can evaluate, the classifier is Boolean, $\Omega = \mathbb{B}$.
When it is not, by Rice's theorem for an opaque predicate or for an existential it cannot search, the characteristic map is partial and its value lands in the Boolean lifted with an \emph{undecided} value \texttt{U}.
That third value is definedness, not a third truth: undecidable queries return a deterministic \texttt{Unknown} rather than risking a translation-time hang.
}
