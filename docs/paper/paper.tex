...
\begin{itemize}
    ...
    \item \textbf{C++ Reification:} The concept \texttt{IsExponential} (completing \texttt{IsCartesianClosed}) gates the structure, and it is carrier-agnostic: any callable \texttt{e} with \texttt{e(a)} of type \texttt{B} satisfies it. This is the rung where the compile-time/runtime split surfaces --- a \texttt{constexpr} lambda \emph{is} the compile-time exponential (usable in constant evaluation), while \texttt{std::function} witnesses the same concept only as the type-erased \emph{runtime} carrier (it is not \texttt{constexpr}-usable). The adjunction arrows \texttt{curry} / \texttt{uncurry} / \texttt{eval} are library-provided --- the currying law exhibited operationally, not yet mechanically enforced.
