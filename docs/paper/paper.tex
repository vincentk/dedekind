...
We further restrict our attention to the \emph{small} slice $\mathbf{Alg}(\Sigma)_{\beth_1}$ --- the $\beth_1$-small algebras, i.e.\ algebras whose underlying carrier has cardinality at most that of the continuum~\cite{maclane1971categories} (Grothendieck-universe convention).
...
The Juliet Posture deliberately rejects type erasure on the categorical surface: an \cppinline{AnyCategory} wrapper would nominally admit higher-categorical abstractions but force a move toward impredicativity and render the categorical witnesses opaque to both the concept checker and the LLVM optimizer, breaking the structuralist gate on which §\ref{sec:compiler-wall} and §\ref{sec:pruning} rest.
...
\paragraph{The intersection's deliberate scope: trade-offs of $\mathbf{Ddk} = \mathbf{Jlt} \cap \mathbf{Alg}(-)_{\beth_1}$.}
The size restriction from §\ref{sec:alg-sigma} (small algebras only) and the type-erasure rejection from §\ref{sec:jlt} compose into a single stratified regime: every claim in $\mathbf{Ddk}$ is a mechanical witness over a small category, and the type-checker's decidability and the optimizer's pruning power both rest on that smallness.
The trade is not free.
The decidability gain --- concept-level checking, structural pruning, named-axiom diagnostics --- is paid for by the engineer's obligation to spell out, by hand, anything that does not fit inside a small category.
Meta-categorical statements (the textbook $\mathrm{Disc} \dashv U$, the size-hierarchy adjunctions, free / forgetful pairs across $\mathbf{Cat}$) cannot land at the type level, since they range over the proper-class meta-categories $\mathbf{Set}$ and $\mathbf{Cat}$~\cite{maclane1971categories}; they survive in $\mathbf{Ddk}$ as documented prose-level CT vocabulary, anchored where possible by mechanically-witnessed \emph{restrictions} to a single small slice.
...
The 2-categorical layer above thus becomes the engineer's honesty obligation, named by reference rather than by type-level proof --- a price for the compile-time decidability the rest of the paper trades on, and one §\ref{sec:compiler-wall} revisits at the wall where the type checker also stops.
