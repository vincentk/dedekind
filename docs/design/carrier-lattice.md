...
```
                      ...
                      ╱     ↑
       embed_sint_ℤ_        │ lift_ℕ_ℤ_  (canonical ℕ ↪ ℤ embedding;
                  ╱         │              Grothendieck-construction unit
                  ╱         │              at object ℕ)
                int  ←──── Cardinality  (ℕ-Form: NNO)
                 ↑          ↑     ↑
        embed_𝕂3_ℤ_         │     │ embed_uint_ℕ_
                 │   embed_uint_sint_    │ (PR #441, the named monic arrow)
                 │   (machine)    │
