...
The library carries two complementary cyclic concepts in the strict / operational pattern (cf.\ \texttt{IsRingLike} vs \texttt{IsRing}):
...
\subsection{Operational witnesses: \texttt{IsRingLike}, \texttt{IsSemiringLike}, \texttt{IsRingLikeHomomorphism}}
\label{subsec:algebra-ring-operational}
...
\texttt{:ring} therefore also exports operational concepts that check closure of the arithmetic operators only:

\begin{itemize}
  \item \texttt{IsRingLike<T>} --- closed $+$, binary $-$, unary $-$, $*$. Parallel to \texttt{IsFieldLikeScalar} but without the $/$ requirement (rings don't in general support division). Fires on \texttt{Rational<long>}, \texttt{int}, \texttt{unsigned int}, \texttt{Complex<R>} (for any \texttt{IsRingLike R}), \texttt{Dual<F>}, and \texttt{Matrix2x2V<R>} over any \texttt{IsRingLike R}---operationally witnessing ``matrices over a ring form a (non-commutative) ring''.
  \item \texttt{IsSemiringLike<T>} --- closed $+$, $*$, structural identities \texttt{T\{\}} and \texttt{T\{1\}}, without subtraction or unary negation. The ``no additive inverse'' carriers: booleans under OR/AND, naturals, tropical semirings.
  \item \texttt{IsRingLikeHomomorphism<Phi, Source, Target>} --- a callable $\phi$ that sends \texttt{Source} to \texttt{Target} with $+$ and $\cdot$ preservation. The structural concept asks that both carriers are ring-like, $\phi(a)$ is well-typed and returns \texttt{Target}, and the two preservation expressions $\phi(a + b) \mathrel{=\!=} \phi(a) + \phi(b)$ and $\phi(a \cdot b) \mathrel{=\!=} \phi(a) \cdot \phi(b)$ are well-formed. Whether each \texttt{==} returns \texttt{true} for all inputs is a value-level claim, witnessed per-instance via \texttt{static\_assert} on concrete probes (cf.\ the $\mathbb{C} \hookrightarrow M_2(\mathbb{Q})$ and $\mathbb{D} \hookrightarrow M_2(\mathbb{Q})$ witnesses in \texttt{dedekind.linear\_algebra:embeddings}).
\end{itemize}

Preservation of distinguished identities ($\phi(0) = 0$, $\phi(1) = 1$) is \emph{not} encoded structurally here: \texttt{IsRingLikeHomomorphism} checks only additive and multiplicative preservation on ring-like carriers and does not require or validate identity preservation, matching the absence of \texttt{T\{\}} / \texttt{T\{1\}} from \texttt{IsRingLike}. Unital laws, when needed, must be witnessed by stricter concepts or on concrete probe values.

\subsection{Shape concepts and the C++/textbook seam: \texttt{HasRingOperators}, \texttt{HasRingOperatorsFor}, \texttt{IsArithmeticRing}}
\label{subsec:algebra-ring-shape-concepts}

Issue \#393 surfaced a recurring need across \texttt{linear\_algebra}, \texttt{optimization}, and the analytic bridges: callsites that require the \emph{operator surface} of a ring (so a templated body can write \texttt{a + b} and \texttt{a * b}) but neither the strict categorical proof nor the operational division surface of \texttt{IsFieldLikeScalar}. The audit identified twenty-three such callsites. The response was a third concept tier --- \emph{pure syntactic shape} --- and a sealing seam between the math textbook and the C++ standard.
...
\begin{itemize}
  ...
  \item \textbf{Operational (\texttt{algebra::IsRingLike<T>}).} Closure of \texttt{+}, binary \texttt{-}, unary \texttt{-}, \texttt{*} as same-type operators. No identities, no axioms.
  \item \textbf{Syntactic shape (\texttt{algebra::HasRingOperators<T>}).} Same operator surface as \texttt{IsRingLike}, but framed explicitly as a \emph{shape} concept --- the document semantics is that the callsite needs the operators to compile and yield \texttt{T}, with no claim of any algebraic structure.
\end{itemize}

The strict concept is the textbook witness; the operational concept is the IEEE-edge-case escape hatch (e.g.\ for \texttt{Rational<long>} under the active numeric policy); the shape concept names the surface a templated body actually consumes. Two siblings refine the surface: \texttt{HasRingOperatorsFor<T, Add, Mult>} parameterises by the chosen functors (so a callsite can ask whether \texttt{std::bit\_xor<T>} / \texttt{std::bit\_and<T>} close on \texttt{T} regardless of what the literal \texttt{+} and \texttt{*} do), and \texttt{IsArithmeticRing<T>} is the \emph{seal}: a carrier that satisfies the strict ring proof under \texttt{(std::plus, std::multiplies)} \emph{and} whose literal \texttt{+}, \texttt{-}, \texttt{*} close strictly on \texttt{T}. \texttt{IsArithmeticRing} is where the math-textbook ring and the standard C++ arithmetic operators agree: writing \texttt{a + b} computes the ring's addition and the result genuinely stays in the carrier.

\paragraph{Where C++ diverges from the textbook.} The bundled \texttt{algebra::IsRing<T, Add, Mult>} composes the strict proof with \texttt{HasRingOperatorsFor<T, Add, Mult>} --- the \emph{functor-parametric} surface. The math-wins-over-C++ stance (per \texttt{README.md}) makes this load-bearing: integer promotion lifts the result of \texttt{unsigned short + unsigned short} to \texttt{int}, so \texttt{unsigned short} is genuinely \emph{not} closed under the literal \texttt{+}. The strict \texttt{HasRingOperators<unsigned short>} correctly refuses, and so does \texttt{IsArithmeticRing<unsigned short>}. But \texttt{std::plus<unsigned short>} and \texttt{std::multiplies<unsigned short>} return \texttt{unsigned short} by signature, so \texttt{HasRingOperatorsFor<unsigned short, std::plus<unsigned short>, std::multiplies<unsigned short>>} fires; combined with the modular-wrap proof at the categorical layer, \texttt{algebra::IsRing<unsigned short>} fires too. The narrow-unsigned scalar is a textbook ring under the right operators; the type system surfaces the disagreement between the textbook and the literal C++ surface as a real distinction. The same pattern handles \texttt{bool} under \texttt{(std::bit\_xor, std::bit\_and)}: \texttt{bool} is the Galois field $\mathbb{F}_2$ under the functor surface, but \texttt{bool + bool $\to$ int} via promotion, so \texttt{HasRingOperators<bool>} refuses --- the right behaviour, since \texttt{bool}'s ring structure lives over different operators than \texttt{+} and \texttt{*}.

Table~\ref{tab:ring-shape-rosetta} catalogues the divergence on the carriers the project actually uses. The pattern: where literal C++ operators agree with the textbook, every column ticks; where they diverge (integer promotion, signed-overflow UB, missing internal product), the table names the precise failure point.

\begin{table}[ht]
\centering
\caption{Rosetta: math textbook $\leftrightarrow$ C++ standard, indexed by carrier and shape concept (Issue~\#393). \textbf{Strict} = \texttt{category::IsRing<T, Add, Mult>} with the conventional functors. \textbf{Functor} = \texttt{HasRingOperatorsFor<T, Add, Mult>}. \textbf{Literal} = \texttt{HasRingOperators<T>}. \textbf{Seal} = \texttt{IsArithmeticRing<T>} = strict $\wedge$ literal under \texttt{(std::plus, std::multiplies)}. ``$\circ$'' marks the operational-only assertion; ``--'' marks not asserted at present.}
...
\begin{enumerate}
  ...
  \item $\mathbb{C}$ and $\mathbb{D}$ as injective ring homomorphisms into $M_2(\mathbb{Q})$ via the \texttt{IsRingLikeHomomorphism} concept, plus $\varepsilon^2 = 0$ lifting to the zero matrix.
\end{enumerate}
...
\begin{itemize}
  ...
  \item \texttt{Matrix2x2<T, a, b, c, d>} (possibly-singular NTTP peer, bound: \texttt{IsRingLike<T>}), \texttt{Identity2x2<T>}, \texttt{Zero2x2<T>}.
