...
The refactoring does not change the public API surface or the behavior of the Mandelbrot approximation tower; only the implementation path is improved.
...
\begin{itemize}
  ...
  \item \textbf{Overloads}: \texttt{divergence\_count\_in\_window} now accepts both:
    \begin{enumerate}
      \item Generic form: \texttt{(window, escape\_criterion)} for arbitrary criteria (e.g., alternative norms, adaptive thresholds).
      \item Specialized form: \texttt{(window, radius\_squared)} for backward compatibility with existing code.
    \end{enumerate}
\end{itemize}
