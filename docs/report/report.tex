...
\begin{enumerate}
  \item \textbf{Parametrize escape criteria}: Extract the hardcoded radius $R = 2$ and iteration limit into an ``escape criterion'' object, allowing alternative convergence theories (e.g., real dynamics in restricted domains).
  \item \textbf{Express escape via quantifiers}: Replace hand-rolled loops with declarative quantifier composition: $\text{escaped} \equiv \exists n \le N : |z_n| > R$, enabling composition with \texttt{exists(prefix(\ldots))$ from the sequences layer.
