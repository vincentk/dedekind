...
At runtime, the Python boundary follows an explicit fail-fast contract for invalid payloads. The current MVP binding accepts integer sequences for its exposed round-trip and path adapters; non-iterable inputs (where a sequence is required) and non-integral sequence members are rejected with Python \texttt{TypeError} exceptions. This contract is intentionally narrow and reviewable, and is verified in CI through dedicated unhappy-path smoke tests so reviewers can inspect both expected and observed failure behaviour directly in pull-request checks.
