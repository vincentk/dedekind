...
The same \#400 / \#407 contract on the C++ side is witnessed in the \texttt{:booleans} partition of \texttt{dedekind.numbers} and the \texttt{:expressions} partition of \texttt{dedekind.sets} (\texttt{Variable<S>::T = element\_of\_t<S>} via \texttt{:boundaries}; the \texttt{$\mathfrak{P}$} fraktur-P alias for \texttt{power\_set}). Per-symbol mirrors for $\mathbb{N}$, $\mathbb{Z}$, $\mathbb{R}$, $\mathbb{C}$, $\mathbb{D}$ are tracked under \#401--\#405 alongside the matching C++ carrier migrations.
