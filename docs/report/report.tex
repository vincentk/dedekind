...
Because a benchmark implementation must terminate with deterministic cost, the practical test target is the finite approximation:
\begin{equation}
\mathcal{M}_N = \left\{c \in \mathbb{C} \;\middle|\; z_0 = 0,\; z_{n+1} = z_n^2 + c,\; \forall n \in \{0,\ldots,N\}: |z_n| \le 2\right\}.
\end{equation}

The code listing in this report should be read as a structural realization of $\mathcal{M}_N$: the set expression captures the mathematical predicate, while the output packing step remains a conventional imperative loop to follow the benchmark-game output contract.
