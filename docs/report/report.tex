...
The Boolean species $\mathbb{B}$ as the base of the embedding chain.  \texttt{FiniteBooleanSetOf<L>} (alias for \texttt{dedekind.sets::FiniteBooleanSet<L>}) is the canonical characteristic-function carrier for $\mathbb{B}$ over a logic $L$ (default \texttt{ClassicalLogic}); \texttt{$\mathbb{B}$ = FiniteBooleanSetOf<>} is the user-facing alias.  The partition also exports a legacy \texttt{Is\_B<M, E>} concept that is currently \emph{not} witnessed on the canonical carrier (the concept body requires \texttt{m.cardinality()} of return type \texttt{std::size\_t}, but \texttt{FiniteBooleanSet}'s \texttt{cardinality()} returns the \texttt{Finite} tag); flagged with \texttt{FIXME(\#379)} as a surgery candidate.  The \texttt{Truth<L>} lifting of truth-values into numerical-species form lives in \texttt{dedekind.category:logic}, not in this partition.
