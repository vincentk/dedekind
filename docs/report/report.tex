...
\paragraph{Carrier-level structural witnesses on $\mathbb{B}$ and $\mathbb{N}$.} The \texttt{:booleans} and \texttt{:naturals} partitions in \texttt{dedekind.numbers} now pin three structural readings on each carrier alongside the algebraic witnesses already in place:
...
PR \#407 introduces the first child of the canonical-species spine on the Python side: \texttt{dedekind.numbers.boolean} mirrors the C++ partition \texttt{dedekind.numbers:booleans} so that callsites read the same way across the language boundary:
