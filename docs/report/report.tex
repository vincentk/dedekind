...
\texttt{algebra::IsFieldLikeScalar} pending species-trait specialisation on
the relevant carriers (FIXME(\#379) breadcrumbs in source).
...
The strict \texttt{Field\_$\mathbb{Q}$} bundle does \emph{not} fire on
\texttt{Rational<...>} today --- the species-trait registry is not specialised
on \texttt{Rational}; the operational \texttt{IsFieldLikeScalar} witness is
the load-bearing field-arithmetic guarantee, and the strict-and-literal seal
