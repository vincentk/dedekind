...
While "duck typing" served as a dynamic precursor to this idea, the \texttt{dedekind} use of C++ Concepts is not a mere type-checking variant of common Python practice.
Python duck typing is runtime and open-world: compatibility is accepted by executing method calls and discovering behavior dynamically.
In contrast, C++ Concepts in \texttt{dedekind} are compile-time contracts over structural requirements with explicit signatures (and, where needed, semantic witnesses), so invalid constructions are rejected before code generation.
