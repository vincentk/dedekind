...
The construction follows the Feynman/Grothendieck reading: $\mathbb{Z}$ is what one obtains by formally adjoining $\texttt{operator-}$ to $(\mathbb{N}, +)$; $\mathbb{Q}$ by adjoining $\texttt{operator/}$ to $(\mathbb{Z}, *)$; $\mathbb{R}$ by completing $\mathbb{Q}$ under suprema. The library's variant carriers \texttt{Cardinality} (the $\mathbb{N}$-proxy with $\aleph_0$ saturation) and \texttt{SignedCardinality} (the $\mathbb{Z}$-proxy with $\pm\aleph_0$ saturation and \texttt{NaZ}) make this construction direct: \texttt{Cardinality} ships $+, *, /, \%$ but deliberately omits $\texttt{operator-}$, holding the rig honest; \texttt{SignedCardinality} adds the unary minus and is consequently a ring. The carrier choices reify the structure-forcing axioms rather than collapsing them.
