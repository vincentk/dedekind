...
\section{Introduction}
The \texttt{dedekind.algebra} module builds upon \texttt{dedekind.sets} to implement rings, fields, semimodules, and modules via reduced-strength algebraic structures. A core design principle is to avoid the convenient \texttt{double} route and instead spell out the algebraic structure explicitly: from boolean rigs through unsigned-integer rings up to the complex-number field. As a corollary, everything in the library is ultimately a complex number---in the same way that everything is a string for front-end developers.

A recurring implementation challenge was the pressure to default to \texttt{double}; the module systematically resisted this in favour of explicit semimodules and semirings, ensuring reduced-strength structures are always named and certified before being promoted.
