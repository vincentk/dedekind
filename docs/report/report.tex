...
The finite fragment \texttt{ExtensionalCardinal<>} is multi-limb arbitrary-precision unsigned integer arithmetic; effectively bigint-flavoured. The $\aleph_0$ alternative marks the saturation regime that takes over once the \texttt{std::size\_t}-bound limb representation cannot hold the result. Strictly mathematically, $\mathbb{N} \cup \{\aleph_0\}$ is not a monoid in the textbook sense (left-cancellation fails on the saturating element), but the library's pragmatic certificate accepts it as a $\mathbb{N}$ proxy because the finite fragment \emph{is} $\mathbb{N}$ exactly and the saturation is the explicit overflow contract: ``if you compute beyond memory limits, you saturate at $\aleph_0$.'' Downstream code that requires strict-$\mathbb{N}$ behaviour tests the finite-fragment alternative via \texttt{std::holds\_alternative<ExtensionalCardinal<>>}.
...
\begin{itemize}
  ...
  \item The finite fragment \texttt{SignedExtensionalCardinal<>} is arbitrary-precision signed-magnitude --- effectively bigint-flavoured signed arithmetic.
\end{itemize}
