...
The general framework that lets every pair $(T_1, T_2)$ in the lattice compose under set operations is tracked as a Paper-3 prerequisite (issue \#362). The current PR ships the existential proof on the variant pair $(\texttt{Cardinality}, \texttt{SignedCardinality})$ --- the load-bearing case --- as a hand-coded overload pair on $\texttt{Set::operator\&}$ and $\texttt{Set::operator|}$, locked in by unit tests covering both the type-level collapse and the runtime semantics.
