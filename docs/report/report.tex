...
The current \texttt{dedekind.numbers:mandelbrot} partition now exhibits the desired separation of concerns.
The recurrence is fixed once (\texttt{mandelbrot\_orbit}), orbit-level criteria are lifted to parameter-level predicates (\texttt{bounded}), and stage-indexed computable approximations are exposed explicitly through \texttt{M\_tower}.
