...
\begin{itemize}
  ...
  \item \textbf{Subtraction} (\texttt{operator-}) is available when $R$ is invertible under
        addition (ring coefficients, e.g.~\texttt{int}, \texttt{double}).  The constraint is
        expressed directly in the function signature via \texttt{requires
        is\_invertible\_v<R, std::plus<R>>}.
