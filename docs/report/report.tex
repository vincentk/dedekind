...
\begin{tikzcd}[column sep=huge, row sep=large, ampersand replacement=\&]
  \texttt{Cardinality}~(\mathbb{N}) \arrow[r, "\texttt{lift\_cardinality\_to\_signed}"]
    \& \texttt{SignedCardinality}~(\mathbb{Z}) \arrow[l, dashed, bend right=30, "\texttt{abs}"'] \\
  \texttt{unsigned~int}~(\mathbb{Z}/2^w\mathbb{Z}) \arrow[u, "\texttt{embed\_unsigned\_to\_Cardinality}"] \arrow[r, "\texttt{embed\_}\mathbb{N}\texttt{\_}\mathbb{Z}~(\text{machine})"]
    \& \texttt{int}~(\text{machine}~\mathbb{Z}) \arrow[u, "\texttt{embed\_signed\_to\_SignedCardinality}"'] \\
