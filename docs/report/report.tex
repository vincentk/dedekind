...
\subsection{The prototypical carrier: \texorpdfstring{$\mathbb{F}_2$}{F2}}

\texttt{algebra:field} exports \texttt{𝔽2}, the Galois field of order two $\mathbb{F}_2 = \{0, 1\}$, as the prototypical concrete witness. The carrier is a single \texttt{bool} ($\mathtt{false} \mapsto 0$, $\mathtt{true} \mapsto 1$), with $+$ defined as XOR ($\hat{\phantom{}}$) and $\cdot$ as AND (\&\&). This literalises the textbook identification $\mathbb{F}_2 = (\text{bool}, \oplus, \wedge)$ at the type level; no modular reduction is needed because \texttt{bool} is already canonical-to-$\{0, 1\}$. Characteristic two gives $-x = x$; the multiplicative group $\mathbb{F}_2^{\times} = \{1\}$ is trivial, so $1^{-1} = 1$ and $a / 1 = a$. The struct registers the full species-trait chain---\texttt{identity\_trait} for $0$ and $1$, \texttt{is\_associative\_v} and \texttt{is\_commutative\_v} for both operations, \texttt{is\_distributive\_v} linking them, \texttt{is\_periodic} for totality, and \texttt{is\_invertible\_v} on both additive and multiplicative sides. The carrier passes \texttt{IsCommutativeRing}, \texttt{category::IsField}, and \texttt{algebra::IsField}, witnessed at compile time by \texttt{static\_assert}s alongside the struct and exhaustively at run time by \texttt{f2\_test.cpp}, which enumerates every pair $(a, b) \in \mathbb{F}_2^2$ against each field axiom (group laws for $+$ and $\cdot$, distributivity, division surface, inverse-of-zero throws).

The carrier is deliberately small: $|\mathbb{F}_2| = 2$ lets the whole Cayley table fit in a handful of \texttt{CHECK}s, so the axioms can be verified by exhaustion rather than by probe. Paper-facing carriers---\texttt{Rational<Z>}, \texttt{Complex<R>}, the Gaussian rationals---adopt the same registration pattern once their ring-trait chains and multiplicative-invertibility claims are registered (tracked under epic \#374, children \#371 axiom-hook auto-lifter and the new \#382 \texttt{FieldRegistration} helper).
...
The strict concepts \texttt{IsSemimodule<M, S>} and \texttt{IsModule<M, S>} compose from \texttt{IsCommutativeMonoid} / \texttt{IsAdditiveGroup} (on the carrier \texttt{M}), \texttt{IsSemiring} / \texttt{IsRing} (on the scalar \texttt{S}), and \texttt{IsLinearAction<S, M>}. \texttt{IsVectorSpace<V, F>} adds \texttt{IsField<F>} on the scalar side.
...
\subsection{Bona fide scalar / field element}
\label{subsec:algebra-modules-bona-fide}

A slogan of the project is that ``a scalar is a 1D semi-module over itself; an element of a field is a 1D vector space over itself''. Encoded as concepts:

\begin{itemize}
  \item \texttt{IsScalar<S>} $= $ \texttt{IsSemimodule<S, S>} (strict).
  \item \texttt{IsFieldElement<F>} $= $ \texttt{IsVectorSpace<F, F>} (strict).
  \item \texttt{IsScalarLike<S>} / \texttt{IsFieldElementLike<F>} --- operational counterparts using \texttt{IsSemimoduleLike} / \texttt{IsVectorSpaceLike}. These fire on machine-backed carriers such as \texttt{int}, \texttt{unsigned int}, \texttt{Rational<long>}.
\end{itemize}

The reified 1D carrier \texttt{OneDimensionalVector<S, Tag>} demonstrates the slogan concretely: a coordinate in the scalar species \texttt{S} with pointwise addition and scalar action, witnessed via \texttt{IsVectorSpaceLike<RealLine, RealLineScalar>} and \texttt{SatisfiesVectorSpaceAxioms} on the same carrier.
