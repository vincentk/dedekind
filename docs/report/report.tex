...
\subsection{The Initial-Ring Form: \texorpdfstring{$\mathbb{Z}$}{Z} as the initial object in Ring}
\label{subsec:initial-ring}

\textbf{$\mathbb{Z}$ as a Form, not a construction.} The categorical reading of $\mathbb{Z}$ mirrors the NNO story for $\mathbb{N}$: $\mathbb{Z}$ is the \emph{initial object in the category of (commutative, unital) rings}~\cite{maclane1971categories}. For every ring $R$ there exists a \emph{unique} ring homomorphism $\chi_R : \mathbb{Z} \to R$ sending $1_\mathbb{Z} \mapsto 1_R$ and determined uniquely by ring-axiom preservation. The carrier is one realisation; the universal property is what is named. The library reifies this directly as an \texttt{IsInitialRing<R>} concept (placement TBD between \texttt{dedekind.algebra:ring} and a sibling \texttt{dedekind.category} partition), with the existence-and-uniqueness of $\chi_R$ as the engineer's honesty obligation.

\textbf{Equivalent reading: the Grothendieck completion.} The initial-object characterisation of $\mathbb{Z}$ is equivalent (up to canonical isomorphism) to the \emph{Grothendieck completion} of the commutative monoid $(\mathbb{N}, +, 0)$: $\mathbb{Z}$ is the free abelian group on $\mathbb{N}$, the left adjoint of the forgetful functor $\mathrm{Ab} \to \mathrm{CMon}$ applied at $\mathbb{N}$. Concretely $\mathbb{Z} = \mathbb{N} \times \mathbb{N} / \sim$ with $(a, b) \sim (c, d) \iff a + d = b + c$; the equivalence class $[(a, b)]$ represents the integer $a - b$. The closure-forcing binary subtraction $\texttt{Cardinality} - \texttt{Cardinality} \to \texttt{SignedCardinality}$ realises this construction at the operator level: the operator is well-defined as a function $\mathbb{N} \times \mathbb{N} \to \mathbb{Z}$, not into $\mathbb{N}$, so the codomain widens, and the lift carries the Grothendieck construction operationally.

\textbf{Three-layer chain.} The architecture mirrors the NNO three-layer chain. \texttt{IsInitialRing} is the Form. \texttt{SignedCardinality} (the variant $\mathbb{Z}$-proxy with $\pm\aleph_0$ saturation and \texttt{NaZ}, in \texttt{dedekind.sets:cardinality}) is the canonical carrier inhabiting the Form, certified by a \texttt{static\_assert} witness exercising the universal property on at least one concrete target ring (\texttt{Modular<n>} is the natural choice; the unique ring homomorphism $\chi_{\mathrm{Modular}<n>}$ is the mod-$n$ reduction). $\mathbb{Z}$ is the alias practitioners use, pointing to \texttt{SignedCardinality} once the variant-carrier alias migration is complete.
