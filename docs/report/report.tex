...
\texttt{Cardinality = std::variant<ExtensionalCardinal<>, $\aleph_0$>} is the user-facing cardinality value. Overflow promotion is explicit:
\texttt{add\_or\_}$\aleph_0$ and \texttt{mul\_or\_}$\aleph_0$ return a \texttt{variant} that holds $\aleph_0\{\}$ whenever the checked arithmetic overflows, making the transition from finite to transfinite arithmetic a type-visible event rather than silent wraparound.
