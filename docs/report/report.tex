...
This partition re-exports \texttt{dedekind.sets} and adds the two proofs that
require the \texttt{numbers} and \texttt{order} layers:
\texttt{IsNatural<ExtensionalCardinal<>>} and
\texttt{IsTotallyOrdered<ExtensionalCardinal<>>}.
The concrete cardinality types themselves---\texttt{Finite}, $\aleph\texttt{<N>}$,
$\aleph_0$, $\beth_1$, \texttt{ExtensionalCardinal<N>}, and the
\texttt{Cardinality} sum type---are defined one layer below in
