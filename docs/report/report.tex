...
The Boolean species $\mathbb{B}$ as the base of the embedding chain.  \texttt{FiniteBooleanSetOf<L>} (alias for \texttt{dedekind.sets::FiniteBooleanSet<L>}) is the canonical characteristic-function carrier for $\mathbb{B}$ over a logic $L$ (default \texttt{ClassicalLogic}).  \texttt{Is\_B<M, E>} names a structure as a Boolean semiring with $|S| = 2$.  The \texttt{Truth<L>} lifting of truth-values into numerical-species form lives in \texttt{dedekind.category:logic}, not in this partition.
...
the new \texttt{SignedCardinality} (PR \#396) ${\pm}\aleph_0$-escalating
