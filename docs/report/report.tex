...
\paragraph{The embedding ladder $\mathbb{B} \to \mathtt{std::unsigned\_integral} \to \mathbb{N}$.} The machine layer mediates between $\mathbb{B}$ (\texttt{bool}, the truth-value carrier; PR~\#407 / issue~\#400) and $\mathbb{N}$-as-\texttt{Cardinality}. The arrow $\mathbb{B} \hookrightarrow \texttt{std::unsigned\_integral}$ is the truthy-positivity injection (\texttt{embed\_}$\mathbb{B}$\texttt{\_}$\mathbb{N}$ at the machine layer, mapping \texttt{false} $\mapsto 0$, \texttt{true} $\mapsto 1$); the arrow $\texttt{std::unsigned\_integral} \to \mathbb{N}$ (called \texttt{embed\_unsigned\_to\_Cardinality}, named explicitly post-issue~\#417) is the structure-forgetting lift from the modular ring into the saturating $\mathbb{N}$-proxy. The composition is the textbook embedding of the Boolean truth-values into the natural numbers, factored through the machine layer. The $\mathbb{K3} \to \texttt{std::signed\_integral} \to \mathbb{Z}$ chain is the symmetric three-element / Kleene-truth-value version, with \texttt{embed\_K3\_}$\mathbb{Z}$ as the first leg and the analogous lift \texttt{embed\_signed\_to\_SignedCardinality} as the second; the symmetric story is tracked under issue~\#418.
