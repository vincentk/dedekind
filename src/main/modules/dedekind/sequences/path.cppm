...
struct Path {
    ...
    friend constexpr auto operator>>=(const Path& m, F&& f) {
        ...
        return Path<U>{
            [m, f = std::forward<F>(f)](std::size_t n) {
    ...
    friend constexpr auto operator<<=(const Path& w, F&& f) {
        ...
        return Path<U>{
            [w, f = std::forward<F>(f)](std::size_t n) {
...
static_assert(IsCoKleisliExtension<Path, int, int>,
...
static_assert(IsArrow<decltype(fmap<Path>(id<int>())), Path<int>, Path<int>>,
