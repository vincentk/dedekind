...
using CanonicalIntSetObject = decltype(ambient_set<int>([](int) { return true; }));

static_assert(IsSet<CanonicalIntSetObject>,
              "Canonical ambient_set<int>(...) must satisfy ETCS IsSet.");
static_assert(HasAxiom5CartesianProduct<typename CanonicalIntSetObject::Ambient>,
              "ETCS IsSet ambient species must satisfy cartesian product witness.");
static_assert(HasAxiom6Exponentiation<typename CanonicalIntSetObject::Ambient>,
