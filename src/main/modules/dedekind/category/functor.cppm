 ...
 * @partition :functor
 * @brief The Functorial Spine (Structure Preservation).
 *
 ...
 *      universal-algebra @c (A, F) reading.  Per the textbook
 *      isomorphism @c Alg(Σ) @c ≅ @c F_Σ-Alg (Pierce 1991, @em Basic
 *      @em Category @em Theory @em for @em Computer @em Scientists §2.2):
 *      every universal-algebra @c IsAlgebra<T, Ops...> corresponds to
 *      an @c IsFAlgebra here for the polynomial endofunctor
 *      @c F_Σ generated by the signature @c Σ @c = @c (Ops...).
 */
...
/**
 * @brief The @b closure of @c x under @c expand: the textbook "closure of @c X
 *        under @c Y".  It is the least fixpoint of @c S @c ↦ @c x @c ⊔ @c
 *        expand(S) --- the smallest element @c ⊒ @c x closed under the one-step
 *        expansion @c expand --- equivalently the Kleene join @c ⨆ₙ @c
 * expandⁿ(x).
 *
 * @details One operator over many constructions (@e proof @e by @e
 *          implementation): reflexive-transitive closure @c R* @c = @c
 *          closure(Δ, @c λS.@c S;R); reachability = its Boolean instance;
 *          semiring / critical-path closure swaps the join; @c subgroup /
 *          @c span / @c convex-hull are @c closure(X, @c ·op·) --- the
 *          universal-algebra "generated by".  @c join is the @c ⊔ (union / the
 *          semilattice join); @c expand applies @c Y once.
 *
 *          Decidable exactly where the carrier @b stabilises (finite / @c
 *          IsExtensional, so @c equal detects the fixpoint) or under the @c
 *          max_iterations bound --- the @c N-th Kleene approximant, i.e. the
 *          finite-subgraph selection a partial evaluation performs before an
 *          infinite structure's closure can be computed at all.
 */
export template <typename T, typename Expand, typename Join,
                 typename Equal = std::equal_to<>>
  requires std::predicate<Equal, T, T>
constexpr T closure(const T& x, Expand expand, Join join, Equal equal = {},
                    std::size_t max_iterations = 1024) {
  return least_fixpoint(
      x, [&x, expand, join](const T& s) -> T { return join(x, expand(s)); },
...
// functors that preserve associativity / commutativity / distributivity
// / saturation) was previously declared here.  It now lives at home
