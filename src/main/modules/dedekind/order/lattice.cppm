...
/** @brief Reachability / the universal-algebra "generated by", over the
 *  power-set lattice: the reflexive-transitive closure of a bitmask @c seed
 *  under a one-step adjacency @c step --- the least bitmask @c ⊒ @c seed (in
 *  @c bit_subset_eq) that is closed under @c step, i.e. @c ⨆ₙ @c stepⁿ(seed)
 *  with join the lattice's own @c bit_or.  It is the Boolean instance of the
 *  generic @c closure (@c category:functor) over @b this lattice; decidable
 *  because the finite bit-carrier stabilises.  Reusing it: reachability of a
 *  graph whose nodes are bit positions and @c step is the adjacency. */
export template <typename T, typename Step>
  requires std::is_integral_v<T>
constexpr T bit_closure(T seed, Step step) {
  return dedekind::category::closure(seed, step, std::bit_or<T>{});
}

namespace detail_bit_closure {
// Witness: reachability on the 4-node path 0→1→2→3 (nodes as bit positions).
// From {0} the whole path {0,1,2,3} is reachable; from {2} only {2,3}.
...
static_assert(bit_closure(0b0001u, edge_step) == 0b1111u,
              "bit_closure of {0} under the edge step is {0,1,2,3}.");
static_assert(bit_closure(0b0100u, edge_step) == 0b1100u,
              "from {2} only {2,3} are reachable — the closure stabilises.");
}  // namespace detail_bit_closure
