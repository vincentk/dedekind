...
export template <typename T, typename Op> inline constexpr auto identity_v = T(0);

// --- 2. THE THEOREMS (Standard Type Proofs) ---
// These MUST be declared here so Concepts can see them immediately.

// Addition (ints)
template <std::integral T> inline constexpr bool is_associative_v<T, std::plus<T>> = true;
template <std::integral T> inline constexpr bool is_commutative_v<T, std::plus<T>> = true;

// Multiplication (ints)
template <std::integral T> inline constexpr bool is_associative_v<T, std::multiplies<T>> = true;
template <std::integral T> inline constexpr bool is_commutative_v<T, std::multiplies<T>> = true;
template <std::integral T> inline constexpr auto identity_v<T, std::multiplies<T>> = T(1);
...
// Strings
template <> inline constexpr bool is_associative_v<std::string, std::plus<std::string>> = true;
template <> inline constexpr auto identity_v<std::string, std::plus<std::string>> = std::string("");
...
export template <typename T, typename Op>
concept IsMagma = requires(T a, T b) { { Op{}(a, b) } -> std::convertible_to<T>; };

export template <typename T, typename Op>
concept IsSemigroup = IsMagma<T, Op> && is_associative_v<T, Op>;

export template <typename T, typename Op>
concept IsMonoid = IsSemigroup<T, Op> && requires { { identity_v<T, Op> } -> std::convertible_to<T>; };

export template <typename T, typename Op>
concept IsGroup = IsMonoid<T, Op> && requires(T a) { { inverse<T, Op>(a) } -> std::convertible_to<T>; };
...
concept IsRing = IsAbelianGroup<T, Add> && IsMonoid<T, Mul> && requires(T a, T b, T c) {
...
concept IsField = IsRing<T, Add, Mul> && 
                  is_commutative_v<T, Mul> && 
