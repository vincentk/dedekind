...
/**
 * @brief The Dedekind Cut: Representing a Real number as a partition of Rationals.
 * @tparam Q An Ordered Field (typically the Rational species).
 */
template <ontology::IsOrderedField Q>
  requires ontology::IsDense<Q> && ontology::IsAleph0<typename Q::cardinality_type>
struct DedekindCut {
...
constexpr bool operator<(const DedekindCut<Q>& a, const DedekindCut<Q>& b) {
    // This is symbolic: there must exist a rational q such that q is in B but not A.
    // In a "Best Effort" library, this triggers a search/proof.
