...
/** @brief Generic additive inverse for a @c :sets carrier that is an additive
 *  group and supports unary negation: the @c +-inverse @b is @c -x, inferred
 *  from the group structure rather than registered per carrier.
 *
 ...
 *  @b computation bridge --- and any sibling additive-group carrier, so the
 *  additive inverse need not be hand-registered per type.  The
 *  @c ExtensionalCardinal<> overload above is a more-specialised non-template
 *  and still wins for that carrier. */
export template <typename T>
  requires dedekind::category::IsGroup<T, std::plus<T>> && requires(T x) {
