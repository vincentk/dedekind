...
TEST_CASE("category:heyting — Bool is the canonical 2-element Heyting lattice",
  ...
  constexpr HeytingExponential<bool, std::less_equal<bool>, Inf> e_top{true};
  STATIC_CHECK(e_top(false) == false);
  STATIC_CHECK(e_top(true) == true);
  // value = false (= bottom), operator()(x) = false for every x.
  constexpr HeytingExponential<bool, std::less_equal<bool>, Inf> e_bot{false};
...
TEST_CASE("category:heyting — integral carriers are Heyting lattices",
  ...
  constexpr HeytingExponential<int, std::less_equal<int>, Inf> e_int{42};
...
TEST_CASE(
  ...
  STATIC_CHECK(
      IsExponential<HeytingExponential<bool, std::less_equal<bool>, Inf>, bool,
                    bool>);
  STATIC_CHECK(IsExponential<HeytingExponential<int, std::less_equal<int>, Inf>,
                             int, int>);
}
