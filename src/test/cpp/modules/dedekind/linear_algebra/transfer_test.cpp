...
// as the bra — plain Ket/Bra literals (both IsArrow), not bespoke index→scalar
// structs.  The same shape serves max-plus and min-plus.
...
TEST_CASE("transfer: tropical bra·ket eigenvalue and rank-1 collapse",
  ...
  const Ket<MP, 2> v{{MP::of(3), MP::of(5)}};
  const Bra<MP, 2> w{{MP::of(5), MP::of(2)}};

  // λ = ⟨w|v⟩ = max(3+5, 5+2) = 8 — the long branch wins.
  const MP lambda = inner_product<2>(w, v);
  CHECK(lambda == MP::of(8));

  const OuterProduct<Ket<MP, 2>, Bra<MP, 2>, Mult> bead{v, w};
...
TEST_CASE("transfer: the semiring is the choice of problem",
  ...
  SECTION("𝔹 ⟹ reachability: is a branch present end to end?") {
    const Ket<bool, 2> all_present{{true, true}};      // v: both branches
    const Bra<bool, 2> only_first{{true, false}};      // w: only the k=0 branch
    const Bra<bool, 2> none_present{{false, false}};   // w: no branch
