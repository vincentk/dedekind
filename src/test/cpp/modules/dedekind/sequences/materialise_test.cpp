 ...
 *
 * The last plank of the intensional → intensional-finite → materialise →
 * extensional bridge: `materialise` realises a finite (IsExtensional) interval
 ...
 * it cannot reach `materialise` at all — the Rice wall made structural.
 */
...
TEST_CASE("materialise: a closed interval becomes its ExtensionalSet",
          "[sequences][ranges][materialise]") {
  constexpr Prefix5 oi{};
  const auto ext = materialise(oi);
...
TEST_CASE("materialise: the filtered form realises argmax over a finite domain",
          "[sequences][ranges][materialise]") {
  constexpr Prefix5 oi{};
  ...
  const auto evens = materialise(oi, [](int x) { return x % 2 == 0; });
  ...
  const auto sole = materialise(oi, [](int x) { return x == 3; });
  CHECK(sole.size() == 1);
  CHECK(sole.contains(3));

  const auto empty = materialise(oi, [](int) { return false; });
  CHECK(empty.size() == 0);
}

TEST_CASE("materialise(argmax(interval, cost)): the endorsed one-liner",
...
TEST_CASE("materialise<N>: a sequence realises to a positional std::array",
          "[sequences][ranges][materialise]") {
  ...
  constexpr auto vec = materialise<4>(squares{});
  static_assert(
      std::same_as<std::remove_cvref_t<decltype(vec)>, std::array<int, 4>>,
      "sequence materialise yields a std::array, not a std::set.");
  CHECK(vec[0] == 0);
  CHECK(vec[1] == 1);
  CHECK(vec[2] == 4);
  CHECK(vec[3] == 9);
