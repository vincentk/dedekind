...
- Priority by PR state:
  - Draft PR: agents SHOULD prioritize narrowing the gap between current source implementation and ticket acceptance criteria.
  - Draft execution loop: assess acceptance criteria coverage, implement highest-impact missing criteria, add/adjust tests, then push a small checkpoint.
  - Near-Ready/Ready PR: agents SHOULD shift priority toward polishing (documentation/report alignment, clarity, and low-risk quality refinements).
