...
- Prefer approved repository `make` targets over ad hoc command sequences whenever possible.
- All commands listed in [`commands.instructions.md`](commands.instructions.md) are pre-approved for this workspace and may be executed without per-invocation confirmation.
- Do not run commands that require manual approval in this environment. If needed functionality is missing, add a clean helper script or `make` target and use that instead.
