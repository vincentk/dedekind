...
- While a PR is in draft state, agents SHOULD prioritize work that narrows the gap between the current source implementation and the ticket acceptance criteria.
- For draft PR execution, agents SHOULD sequence work as: assess acceptance criteria coverage, implement the highest-impact missing criteria, add/adjust tests for those criteria, then push a small checkpoint.
